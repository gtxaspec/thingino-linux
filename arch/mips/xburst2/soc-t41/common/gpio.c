...
#define PXPDS2		0x150   /* Port Driver-strength 2  Register,Only PA support */
#define PXPDS2S		0x154   /* Port Driver-strength 2 Set Register,Only PA support */
#define PXPDS2C		0x158   /* Port Driver-strength 2 Clear Register,Only PA support */
#define PXPSLW		0x160   /* Port Slew Rate Register, Only PA & PC support */
#define PXPSLWS		0x164   /* Port Slew Rate Set Register, Only PA & PC support */
#define PXPSLWC		0x168   /* Port Slew Rate Clear Register, Only PA & PC support */
			  ...
			  enum gpio_function func, unsigned int pins)
{
   ...
   if((func&0xf0) == 0x90) { /*pull up*/
        comp = readl(chip->reg + PXPUEN);
		comp &= ~pins;
		comp |= pins;
		writel(comp, chip->reg + PXPUEN);
    } else if((func&0xf0) == 0xa0) { /*pull down*/
		comp = readl(chip->reg + PXPDEN);
		comp &= ~pins;
		comp |= pins;
        writel(comp, chip->reg + PXPDEN);
	}else if((func&0xf0) == 0xc0){ /*pull down Disable*/
        comp = pins;
        writel(comp, chip->reg + PXPDENC);
    }
