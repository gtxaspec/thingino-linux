...
static int enable_restore_fp_context(unsigned int from)
{
	...
	if (!(from & MSA) && !thread_msa_context_live())
		return own_fpu(1);
...
asmlinkage void do_cpu(struct pt_regs *regs)
{
	...
	case 1:
		err = enable_restore_fp_context(FPU);
	...
	case 2:
		...
		//err = enable_restore_fp_context(MSA);
...
asmlinkage void do_msa_fpe(struct pt_regs *regs, unsigned int msacsr)
{
	...
	prev_state = exception_enter();
	trap_nr = (regs->cp0_cause >> 2) & 0x1f;
	if (notify_die(DIE_MSAFP, "MSA FP exception", regs, 0,
		       trap_nr, SIGFPE) == NOTIFY_STOP)
		goto out;
...
asmlinkage void do_msa(struct pt_regs *regs)
{
	...
	die_if_kernel("do_msa invoked from kernel context!", regs);
        ...
        /* Enable both MSA and MXA to avoid calling restore_msa from MXA exception while MSA registers are in use. */
	err = enable_restore_fp_context(MSA);
	if (err)
		force_sig(SIGILL, current);
out:
