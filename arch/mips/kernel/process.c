...
{
        int i;
	u64 fpr_val[NUM_FPU_REGS];

	if (sizeof(current->thread.fpu.fpr[i]) == sizeof(elf_fpreg_t)) {
	  memcpy(r, &current->thread.fpu, sizeof(current->thread.fpu));
	  goto out;
	}

	for (i = 0; i < NUM_FPU_REGS; i++)
	  fpr_val[i] = get_fpr64(&current->thread.fpu.fpr[i], 0);
	memcpy(r, fpr_val, NUM_FPU_REGS);
...
int dump_task_fpu(struct task_struct *t, elf_fpregset_t *fpr)
{
	...
	if (sizeof(t->thread.fpu.fpr[i]) == sizeof(elf_fpreg_t)) {
	  memcpy(fpr, &t->thread.fpu, sizeof(t->thread.fpu));
	  goto out;
	}

	for (i = 0; i < NUM_FPU_REGS; i++)
	  fpr_val[i] = get_fpr64(&t->thread.fpu.fpr[i], 0);
	memcpy(fpr, fpr_val, NUM_FPU_REGS);
