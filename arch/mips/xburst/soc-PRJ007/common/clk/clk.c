...
static unsigned long set_cpu_freqs[] = {
	...
	792000,1000000, 1008000,118800, 1200000,1400000,
	CPUFREQ_TABLE_END
};

#ifdef CONFIG_PM
struct ingenic_clk_reg_sleep clk_reg_sleep[] = {
	REG_SLEEP(CPM_VPUCDR,		29, 28, 0),
	REG_SLEEP(CPM_RSACDR,		29, 28, 0),
	REG_SLEEP(CPM_MACCDR,		29, 28, 0),
	REG_SLEEP(CPM_MACPHY,		29, 28, 0),
	REG_SLEEP(CPM_LPCDR,		29, 28, 0),
	REG_SLEEP(CPM_SFC,		29, 28, 0),
	REG_SLEEP(CPM_SFC1,		29, 28, 0),
	REG_SLEEP(CPM_MSC0CDR,		29, 28, 0),
	REG_SLEEP(CPM_MSC1CDR,          29, 28, 0),
	REG_SLEEP(CPM_SSI,		29, 28, 0),
	REG_SLEEP(CPM_SSI_SLV,		29, 28, 0),
	REG_SLEEP(CPM_CIMCDR,		29, 28, 0),
	REG_SLEEP(CPM_ISPSCDR,		29, 28, 0),
	REG_SLEEP(CPM_ISPMCDR,		29, 28, 0),
	REG_SLEEP(CPM_ISPACDR,		29, 28, 0),
	REG_SLEEP(CPM_TNPU,		29, 28, 0),
	REG_SLEEP(CPM_PWM,		29, 28, 0),
	REG_SLEEP(CPM_I2SSPKCDR,	0,  0,  0),
	REG_SLEEP(CPM_I2SSPKCDR1,	0,  0,  0),
	REG_SLEEP(CPM_I2SMICCDR,	0,  0,  0),
	REG_SLEEP(CPM_I2SMICCDR1,	0,  0,  0),
	REG_SLEEP(CPM_CLKGR,            0 , 0,  0),
	REG_SLEEP(CPM_CLKGR1,           0 , 0,  0),

};
#define NR_CLK_REGS ARRAY_SIZE(clk_reg_sleep);
#endif/*CONFIG_PM*/

static int clk_suspend(void)
{
#ifdef CONFIG_PM
	struct ingenic_clk_reg_sleep *save_clk = clk_reg_sleep;
	unsigned int num_regs = NR_CLK_REGS;
	for (; num_regs > 0; --num_regs, ++save_clk){
		save_clk->value = cpm_inl(save_clk->offset);
	}
	printk("clk suspend!\n");
#endif
	return 0;
...
static void clk_resume(void)
{
	...
	int bit = 0;
	struct clk *resume_clk_id = NULL;
	struct ingenic_clk_reg_sleep *resume_clk = clk_reg_sleep;
	unsigned int clkgr[2] = {CPM_CLKGR,CPM_CLKGR1};
	unsigned int num_regs = NR_CLK_REGS;
	for (; num_regs > 0; --num_regs, ++resume_clk) {
		if(!resume_clk->ce && !resume_clk->busy) {
			cpm_outl(resume_clk->value, resume_clk->offset);
		} else {
			if(resume_clk->offset == CPM_TNPU){
				resume_clk_id = get_clk_from_id(CLK_ID_TNPU);
				bit = CLK_GATE_BIT(resume_clk_id->flags);
				cpm_clear_bit(bit % 32, clkgr[bit / 32]);
			}
			if(resume_clk->offset == CPM_MSC0CDR){
				//The clock control needs to be enabled before restoring the MSC0 clock.
				outl(0x7, MSC0_IOBASE + SDHCI_CLOCK_CONTROL);
			}
			if(resume_clk->offset == CPM_MSC1CDR){
				//The clock control needs to be enabled before restoring the MSC1 clock.
				outl(0x7, MSC1_IOBASE + SDHCI_CLOCK_CONTROL);
			}

			cpm_outl(resume_clk->value | (1 << resume_clk->ce), resume_clk->offset);
			timeout = 0xffffff;
			while(--timeout && cpm_inl(resume_clk->offset) & (1 << resume_clk->busy));
			if(!timeout) {
				printk("Error restore clk regs: %x, value: %x\n", resume_clk->offset, resume_clk->value);
			}
		}
	}
	printk("clk resume!\n");
...
void __init init_all_clk(void)
{
	...
	for(i = 0; i < clk_srcs_size; i++) {
		...
		if(clk_srcs[i].flags & CLK_FLG_DIV) {
			init_div_clk(&clk_srcs[i]);
		}
...
void clk_disable(struct clk *clk)
{
	if(!clk)
		return;
	if((strcmp(clk->name, "h0clk") == 0) || (strcmp(clk->name, "h2clk") == 0) || (strcmp(clk->name, "pclk") == 0)) {
		return;
	}
    if((strcmp(clk->name, "apll") == 0) || (strcmp(clk->name, "sclka") == 0) || (strcmp(clk->name, "mpll") == 0) || (strcmp(clk->name, "vpll") == 0)){
		return;
    }
	if(clk->count == 0)
		return;
	...
	if(m->private != NULL) {
		...
		for(i = 0; i < get_clk_sources_size(); i++) {
			if (clk_srcs[i].name == NULL) {
				...
				if(clk_srcs[i].flags & CLK_FLG_ENABLE)
					mhz = clk_srcs[i].rate / 1000;
				else
					mhz = 0;
				len += seq_printf(m,"%2d %-12s %4d.%03dMHz %3sable	 %d %s\n",i,clk_srcs[i].name
						, mhz/1000, mhz%1000
...
static ssize_t clock_set(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
	char clock[10];
	unsigned int rate = 0;
	int ret = 0;
	...
	if(copy_from_user(buf, buffer, count)) {
		kfree(buf);
		printk("**********use help**********\n");
		printk("echo pll_set sclka 1400000000 > /proc/jz/clock/clocks, to set apll/mpll/vpll\n");
		printk("echo pll_get sclka > /proc/jz/clock/clocks, to set apll/mpll/vpll\n");
		return EFAULT;
	}

	if (!strncmp(buf, "pll_set", sizeof("pll_set")-1)) {
		ret = sscanf(buf, "pll_set %s %i", clock, &rate);
		if (2 != ret){
			printk("**********use help**********\n");
			printk("echo pll_set sclka 1400000000 > /proc/jz/clock/clocks, to set apll/mpll/vpll\n");
			return EFAULT;
		}
		clks = clk_get(NULL, clock);
...
static int __init init_clk_proc(void)
{
	...
	cpu_clk = clk_get(NULL, "cpuclk");
	if (IS_ERR(cpu_clk)) {
		printk("ERROR:cpuclk request fail!\n");
		return -1;
	}
