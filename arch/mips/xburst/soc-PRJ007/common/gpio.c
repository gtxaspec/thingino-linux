...
static void check_gpio_conflicts(struct jzgpio_chip *jz, unsigned int pins)
{
	unsigned int offset;
	...
	for (offset = 0; offset < 32; offset++) {
		...
		if (jz->dev_map[0] & (1 << offset)) {
			printk(KERN_DEBUG "gpio functions has redefinition: port %d pin %d\n", jz->gpio_chip.base / 32, offset);
		}
	}
}

static void update_gpio_status(struct jzgpio_chip *jz, unsigned int pins)
{
	unsigned int offset;
			  ...
			  enum gpio_function func, unsigned int pins)
{
	...
	if ((func != GPIO_PULL_UP) && (func != GPIO_PULL_DOWN) && (func != GPIO_PULL_HIZ)) {
		check_gpio_conflicts(chip, pins);
		update_gpio_status(chip, pins);
	}
#endif
	...
	else
	{
		if((func == GPIO_INT_LO)||(func == GPIO_INT_HI)||(func == GPIO_INT_FE)||(func == GPIO_INT_RE))
		{
			...
			writel(1, chip->reg + PXUPD);
			writel(1, chip->reg + PXSHDC);
		}
		else
		{
...
static int jz_gpio_request(struct gpio_chip *chip, unsigned offset)
{
	...
	if(!test_bit(offset, jz->gpio_map)) {
		printk(KERN_WARNING "gpio has conflict in request\n");
		return -EINVAL;
	}
	if(jz->dev_map[0] & (1 << offset)) {
		dump_stack();
		printk("%s:gpio functions has redefinition: port %d pin %d\n", __FILE__, jz->gpio_chip.base / 32, offset);
	}
	jz->dev_map[0] |= 1 << offset;
