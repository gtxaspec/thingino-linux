...
int bcm_wlan_power_on(int flag)
{
	...
	printk("F:%s  WL_REG_EN = 1 L:%d\n",__func__,__LINE__);
	usleep_range(1, 1000);
	gpio_direction_output(WL_REG_EN, 0);
	printk("F:%s  WL_REG_EN = 0 L:%d\n",__func__,__LINE__);
	usleep_range(1, 1000);
	gpio_direction_output(WL_REG_EN, 1);
	printk("F:%s  WL_REG_EN = 1 L:%d\n",__func__,__LINE__);
	msleep(5);
