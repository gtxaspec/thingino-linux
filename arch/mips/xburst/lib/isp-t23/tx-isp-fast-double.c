...
/* 0：un-direct(default configuration)  2: double sensor direct */
static int zrt_direct_mode = 2;
...
static int tx_isp_riscv_hf_resize(void)
{
	...
	int on = 0;
    int enable;
	struct v4l2_input inputt;
	struct v4l2_input inputt1;
    ...
    ispdev->active_link[0] = -1;
    ispdev->active_link[1] = -1;
    /* isp open */	
    ...
    if(sensor_number == 2){
        ...
        // Set NCU buf
        ret = tx_isp_get_buf(module, (unsigned long)&g_ncu_buf1, K_MODE);
        if(ret < 0) {
            printk("tx_isp_get_buf failed\n");
            return ret;
        }
        ...
        sprintf(ncu_1_buf_len,"len=%d", g_ncu_buf1.size);
        ret = tx_isp_set_buf(module, (unsigned long)&g_ncu_buf1, K_MODE);
        if(ret < 0) {
            printk("tx_isp_set_buf failed\n");
            return ret;
        }
...
//	}
    ...
    index = -1;
    ret = tx_isp_sensor_get_input(module, (unsigned long)&index, K_MODE);
    if(ret < 0){
        printk("%s failed \n", __func__);
        return ret;
    }
    ...
    if(index == -1)
        printk("----> step %s %d sensor hasn't been added \n");

    enable = 1;
    ret = tx_isp_video_s_stream(module, enable, K_MODE);
    if(ret < 0) {
    ...
    index = 0;
    ret = tx_isp_video_link_setup(module, (unsigned long)&index, K_MODE);
    if(ret < 0) {
	...
	on = 1;
    ret = tx_isp_video_link_stream(module, on, K_MODE);
    if(ret < 0) {
        printk("tx_isp_video_link_setup failed\n");
        return ret;
    }
