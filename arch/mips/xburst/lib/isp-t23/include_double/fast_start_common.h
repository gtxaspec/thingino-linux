#ifndef __SENSOR_FAST_START_COMMON_H__
#define __SENSOR_FAST_START_COMMON_H__

#include <linux/kernel.h>

#define Zeratul_Platform 0				/* 0:dircet 1:undirect*/
#define TX_ISP_FAST_START 1   /* ZRT PLATFORM */
#define TX_ISP_NRVBS 2
#define TX_ISP_FRAME_BUFFER_IGNORE_NUM 1    /* Drop frames in order */
#define FRAME_CHANNEL_DEF_WIDTH 1920
#define FRAME_CHANNEL_DEF_HEIGHT 1080

#define SENSOR_DAY_FPS 15   /* Daytime frame rate */
#define SENSOR_NIGHT_FPS 10 /* Night frame rate */

#define SENSOR_HIGH_FRAME_RATE_MODE 1 /* Sensor High frame rate small image */
#define SENSOR_HIGH_FRAME_RATE_MODE_CHANGE_NUM 5  /* Number of small image frames */

#define DEBUG_TTFF(a) printk("TTFF %s:%d %s:%d\n", __func__, __LINE__, (a==NULL? " " : a), 1000 * *(volatile u32 *)0xb0002078 * 16 / ((24 * 1000 * 1000) / 512))

#define K_MODE 1
#define U_MODE 0

#define ALIGN_BLOCK_SIZE (1 << 12)
#define ALIGN_ADDR(base) (((base)+((ALIGN_BLOCK_SIZE)-1))&~((ALIGN_BLOCK_SIZE)-1))
#define ALIGN_SIZE(size) (((size)+((ALIGN_BLOCK_SIZE)-1))&~((ALIGN_BLOCK_SIZE)-1))

char * get_sensor_name(void);
int get_sensor_i2c_addr(void);
int get_sensor_width(void);
int get_sensor_height(void);
int get_sensor_wdr_mode(void);
char * get_sensor1_name(void);                                                                  
int get_sensor1_i2c_addr(void);
int get_sensor1_width(void);
int get_sensor1_height(void);
int get_sensor1_wdr_mode(void);

#ifdef CONFIG_BUILT_IN_SENSOR_SETTING
uint8_t *get_sensor_setting(void);
int get_sensor_setting_len(void);
char *get_sensor_setting_date(void);
char *get_sensor_setting_md5(void);
uint8_t *get_sensor1_setting(void);
int get_sensor1_setting_len(void);
char *get_sensor1_setting_date(void);
char *get_sensor1_setting_md5(void);
#endif

int init_sensor(void);
int init_sensor1(void);
void exit_sensor(void);
void exit_sensor1(void);

struct tx_isp_callback_ops {

	/**
	 * Kernel starts ISP work
	 * Return value: 0 successful, 1 failed
	 */
	int (*tx_isp_start_device)(struct tx_isp_module *module);

	/**
	 * Kernel high frame rate small image mode, Switch process
	 * Return value: 0 successful, 1 failed
	 */
	int (*tx_isp_kernel_hf_resize)(void);

	/**
	 * Riscv high frame rate small image mode, Switch preparation
	 * Return value: 0 successful, 1 failed
	 */
	int (*tx_isp_riscv_hf_prepare)(void);

	/**
	 * Riscv high frame rate small image mode, Switch process
	 * Return value: 0 successful, 1 failed
	 */
	int (*tx_isp_riscv_hf_resize)(void);

	/**
	 * Frame completion interrupt callback function
	 * count: Switch process
	 * Return value: 0 successful, 1 failed
	 * PS: Interrupt handling function, Do not perform complex processing operations
	 */
	int (*tx_isp_frame0_done_int_handler)(int count);
	int (*tx_isp_frame3_done_int_handler)(int count);
};

extern struct tx_isp_callback_ops g_tx_isp_callback_ops;

#endif
