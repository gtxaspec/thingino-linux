...
int zrt_isp_copyframe(int frame_channel, unsigned int frame_virAddr)
{
	...
	printk("%s start vinum:%d channel:%d\n",__func__,vinum,channel);
	list_for_each_safe(pos, n, &dma_alloc_frame_list[vinum * 3 + channel]) {
		frame = list_entry(pos, struct fast_frame_list, flist);
...
void vic_irq_snap_raw_work(struct work_struct *work)
{
	...
	lib_tisp_vic_save_info_t info = {
		.filePath = NULL,
		.rawType = TISP_RAW_16,
		.rawSaveFileType = TISP_SFILE_CUSTOM_ADDR,
		.rawSaveType = TISP_SRAW_CUSTOM_INDEX,
		.wdrEn = 0,
		.mallocMode = 1,
		.frameNum = 1,
		.wdrRawMode = 0,
	...
	lib_tisp_vic_save_frame_index_raw(data->vinum, &info);
}
#endif
...
static int tx_isp_riscv_hf_resize(void)
{
	...
	int vi_num = 0, i = 0,j = 0,default_boot = 0, i2c_adaptor_id;
...
//			  tx_isp_unlocked_ioctl_first(TISP_VIDIOC_SET_ISP_WDR_OPEN,(unsigned long)&wdr_attr);
    ...
    /* enc video  */
    if(kernel_encode)
        channel_buf_addr = ALIGN_ADDR(enc_video(channel_buf_addr));
