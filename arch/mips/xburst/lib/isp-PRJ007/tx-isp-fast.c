...
int enc_video(unsigned int addr)
{
...
static int frame_channel_fast_start(int chn_num, unsigned int addr)
{
	...
	format.pix.width = frame_channel_width[chn_num];
	format.pix.height = frame_channel_height[chn_num];
	format.crop_width = frame_channel_width[chn_num];
	format.crop_height = frame_channel_height[chn_num];
	format.scaler_out_width = frame_channel_width[chn_num];
	format.scaler_out_height = frame_channel_height[chn_num];
	...
	for(buf_i = 0; buf_i < count; buf_i++) {
		...
		buf.index = buf_i;
        buf.length = (frame_channel_width[chn_num] * ((frame_channel_height[chn_num] + 0xF) & ~0xF) * 3 / 2);
...
//            tx_isp_unlocked_ioctl_first(TISP_VIDIOC_SET_ISP_WDR_OPEN,(unsigned long)&wdr_attr);
	...
	default_boot = 0;
	i2c_adaptor_id = 0;
	ret = register_sensor(0, fast_sensors[0].name, fast_sensors[0].i2c_addr, i2c_adaptor_id, GPIO_PA(20), TISP_SENSOR_VI_MIPI_CSI0, TISP_SENSOR_MCLK1, default_boot);
	if(ret < 0){
		return ret;
	}
	//set isp buf
	...
	if (sensor_number == 2) {
		...
		default_boot = 0;
		i2c_adaptor_id = 1;
		ret = register_sensor(vi_num, fast_sensors[1].name, fast_sensors[1].i2c_addr, 1, GPIO_PA(21), TISP_SENSOR_VI_MIPI_CSI1, TISP_SENSOR_MCLK2, default_boot);
		if(ret < 0){
			return ret;
		}
	...
	if (sensor_number >1) {
//        channel_buf_addr = set_multicamera_buf(channel_buf_addr);
	}
