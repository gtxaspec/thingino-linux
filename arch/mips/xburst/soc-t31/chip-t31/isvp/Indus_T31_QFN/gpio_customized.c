...
int gpio_drive_strength_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(soc_gpio_drive_strength_table); i++) {
		pr_debug("set gpio strength: %d-%d",
			 soc_gpio_drive_strength_table[i].gpio_num, soc_gpio_drive_strength_table[i].drv_level); jz_gpio_set_drive_strength(soc_gpio_drive_strength_table[i].gpio_num, soc_gpio_drive_strength_table[i].drv_level); } return 0;
}
