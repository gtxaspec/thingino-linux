...
/* mtd crypto_signal */
bool is_mtd_crypt_mode = false;
EXPORT_SYMBOL(is_mtd_crypt_mode);
...
void dump_aes_reg(struct ingenic_aes_dev *aes){
	...
	dev_info(aes->dev,"AES_ASINTM: 0x%08lx\n",aes_read(aes,AES_ASINTM));
	dev_info(aes->dev,"AES_ASSA: 0x%08lx\n",aes_read(aes,AES_ASSA));
	dev_info(aes->dev,"AES_ASSTC: 0x%08lx\n",aes_read(aes,AES_ASSTC));
	dev_info(aes->dev,"AES_ASDA: 0x%08lx\n",aes_read(aes,AES_ASDA));
	dev_info(aes->dev,"AES_ASDTC: 0x%08lx\n",aes_read(aes,AES_ASDTC));
#endif
...
static inline int ingenic_aes_clear_data(struct ingenic_aes_dev *aes)
{
	unsigned long val,init_data;
	int err;
...
#ifdef CONFIG_SOC_PRJ007
	aes_write(aes,AES_ASSR, aes_read(aes,AES_ASSR)&(ASSR_KEYD | ASSR_AESD | ASSR_DMAD));
#elif defined (CONFIG_SOC_PRJ008)
	aes_write(aes,AES_ASSR, aes_read(aes,AES_ASSR)&(ASSR_AESD | ASSR_DMAD));
#endif
...
static inline int ingenic_aes_write_ctrl(struct ingenic_aes_dev *aes)
{
	unsigned int key32;
	int i;
	unsigned long val,init_data,mode,alg,key_mode,user_key;
	int err;
	err = ingenic_aes_hw_init(aes);
	if (err){
		dev_err(aes->dev,"aes hw init failed.\n");
    ...
    // 2. AES enable and clear data for new process.
	aes_write(aes,AES_ASCR, ASCR_CLR | ASCR_EN);
	aes_write(aes,AES_ASCR, 0);
	//init_data |= ASCR_PS_DEF | ASCR_CLR | ASCR_EN;
	//aes_write(aes,AES_ASCR,init_data);

	// 3. configure AES
	if (is_mtd_crypt_mode) {
#ifdef CONFIG_INGENIC_MTD_CRYPTO_ROOTFS_KEY256
		val = 2;  /* 256-bit */
		user_key = 1; /* use efuse mode */
		key_mode = 3; /* use efuse 256bit */
#else
		val = 0;  /* 128-bit */
		user_key = 1; /* use efuse mode */
		key_mode = 1; /*use efuse 128bit */
#endif
	} else {
	...
	if (is_mtd_crypt_mode) {
		init_data = ASCR_PS_DEF | ASCR_KEYL(val) |ASCR_USER_KEY(user_key) | ASCR_KEY_MODE(key_mode)| ASCR_ALGS(alg) | ASCR_DECE(mode) | ASCR_EN;
	} else {
		init_data = ASCR_PS_DEF | ASCR_KEYL(val) | ASCR_ALGS(alg) | ASCR_DECE(mode) | ASCR_EN;
	}

	if (is_mtd_crypt_mode) {
		aes_write(aes,AES_ASCR,init_data | ASCR_EDOUT |ASCR_EDIN);
	} else {
#ifdef CONFIG_SOC_PRJ008
		aes_write(aes, AES_ASCR, (init_data | ASCR_EDOUT) & (~ASCR_EDIN));
#elif defined (CONFIG_SOC_PRJ007)
		aes_write(aes,AES_ASCR,init_data | ASCR_EDIN);
#endif
	}
	...
	if (!is_mtd_crypt_mode) {
		// Initialize Key.
		for (i = 0; i < key32; i++) {
			aes_write(aes, AES_ASKY, __be32_to_cpu(aes->ctx->key[i]));
			//printk("%d,%s, key======== :0x%08lx 0x%08x\n",__LINE__, __func__, aes->ctx->key[i],__be32_to_cpu(aes->ctx->key[i]));
		}
	}
	...
	if (alg == CBC_ALG){
		...
		if(1){
			for (i = 0; i < 4; i++) {
				aes_write(aes, AES_ASIV, __be32_to_cpu(aes->ctx->updateiv[i]));
				//printk("%d,%s, iv======== iv:0x%08lx 0x%08x\n",__LINE__, __func__, aes->ctx->updateiv[i],__be32_to_cpu(aes->ctx->updateiv[i]));
			}
			init_data = aes_read(aes,AES_ASCR);
...
#elif defined (CONFIG_SOC_PRJ008)
	aes_write(aes,AES_ASSTC,length / 4);
...
static int ingenic_aes_crypt_dma_start(struct ingenic_aes_dev *aes)
{
	...
	//} else {
		...
		unsigned int mode = (aes->flags & FLAGS_DECRYPT) == FLAGS_DECRYPT ? DECRYPTO_MODE : ENCRYPTO_MODE;
		if(mode == DECRYPTO_MODE){
			memcpy(aes->ctx->updateiv, sg_virt(&aes->in_sgl)+ aes->in_sgl.length - 16, 16);
		}
		if (!is_mtd_crypt_mode) {
#ifdef CONFIG_SOC_PRJ008
			prep_sgdata(in_sg);
		...
		aes->flags &= ~FLAGS_FAST;
		dma_cache_sync(aes->dev, aes->buf_in, len32,DMA_TO_DEVICE);
		memcpy(aes->buf_out, aes->buf_in, 16);
		dma_cache_sync(aes->dev, aes->buf_out,len32,DMA_FROM_DEVICE);
		//}
		aes->total -= count;
		unsigned int *data;
		data = sg_virt(in_sg);
...
static void ingenic_aes_finish_req(struct ingenic_aes_dev *aes, int err)
{
	...
	unsigned int mode = (aes->flags & FLAGS_DECRYPT) == FLAGS_DECRYPT ? DECRYPTO_MODE : ENCRYPTO_MODE;
	if(mode == ENCRYPTO_MODE){
		memcpy(aes->ctx->updateiv, sg_virt(&aes->out_sgl)+ aes->out_sgl.length - 16, 16);
	}
	mutex_unlock(&aes->mlock);
...
static int ingenic_aes_crypt_dma_stop(struct ingenic_aes_dev *aes)
{
	...
	//	dma_unmap_sg(aes->dev, aes->in_sg, 1, DMA_TO_DEVICE);
	//} else {
	//dma_sync_single_for_device(aes->dev, aes->out_sg->dma_address,
	//		aes->out_sg->length, DMA_FROM_DEVICE);
	...
	/* copy data */
	unsigned int *data;
	data = sg_virt(&aes->in_sgl);

	data = sg_virt(&aes->out_sgl);

	while(((*(volatile unsigned int *)(aes->dma_addr_out | 0xa0000000)) == *(unsigned int *)aes->buf_in) && dma_wait_count){
		dma_wait_count--;
	}
...
static int ingenic_aes_crypt(struct ablkcipher_request *req, unsigned long mode)
{
	...
	if (mode & FLAGS_CBC) {
		if (req->info) {
			if (ctx->org_iv) {
				if (memcmp(req->info, ctx->org_iv, 16)) {
					memcpy(ctx->updateiv, req->info, sizeof(ctx->updateiv));
					memcpy(ctx->org_iv, req->info, sizeof(ctx->updateiv));
					printk("IV updated for CBC mode\n");
				}
			} else {
				memcpy(ctx->updateiv, req->info, sizeof(ctx->updateiv));
				memcpy(ctx->org_iv, req->info, sizeof(ctx->updateiv));
				printk("Initial IV set for CBC mode\n");
			}
		} else {
			printk("Warning: CBC mode but no IV provided\n");
		}
	} else {
		if (req->info) {
			printk("Note: ECB mode ignoring IV pointer\n");
		}
	}
...
static irqreturn_t ingenic_aec_irqthread(int irq, void *data)
{
	...
	if(val & ASSR_DMAD){
		...
		aes_write(aes,AES_ASSR,ASSR_DMAD);
		err = aes->err ? : err;
		if (aes->total && !err) {
			err = ingenic_aes_crypt_dma_start(aes);
			if (!err)
	...
	}else if(val & ASSR_AESD){
		aes_write(aes,AES_ASSR,ASSR_AESD);
#ifdef CONFIG_SOC_PRJ007
	}else if(val & ASSR_KEYD){
		aes_write(aes,AES_ASSR,ASSR_KEYD);
#endif
	}else{
