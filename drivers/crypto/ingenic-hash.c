...
#define MAX_PAGE_SIZE	(16*4096)

static int ingenic_hash_xmit_start(struct ingenic_hash_dev *hash, dma_addr_t dma_addr, size_t length, int final);
static int ingenic_hash_handle_queue(struct ingenic_hash_dev *hash,struct ahash_request *req);
 ...
 */
static void ingenic_hash_fill_padding_unaligned(struct ingenic_hash_reqctx *ctx, char *buffer, int length)
{
	...
	if (ctx->flags & (HASH_FLAGS_SHA384 | HASH_FLAGS_SHA512)) {
		...
		*(buffer + length) = 0x80;
		memset(buffer + length + 1, 0, padlen-1);
		memcpy(buffer + length + padlen, bits, 16);
		ctx->bufcnt += padlen + 16 + length;
		...
		*(buffer + length) = 0x80;
		memset(buffer + length + 1, 0, padlen-1);
		memcpy(buffer + length + padlen, &bits[1], 8);
		ctx->bufcnt += padlen + 8 + length;
		ctx->flags |= HASH_FLAGS_PAD;
	}
	//printk("%d,%s, filling finish\n", __LINE__, __func__);
		...
		size_t length, int final)
{
	...
	hash_write(hash, HASH_HSBNUM, length / 64);
	hash_write(hash, HASH_HSTC, length / 4);
#elif defined(CONFIG_SOC_PRJ007)
	hash_write(hash, HASH_HSTC, length / 64);
#endif
	...
	//dump_reg(hash);
	{
		...
		hash_write(hash, HASH_HSCR, val | (1 << 5));
		val = hash_read(hash, HASH_HSCR);
		hash_write(hash, HASH_HSCR, val | (1 << 6));
	}
	return -EINPROGRESS;
}

static int ingenic_hash_xmit_start_unaligned(struct ingenic_hash_dev *hash, dma_addr_t dma_addr,
		size_t length, int final)
{
	struct ingenic_hash_reqctx *ctx = ahash_request_ctx(hash->req);
	int ret = 0;
	...
	if (ctx->digcnt[0] < length)
		ctx->digcnt[1]++;
	if(!(hash->flags & HASH_FLAGS_NO_PAD)){
		ingenic_hash_fill_padding_unaligned(ctx, hash->vaddr, length);
	}
	...
	hash_write(hash,HASH_HSSA,dma_addr);
#ifdef CONFIG_SOC_PRJ008
	hash_write(hash, HASH_HSBNUM, ctx->bufcnt / 64 );
	hash_write(hash, HASH_HSTC, ctx->bufcnt / 4 );
#elif defined(CONFIG_SOC_PRJ007)
	hash_write(hash, HASH_HSTC, ctx->bufcnt / 64 );
#endif
	if(hash->flags & HASH_FLAGS_NO_PAD){
		dma_sync_single_for_device(hash->dev, dma_addr, MAX_PAGE_SIZE, DMA_TO_DEVICE);
	}else{
		dma_sync_single_for_device(hash->dev, dma_addr, ctx->bufcnt, DMA_TO_DEVICE);
	}
	...
	wait_for_completion(&hash->hash_completion);
...
static int ingenic_hash_update_dma_start(struct ingenic_hash_dev *hash)
{
	...
	{
		void *virt_addr;
		dma_addr_t addr = ctx->sg->dma_address;
		printk("%d %s, sg dma addr = %08x\n", __LINE__, __func__, ctx->sg->dma_address);
		printk("SG entry %d address: 0x%08x, data: 0x%08x\n", __LINE__, addr, *(volatile unsigned int *)(ctx->sg->dma_address + 0xa0000000));
		printk("SG entry %d address: 0x%08x, data: 0x%08x\n", __LINE__, addr, *(volatile unsigned int *)(ctx->sg->dma_address + 0xa0000004));
		printk("SG entry %d address: 0x%08x, data: 0x%08x\n", __LINE__, addr, *(volatile unsigned int *)(ctx->sg->dma_address + 0xa0000000 + 0xf3c));
	}
#endif
	/* next call does not fail... so no unmap in the case of error */
...
static void hash_dma_done(struct ingenic_hash_dev *hash)
{
	...
	if(hash->unaligned_flag){
		ctx->bufcnt = 0;
		complete(&hash->hash_completion);
		if(hash->flags & HASH_FLAGS_NO_PAD){
			return;
		}
...
static irqreturn_t ingenic_hash_irqthread(int irq, void *data)
{
	...
	if(val & HASH_HSSR_MRD){
		hash_dma_done(hash);
	}else{
		dev_warn(hash->dev, "Don't support the irq\n");
	}
	return IRQ_HANDLED;
}
...
static int ingenic_hash_init(struct ahash_request *req)
{
	...
	hash->unaligned_flag = 0;
	hash->data_length = 0;
	ingenic_hash_hw_init(hash,rctx);
	init_completion(&hash->hash_completion);
	EXIT();
...
static int ingenic_hash_final(struct ahash_request *req)
{
	struct ingenic_hash_reqctx *ctx = ahash_request_ctx(req);
	struct ingenic_hash_ctx *tctx = crypto_tfm_ctx(req->base.tfm);
	struct ingenic_hash_dev *hash = ingenic_hash_find_dev(tctx);
	int err = 0;
	...
	if(hash->unaligned_flag){
		hash->req = req;
		ingenic_hash_xmit_start_unaligned(hash, hash->dma_addr, hash->data_length, 1);
		ctx->bufcnt = 0;
		return -EINPROGRESS;
	}
...
static int ingenic_hash_update(struct ahash_request *req)
{
	...
	if(hash->unaligned_flag){
		...
		while(!sg_is_last(sg)){
			...
			if (!dma_map_sg(hash->dev, sg, 1, DMA_TO_DEVICE)) {
				dev_err(hash->dev, "dma_map_sg  error\n");
				return -EINVAL;
			}
			sg_vaddr = phys_to_virt(sg_dma_address(sg));
			if((hash->data_length + sg->length) > MAX_PAGE_SIZE){
				hash->req = req;
				hash->flags |= HASH_FLAGS_NO_PAD;
				ctx->bufcnt = MAX_PAGE_SIZE;
				ingenic_hash_xmit_start_unaligned(hash, hash->dma_addr, MAX_PAGE_SIZE, 1);
				ctx->bufcnt = 0;
				tmp_length = (hash->data_length + sg->length) - MAX_PAGE_SIZE;
				hash->data_length = 0;
				memcpy(hash->vaddr + hash->data_length, sg_vaddr, sg->length);
				hash->data_length += sg->length;
				hash->flags &= ~HASH_FLAGS_NO_PAD;
			}else{
				if(!sg_vaddr){
					dev_err(hash->dev, "%d %s, sg vaddr error\n", __LINE__, __func__);
				}
		...
		struct platform_device *pdev)
{
	...
	hash->io_base = devm_ioremap_resource(dev, r);
	if(IS_ERR(hash->io_base)) {
		ret = PTR_ERR(hash->io_base);
		goto err;
	}
	...
	err = devm_request_threaded_irq(dev, hash->irq, NULL, ingenic_hash_irqthread,
			IRQF_ONESHOT, dev_name(dev), hash);
	if (err) {
		dev_err(dev, "unable to request hash irq.\n");
		ret = err;
		goto err;
	}
...
static int ingenic_hash_probe(struct platform_device *pdev)
{
	...
	hash = devm_kzalloc(dev, sizeof(*hash), GFP_KERNEL);
	if (hash == NULL) {
	...
	hash->clk_gate = devm_clk_get(dev, "gate_hash");
	if (IS_ERR(hash->clk_gate)) {
		dev_err(dev, "cannot find hash clock\n");
		return PTR_ERR(hash->clk_gate);
	}

	err = clk_prepare_enable(hash->clk_gate);
	if (err)
		return err;
	...
	hash->vaddr = dma_alloc_coherent(hash->dev, 68*1024, &hash->dma_addr, GFP_KERNEL);
	if (!hash->vaddr) {
	...
	for (i = 0; i < hash->pdata->size; i++) {
		...
		if (err) {
			dev_err(dev, "failed to register %s algorithm\n",
					algp->halg.base.cra_name);
			goto err_algs;
		}
	...
	dev_info(dev, "ingenic hash[%d] register ok.\n", hash->pdata->registered);
	hash_write(hash,HASH_HSSR,HASH_HSSR_ORD | HASH_HSSR_MRD);

	return 0;

err_algs:
	for (i = hash->pdata->registered - 1; i >= 0; i--)
		crypto_unregister_ahash(&hash->pdata->algs_list[i]);
	dma_free_coherent(hash->dev, 68*1024, hash->vaddr, hash->dma_addr);
err_pm:
	pm_runtime_disable(dev);
err_clk:
	clk_disable_unprepare(hash->clk_gate);
	return err;
}
...
static SIMPLE_DEV_PM_OPS(ingenic_hash_pm_ops, ingenic_hash_suspend, ingenic_hash_resume);
