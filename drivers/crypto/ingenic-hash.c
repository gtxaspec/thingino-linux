...
static int ingenic_hash_probe(struct platform_device *pdev)
{
	...
	hash->vaddr = dma_alloc_coherent(hash->dev, HASH_DATA_BUFFER, &hash->dma_addr, GFP_KERNEL);
...
static int ingenic_hash_remove(struct platform_device *pdev)
{
	...
	dma_free_coherent(hash->dev, HASH_DATA_BUFFER, hash->vaddr, hash->dma_addr);
	kfree(hash);
	hash = NULL;
