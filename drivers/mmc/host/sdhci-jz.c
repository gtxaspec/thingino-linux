...
static void sdhci_jz_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	struct sdhci_jz *jz = sdhci_priv(host);
#if 1
	/* The 400K clock cannot be divided, so you need to switch the clock source to EXT. The 25M clock uses MPLL*/
	if(clock > 4000000)
	{
		value = clk_set_parent(jz->clk, jz->pll);
		if(value)
			printk("clk_set_parent err, value is %d\n", value);
	}else{
		/* If clk_set_rate is configured in advance, the parent clock can be successfully switchover.
		 * Otherwise, the first switchover fails */
		clk_set_rate(jz->clk, clock);
		*(volatile unsigned int *)0xB0000068 |= 1 << 21;
		ext = clk_get(NULL,"ext1");
		value = clk_set_parent(jz->clk, ext);
		if(value)
			printk("clk_set_parent err, value is %d\n", value);
	}
#endif
