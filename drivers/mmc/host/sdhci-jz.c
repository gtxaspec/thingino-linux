...
static void sdhci_jz_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	char *pll_name[4] = {"apll", "mpll", "vpll", "ext1"};
	...
	if (clock > 400000)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			{
				cdr = (clk_get_rate(jz->pll) / clock / 4 - 1) & 0xff;
...
static int sdhci_jz_probe(struct platform_device *pdev)
{
	...
	{
		dev_info(&pdev->dev, "The MSC_CLK parent is not obtained, and APLL is used.\n");
		jz->pll = clk_set_parent(jz->clk, clk_get(NULL, "apll"));
		if (IS_ERR(jz->pll))
			return PTR_ERR(jz->pll);
	}
