...
static inline void ingenic_mmc_clk_onoff(struct sdhci_ingenic *ingenic_ing, unsigned int on)
{
	if(on) {
		...
		clk_prepare_enable(ingenic_ing->clk_mux);
		clk_prepare_enable(ingenic_ing->clk_pll);
	} else {
		clk_disable_unprepare(ingenic_ing->clk_cgu);
		clk_disable_unprepare(ingenic_ing->clk_gate);
		clk_disable_unprepare(ingenic_ing->clk_mux);
		clk_disable_unprepare(ingenic_ing->clk_pll);
	}
}

#define MSC_WAIT_BUSY_TIME 300
static inline void ingenic_mmc_clk_switch_onoff(struct sdhci_host *host, struct sdhci_ingenic *ingenic_ing, unsigned int on)
{
	volatile void *msc_base_addr;
	unsigned int val;
	unsigned int time = MSC_WAIT_BUSY_TIME;

	if (0 == ingenic_ing->pdev->id)
		msc_base_addr = (volatile void *)CPM_MSC0_CLK_R;
	else if (1 == ingenic_ing->pdev->id)
	...
	if (on)
	{
		...
		val &= ~(MSC_CLK_CE);
		writel(val, msc_base_addr);
		sdhci_set_sd_clock(host, 1);
	}
	else
	{
		sdhci_set_sd_clock(host, 0);
		while(time--)
		{
			if(!(readl(msc_base_addr) & MSC_CLK_BUSY))
				break;
...
static void sdhci_ingenic_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	char *pll_name[4] = {"sclka", "mpll", "ext", "ext"};
#else
	char *pll_name[4] = {"sclka", "mpll", "vpll", "ext"};
#endif
	...
	sdhci_ing->clk_mux = devm_clk_get(sdhci_ing->dev, clkname);

	ingenic_mmc_clk_switch_onoff(host, sdhci_ing, 0);

	if (0 == sdhci_ing->pdev->id)
		msc_base_addr = (volatile void *)CPM_MSC0_CLK_R;
	else if (1 == sdhci_ing->pdev->id)
		msc_base_addr = (volatile void *)CPM_MSC1_CLK_R;
	...
	if (clock > 400000)
	{
		parent_name = clk_get_name(clk_get_parent(sdhci_ing->clk_mux));
		for (i = 0; i < 4; i++)
		{
			if (!strcmp(pll_name[i], parent_name))
			{
				pll_rate = clk_get_rate(sdhci_ing->clk_pll);
	...
	ingenic_mmc_clk_switch_onoff(host, sdhci_ing, 1);
...
static void sdhci_ingenic_set_driver_strength(struct sdhci_ingenic *sdhci_ing)
{
	struct sdhci_ingenic_pdata *pdata = sdhci_ing->pdata;
	int strength,i,pull_flag;

	if(pdata->driver_strength == 0 && pdata->gpio_pull_flag == 0)
		return;

	switch(pdata->driver_strength){
#ifdef CONFIG_SOC_PRJ008
		case 4:
			strength = GPIO_DRIVE_STRENGTH_MODE_2;
			break;
		case 8:
			strength = GPIO_DRIVE_STRENGTH_MODE_3;
			break;
		case 12:
			strength = GPIO_DRIVE_STRENGTH_MODE_4;
			break;
		default:
			strength = GPIO_DRIVE_STRENGTH_MODE_2;
			break;
#else
		case 4:
			strength = GPIO_DRIVE_STRENGTH_4;
			break;
		case 8:
			strength = GPIO_DRIVE_STRENGTH_8;
			break;
		case 12:
			strength = GPIO_DRIVE_STRENGTH_12;
			break;
		default:
			strength = GPIO_DRIVE_STRENGTH_4;
			break;
	...
	switch(pdata->gpio_pull_flag){
		case 0:
			pull_flag = GPIO_PULL_HIZ;
			break;
		case 1:
			pull_flag = GPIO_PULL_UP;
			break;
		case 2:
			pull_flag = GPIO_PULL_DOWN;
			break;
		default:
			pull_flag = GPIO_PULL_HIZ;
			break;
	...
	if(sdhci_ing->pdev->id == 0 && !strcmp(pdata->gpio_group_name, "gpb")){
		for(i = 0; i < 6; i++){
			jzgpio_set_func(GPIO_PORT_B, strength, i);
			if (i != 4)
				jzgpio_set_func(GPIO_PORT_B, pull_flag, 1 << i);
		}
	}else if(sdhci_ing->pdev->id == 1 && !strcmp(pdata->gpio_group_name, "gpc")){
		for(i = 2; i < 8; i++){
			jzgpio_set_func(GPIO_PORT_C, strength, i);
			if (i != 2)
				jzgpio_set_func(GPIO_PORT_C, pull_flag, 1 << i);
		}
	}else if(sdhci_ing->pdev->id == 1 && !strcmp(pdata->gpio_group_name, "gpa")){
		for(i = 14; i < 23; i++) {
			if ((i > 13 && i < 16) || (i == 17) || (i > 19 && i < 23)){
				jzgpio_set_func(GPIO_PORT_A, strength, i);
				if (i != 17)
					jzgpio_set_func(GPIO_PORT_A, pull_flag, 1 << i);
			}
		}
	}else if(sdhci_ing->pdev->id == 1 && !strcmp(pdata->gpio_group_name, "gpb")){
		for(i = 8; i < 15; i++) {
			if ((i > 7 && i < 12) || (i > 12 && i < 15)){
				jzgpio_set_func(GPIO_PORT_B, strength, i);
				if (i != 8)
					jzgpio_set_func(GPIO_PORT_B, pull_flag, 1 <<  i);
			}
		...
		struct sdhci_ingenic_pdata *pdata)
{
	...
	unsigned int val;
	struct of_phandle_args pinmux_spec;
	pdata->gpio_group_name = "gpb";
	...
	if (of_get_property(np, "non-initial", NULL)) {
		pdata->cd_type = SDHCI_INGENIC_CD_NON_INITIAL;
	}
	...
	if(!(of_property_read_u32(np, "driver-strength", &val)))
		pdata->driver_strength = val;

	if(!(of_property_read_u32(np, "gpio-pull-flag", &val)))
		pdata->gpio_pull_flag = val;
	...
	pdata->gpio_group_name = "gpb";
	if(!(pinctrl_node = of_parse_phandle(np, "pinctrl-0", 0))){
		dev_err(dev, "Failed to parse pinctrl-0\n");
	}
	if(of_parse_phandle_with_fixed_args(pinctrl_node, "ingenic,pinmux", 2, 0, &pinmux_spec)){
		dev_err(dev, "Failed to parse ingenic,pinmux\n");
		of_node_put(pinctrl_node);
		return 0;
	}

	pdata->gpio_group_name = pinmux_spec.np->name;
	host->mmc->mmc_gpio_group_name = pinmux_spec.np->name;
...
/****************Position: /sys/devices/platform/ahb0/13060000.msc ***********/
...
static DEVICE_ATTR(sdhci_rx_sel, S_IWUSR | S_IRUSR, jz_sdhci_rx_sel_show, jz_sdhci_rx_sel);
static DEVICE_ATTR(sdhci_tx_sel, S_IWUSR | S_IRUSR, jz_sdhci_tx_sel_show, jz_sdhci_tx_sel);

static struct attribute *jzmmc_attributes[] = {
	&dev_attr_present.attr,
	&dev_attr_sdhci_rx_sel.attr,
	&dev_attr_sdhci_tx_sel.attr,
	NULL
};
...
static int sdhci_ingenic_probe(struct platform_device *pdev)
{
	...
	sdhci_ing->clk_mux = devm_clk_get(&pdev->dev, clkname);
	sdhci_ing->clk_pll = clk_get_parent(sdhci_ing->clk_mux);

	if ((!sdhci_ing->clk_pll) ||
			(CONFIG_EXTAL_CLOCK * 1000000 == clk_get_rate(sdhci_ing->clk_pll)))
	{
		dev_info(&pdev->dev, "The MSC_CLK parent is not obtained, and MPLL is used.\n");
		sdhci_ing->clk_pll = clk_get(NULL, "mpll");
	}
	...
	if (!(ret & MSC_CLK_EXCLK_EN))
		writel(ret | MSC_CLK_EXCLK_EN, (volatile void *)CPM_MSC0_CLK_R);
	...
	sdhci_ingenic_set_driver_strength(sdhci_ing);
	/* sdio for WIFI init*/
	if (pdata->sdio_clk) {
	...
	if (pdata->cd_type == SDHCI_INGENIC_CD_NON_INITIAL) {
		host->flags |= SDHCI_DEVICE_DEAD;
		host->quirks &= ~SDHCI_QUIRK_BROKEN_CARD_DETECTION;
	}
...
static int sdhci_ingenic_remove(struct platform_device *pdev)
{
	...
	devm_clk_put(&pdev->dev, sdhci_ing->clk_gate);
	devm_clk_put(&pdev->dev, sdhci_ing->clk_cgu);

	devm_kfree(&pdev->dev, sdhci_ing->pdata);
	iounmap(host->ioaddr);
...
static const struct dev_pm_ops sdhci_ingenic_pmops = {
	...
	SET_RUNTIME_PM_OPS(sdhci_ingenic_runtime_suspend, sdhci_ingenic_runtime_resume,NULL)
};
