...
static void sdhci_ingenic_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (clock > 400000) {
		value = clk_set_parent(sdhci_ing->clk_mux, sdhci_ing->clk_pll);
		if(value)
			printk("clk_set_parent set err! value is %d\n", value);
#if 0
		sdhci_ing->clk_mux = devm_clk_get(sdhci_ing->dev, clkname);
		value = clk_get_rate(sdhci_ing->clk_mux);
		printk("clk_mux is %d\n", value);
#endif
		/* prepare后无法继续配置父时钟 */
	} else {
	...
	//value = *(volatile unsigned int *)0xB0000068;
	//value1 = *(volatile unsigned int *)0xB000006C;
	//printk("value is %x, value1 is %x, clk is %d\n", value, value1, clock);
