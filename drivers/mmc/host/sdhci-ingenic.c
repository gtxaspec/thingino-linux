...
static inline void ingenic_mmc_clk_onoff(struct sdhci_ingenic *ingenic_ing, unsigned int on)
{
	if(on) {
        ...
        clk_prepare_enable(ingenic_ing->clk_ext);
	} else {
        ...
        clk_disable_unprepare(ingenic_ing->clk_ext);
	}
}
...
static inline void ingenic_mmc_clk_switch_onoff(struct sdhci_ingenic *ingenic_ing, unsigned int on)
{
...
static void sdhci_ingenic_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	char clkname[16];
	char *pll_name[4] = {"sclka", "mpll", "epll", "ext"};
	...
	ingenic_mmc_clk_switch_onoff(sdhci_ing, 0);
	...
	if (clock > 400000)
	{
		...
		for (i = 0; i < 4; i++)
		{
			...
			{
				// printk(KERN_INFO "msc%d clk_get_parent %s\n", sdhci_ing->pdev->id, parent_name);
				cdr = (clk_get_rate(sdhci_ing->clk_pll) / clock / 4 - 1) & 0xff;
	...
	{
		val |= (MSC_CLK_CS(3) | MSC_CLK_EXCLK_EN);
		cdr = ((CONFIG_EXTAL_CLOCK * 1000000 / clock) / 4 - 1) & 0xff;
		val |= cdr;
		writel(val, msc_base_addr);
	}

	ingenic_mmc_clk_switch_onoff(sdhci_ing, 1);
...
static void sdhci_ingenic_notify_change(struct platform_device *dev, int state)
{
	...
	if (host) {
		...
		if (state) {
			old_status = 0;
			printk("%s: card inserted    %d\n", mmc_hostname(host->mmc),old_status);
...
static int sdhci_ingenic_probe(struct platform_device *pdev)
{
	...
	//Keep EXCLK_EN ON, MSC1 EXCLK ENABLE CONTROL is shared with MSC0
