...
#define CPM_MSC0_CLK_R		(0xB0000098)
#define CPM_MSC1_CLK_R		(0xB000009C)
...
static void sdhci_ingenic_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (clock > 400000) {
		value = clk_set_parent(sdhci_ing->clk_mux, sdhci_ing->clk_mpll);
		printk("clk_set_parent value is %d\n",value);
		/* prepare后无法继续配置父时钟 */
	//	clk_prepare_enable(sdhci_ing->clk_cgu);
	} else {
		value = clk_set_parent(sdhci_ing->clk_mux, sdhci_ing->clk_ext);
		*(volatile unsigned int *)0xB0000098 |= 1 << 21;
	}
		...
		int host_drv, int card_drv, int *drv_type)
{
	...
	switch (type) {
		case INGENIC_TYPE_0:
			/* Type 0: 50 */
			*(volatile unsigned int*)(0xb0011000 + 0x134) |= 0x3f;
			*(volatile unsigned int*)(0xb0011000 + 0x144) |= 0x3f;
			drive_strength = 0;
			break;
		case INGENIC_TYPE_1:
			/* Type 1: 33 */
			*(volatile unsigned int*)(0xb0011000 + 0x144) |= 0x3f;
			*(volatile unsigned int*)(0xb0011000 + 0x154) |= 0x3f;
...
static void sdhci_ingenic_notify_change(struct platform_device *dev, int state)
{
	...
	if (host) {
		...
		if (state) {
			...
			if(gpio_is_valid(sdhci_ing->pdata->gpio->pwr.num)) {
				gpio_direction_output(sdhci_ing->pdata->gpio->pwr.num,
						sdhci_ing->pdata->gpio->pwr.enable_level);
			}
