...
void mmc_pwrseq_post_power_on(struct mmc_host *host)
{
	...
	if (host->index == 0)
	{
		if (!strcmp(host->mmc_gpio_group_name, "gpb"))
			jzgpio_set_func(GPIO_PORT_B, GPIO_FUNC_0, 0x3f);
	}
	if (host->index == 1)
	{
		if (!strcmp(host->mmc_gpio_group_name, "gpa"))
		{
			jzgpio_set_func(GPIO_PORT_A, GPIO_FUNC_2, (0x7 << 20 | 0x1 << 17 | 0x3 << 14));
		}
		else if (!strcmp(host->mmc_gpio_group_name, "gpb"))
		{
			jzgpio_set_func(GPIO_PORT_B, GPIO_FUNC_1, (0x3f << 8));
		}
		else if (!strcmp(host->mmc_gpio_group_name, "gpc"))
		{
