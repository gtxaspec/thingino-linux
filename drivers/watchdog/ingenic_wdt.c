...
static __deprecated int reset_task(void *data)
{
	...
	if (test_and_set_bit(WDOG_NO_WAY_OUT, &wdt_utils->wdt->status)) {
		pr_warn("watchdog conflict\n");
		wdt_utils->stop = 1;
		return 0;
	}

	mutex_lock(&wdt_utils->lock);

	ingenic_wdt_set_timeout(wdt_utils->wdt, (wdt_utils->msecs + 1000 - 1)/1000);
	ingenic_wdt_start(wdt_utils->wdt);

	mutex_unlock(&wdt_utils->lock);

	while (1) {
		if(kthread_should_stop()) {
			clear_bit(WDOG_NO_WAY_OUT, &wdt_utils->wdt->status);
			mutex_lock(&wdt_utils->lock);
