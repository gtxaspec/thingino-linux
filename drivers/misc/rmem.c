...
static long rmem_cmd_flush_cache(long arg)
{
	...
	offset = info.addr - rmem_addr.paddr;
	dma_cache_sync(NULL, (void *)(rmem_addr.vaddr + offset), info.len, info.dir);
	/* dma_sync_single_for_device(NULL, (dma_addr_t)info.addr, info.len, info.dir); */
...
static int rmem_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	/* printk("vm_vaddr = 0x%08x, vm_paddr = 0x%08x\n",rmem_addr.vaddr,rmem_addr.paddr); */
	return 0;
}
