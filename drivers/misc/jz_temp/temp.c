...
static void temp_compensation(int *ptemperature)
{
	int temp;
	...
	temp_compensation = 1000*temp_compensation;//系数放大
	// printk("compensation:%d,temp:%d\n",temp_compensation,temp);
	*ptemperature = (temp + temp_compensation);
}
...
static int temp_open(struct inode *inode, struct file *file)
{
	...
	if(clk_get_rate(stemp_ope->clk) > 24000000){
		clk_set_rate(stemp_ope->clk, 24*1000*1000);
		// printk("1rate:%lu\n",clk_get_rate(stemp_ope->clk));
	}
...
static int temp_info_show(struct seq_file *m, void *v)
{
	...
	len += seq_printf(m ,"Temprature:%d.%d\n",temperature/1000,abs(temperature%1000));
...
{
	if (is_on && !tsPI.tsensor_timer.data)
	{
		printk("TSensor enable auto tsRefi_func\n");
		tsPI.tsensor_timer.data = 1;
		tsensor_timer_callback(1);
	}
	else if(!is_on && tsPI.tsensor_timer.data)
	{
		printk("Wait for TSensor auto tsRefi_func to shut down...\n");
		tsPI.tsensor_timer.data = 0;
	}
...
failed_iomap:
	// release_mem_region(stemp_ope->res->start, stemp_ope->res->end - stemp_ope->res->start + 1);
failed_req_region:
failed_get_mem:
	kfree(stemp_ope);
	return ret;
...
static int __init tsensor_timer_init(void)
{
	...
	setup_timer(&tsPI.tsensor_timer, tsensor_timer_callback, 1);
