...
#ifdef CONFIG_MFD_INGENIC_SADC_AUX
#define ADCUSEMFD (1)
#else
#define ADCUSEMFD (0)
#endif
...
DEFINE_MUTEX(tmutex);
struct mutex adcmutex;/*for ADC and Tsensor mutex*/
...
static void tsensor_clk_on(struct stemp_operation *stemp_ope)
{
	...
	if (!__clk_is_enabled(stemp_ope->clk)) {
		clk_prepare_enable(stemp_ope->clk);
	}
...
#if 0
	if (__clk_is_enabled(stemp_ope->clk)) {
		clk_disable_unprepare(stemp_ope->clk);
		clk_put(stemp_ope->clk);
		stemp_ope->clk = NULL;
	}
#endif
					...
					char *buf)
{
	...
	if (tsPI.b_AutoEnabled)
	{
		count = sprintf(buf, "enable refi(%d) reg:0x%08x[%x %x %x]\n", tsPI.trefi, tsreadl(RTR),
			tsPI.Trefi7800, tsPI.Trefi3900, tsPI.Trefi2920);
	}
	else
	{
...
static const struct proc_ops temp_info_fops ={
	.proc_read = seq_read,//fs/seq_file.c
	.proc_open = temp_info_open,
	.proc_lseek = seq_lseek,//fs/seq_file.c
	.proc_release = single_release,//fs/seq_file.c
};
...
static void ts_CalRefiInit(struct stemp_operation *stemp_ope)
{
	...
	tsPI.tsensor_stemp_ope = stemp_ope;

	rate = clk_get_rate(clk_get(NULL, "mpll")) / 1000000;
	rate = rate * 2 / ((*(volatile unsigned int*)(0xb000002c) & 0xff) + 1);
	p2t = 1000000 / rate;

	tsPI.trefi = (tsreadl(RTR) >> 16) & 0xff;

	ts_debug("rate:%dMHz;  tck_ps:%dps; tsPI.trefi:0x%08x\n", rate, p2t, tsPI.trefi);

	tsPI.Trefi7800 = 7800000 / p2t / 64;
	tsPI.Trefi3900 = 3900000 / p2t / 64;
	tsPI.Trefi2920 = 2920000 / p2t / 64;
}
...
static void work_handler(struct work_struct *work)
{
	int temperature = 0;
#if !ADCUSEMFD
	tsensor_clk_on(tsPI.tsensor_stemp_ope);
#endif
	mutex_lock(&tmutex);
	temperature = tsensor_get_temp(tsPI.tsensor_stemp_ope) / 1000;
	mutex_unlock(&tmutex);
#if !ADCUSEMFD
	tsensor_clk_off(tsPI.tsensor_stemp_ope);
#endif
	ts_SetRefi(temperature);
}

static void tsensor_timer_callback(struct timer_list *timer)
{
	struct ts_AutoRefiInfo  *tsPI = container_of(timer, struct ts_AutoRefiInfo, tsensor_timer);
	if (tsPI->b_AutoEnabled)
	{
		queue_work(tsPI->wq, &tsPI->wrk);
		mod_timer(&tsPI->tsensor_timer, jiffies + msecs_to_jiffies(CONFIG_AUTOREFI_DELAY));
	}
	else
	{
...
{
	if (is_on && !tsPI.b_AutoEnabled)
	{
		printk("TSensor enable auto tsRefi_func\n");
		tsPI.b_AutoEnabled = 1;
		tsensor_timer_callback(&tsPI.tsensor_timer);
	}
	else if(!is_on && tsPI.b_AutoEnabled)
	{
		printk("Wait for TSensor auto tsRefi_func to shut down...\n");
		tsPI.b_AutoEnabled = 0;
	}
...
static int __init tsensor_workenue_init(void)
{
	...
	tsPI.wq = create_singlethread_workqueue("tsensor_single_thread");

	INIT_WORK(&tsPI.wrk, work_handler);
...
{
	flush_workqueue(tsPI.wq);
	destroy_workqueue(tsPI.wq);
...
static int __init tsensor_timer_init(void)
{
	...
	timer_setup(&tsPI.tsensor_timer, tsensor_timer_callback, 0);
	ts_debug("TSensor Setup timer to fire in 30s (%ld)\n", jiffies);

	tsPI.b_AutoEnabled = false;
	add_timer(&tsPI.tsensor_timer);
...
{
	int retval;
	retval = del_timer(&tsPI.tsensor_timer);
	if (retval)
