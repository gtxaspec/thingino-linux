...
EXPORT_SYMBOL(tcu_chn_free);

void tcu_open_clock(unsigned char chn)
{
	if (chn < NR_TCU_CHNS || chn == 16 || chn == 15){
		writel(1 << chn, tcu->base + TCU_TSCR);
	}
	else{
		printk("TCU channel error:%d\n", chn);
	}
}
EXPORT_SYMBOL(tcu_open_clock);

void tcu_close_clock(unsigned char chn)
{
	if (chn < NR_TCU_CHNS || chn == 16 || chn == 15){
		writel(1 << chn, tcu->base + TCU_TSSR);
	}
	else{
		printk("TCU channel error:%d\n", chn);
	}
...
int tcu_set_config(struct tcu_config *config)
{
	...
	if (config->mode == TCU_BYPASS_MODE){
		reg |= 1 << 11; //BYPASS
		writel((1 << (config->chn+16)) | (1 << config->chn), tcu->base + TCU_TMSR); //mask irq
	}
		...
		writel(1 << (config->chn+16), tcu->base + TCU_TMSR); //mask half irq
		writel(1 << config->chn, tcu->base + TCU_TMCR);		 //unmask full irq
	}
	writel(reg, tcu->base + TCU_TCSR0 + 0x10 * config->chn);

	return 0;
}
EXPORT_SYMBOL(tcu_set_config);
...
void tcu_set_initl(unsigned char chn, unsigned char initl)
{
	...
	reg = readl(tcu->base + TCU_TCSR0 + 0x10 * chn);
	if (initl){
		reg |= 1 << 8; // INITL/polarity
	}
	else{
	...
	writel(reg, tcu->base + TCU_TCSR0 + 0x10 * chn);
}
EXPORT_SYMBOL(tcu_set_initl);

void tcu_start_count(unsigned char chn)
{
	if (chn < NR_TCU_CHNS || chn == 15){
		writel(1 << chn, tcu->base + TCU_TESR);
	}
	else{
		printk("TCU channel error:%d\n", chn);
	}
}
EXPORT_SYMBOL(tcu_start_count);

void tcu_stop_count(unsigned char chn)
{
	if (chn < NR_TCU_CHNS || chn == 15){
		writel(1 << chn, tcu->base + TCU_TECR);
	}
	else{
		printk("TCU channel error:%d\n", chn);
	}
}
EXPORT_SYMBOL(tcu_stop_count);
...
void tcu_set_count(unsigned char chn, unsigned int value)
{
	...
	writel(value, tcu->base + TCU_TCNT0 + chn * 0x10);
}
EXPORT_SYMBOL(tcu_set_count);
...
void tcu_set_tdfr(unsigned char chn, unsigned int value)
{
	...
	writel(value, tcu->base + TCU_TDFR0 + chn * 0x10);
}
EXPORT_SYMBOL(tcu_set_tdfr);
...
void tcu_set_tdhr(unsigned char chn, unsigned int value)
{
	...
	writel(value, tcu->base + TCU_TDHR0 + chn * 0x10);
}
EXPORT_SYMBOL(tcu_set_tdhr);

inline unsigned int tcu_get_count(unsigned char chn)
{
	return readl(tcu->base + TCU_TCNT0 + chn * 0x10);
}
EXPORT_SYMBOL(tcu_get_count);
