...
#define INGENIC_REG_ADC_STB    0x24
...
#define CLKDIV		120
#define CLKDIV_US		2
#define CLKDIV_TSEN		3
#define CLKDIV_MS		300
...
static void ingenic_adc_clk_div(struct ingenic_adc *adc, const unsigned char clkdiv,
		const unsigned char clkdiv_us, const unsigned char clkdiv_tsen,const unsigned short clkdiv_ms)
{
	unsigned int val;

	val = clkdiv | (clkdiv_us << 8) | (clkdiv_tsen << 16) | (clkdiv_ms << 24);
	writel(val, adc->base + INGENIC_REG_ADC_CLKDIV);
}
static int adc_init(struct device *dev)
{
    unsigned char clkdiv, clkdiv_us,clkdiv_tsen;
    ...
    msleep(5);
    writew(0x500, adc->base + INGENIC_REG_ADC_ENABLE);

    clkdiv = CLKDIV - 1;
    clkdiv_us = CLKDIV_US - 1;
    clkdiv_tsen = CLKDIV_TSEN - 1;
    clkdiv_ms = CLKDIV_MS - 1;

    ingenic_adc_clk_div(adc, clkdiv, clkdiv_us,clkdiv_tsen, clkdiv_ms);
    writel(5, adc->base + INGENIC_REG_ADC_STB);
    writel(100, adc->base + INGENIC_REG_ADC_RETM);
    return 0;
}
...
static struct resource jz_aux_resources_tsensor[] = {
	{
...
struct jz_adc_tsensor g_adc_platform_data = {0};
...
static struct mfd_cell ingenic_adc_cells[] = {
	...
	//T-sensor
	{
		.id = 2,
		.name   = "jz-temp",
		.num_resources  = ARRAY_SIZE(jz_aux_resources_tsensor),
		.resources   = jz_aux_resources_tsensor,
		.platform_data = &g_adc_platform_data,
...
static int ingenic_adc_probe(struct platform_device *pdev)
{
	...
	g_adc_platform_data.iomem = adc->base;
	// printk("adc->mem->start:end=(%#x,%#x),base:%p\n",adc->mem->start,adc->mem->end,adc->base);
	...
	if(!__clk_get_enable_count(adc->clk))
		clk_prepare_enable(adc->clk);

	devm_gpio_request(&pdev->dev, 10, "ADC_PIN");
	jzgpio_set_func(GPIO_PORT_A, GPIO_FUNC_0, 0x400);
	adc_init(&pdev->dev);
...
#ifdef CONFIG_PM
static int ingenic_adc_suspend(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct ingenic_adc *adc = platform_get_drvdata(pdev);
	int ret = 0;
...
static int ingenic_adc_resume(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct ingenic_adc *adc = platform_get_drvdata(pdev);
	int ret = 0;
...
static const struct dev_pm_ops ingenic_adc_pm_ops = {
    .suspend = ingenic_adc_suspend,
    .resume = ingenic_adc_resume,
};
#else
#define ingenic_adc_pm_ops NULL
#endif
...
MODULE_DESCRIPTION("Ingenic ADC driver");
MODULE_AUTHOR("Guo Xu<xu.guo@ingenic.com>");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:PRJ008-adc");
