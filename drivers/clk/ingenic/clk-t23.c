...
#include <soc/cpm.h>
#include <soc/base.h>
#include <dt-bindings/clock/ingenic-t23.h>
 ...
 *      FIXED CLK
 ********************************************************************************/
static struct ingenic_fixed_rate_clock t23_fixed_rate_ext_clks[] __initdata = {
...
static struct ingenic_pll_rate_table t23_pll_rate_table[] = {
...
static struct ingenic_pll_clock t23_pll_clks[] __initdata = {
	PLL(CLK_PLL_APLL, "apll", "ext", &apll_hwdesc, t23_pll_rate_table),
	PLL(CLK_PLL_MPLL, "mpll", "ext", &mpll_hwdesc, t23_pll_rate_table),
};
...
PNAME(mux_table_4)	= {"sclka",	"ext"};
PNAME(mux_table_5)	= {"mux_ahb2",	"ext"};

static unsigned int ingenic_mux_table[] = {0, 1, 2, 3};


static struct ingenic_mux_clock t23_mux_clks[] __initdata = {
...
static struct ingenic_bus_clock t23_bus_div_clks[] __initdata = {
...
static struct clk_div_table t23_clk_div_table[256] = {};

static struct ingenic_div_clock t23_div_clks[] __initdata = {
	DIV(CLK_DIV_DDR,		"div_ddr",		"mux_ddr",		CPM_DDRCDR,	    4,  0, NULL),
	DIV(CLK_DIV_MACPHY,		"div_macphy",	"mux_macphy",	CPM_MACCDR,	    8,  0, NULL),
	DIV_SPECIAL(CLK_DIV_LCD,"div_lcd",		"mux_lcd",		CPM_LPCDR,	    8,  0, NULL),
	DIV(CLK_DIV_MSC0,		"div_msc0",		"mux_msc0",		CPM_MSC0CDR,	8,  0, t23_clk_div_table),
	DIV(CLK_DIV_MSC1,		"div_msc1",		"mux_msc1",		CPM_MSC1CDR,	8,  0, t23_clk_div_table),
	DIV_SPECIAL(CLK_DIV_SFC,"div_sfc",		"mux_sfc",		CPM_SFCCDR,	    8,  0, NULL),
	DIV_SPECIAL(CLK_DIV_SSI,"div_ssi",		"mux_ssi",		CPM_SSICDR,	    8,  0, NULL),
...
static struct ingenic_fra_div_clock t23_fdiv_clks[] __initdata = {
 ...
 *      GATE
 ********************************************************************************/
static struct ingenic_gate_clock t23_gate_clks[] __initdata = {
	...
	GATE(CLK_GATE_IVDC,     "gate_ivdc",        "div_vpu",		CPM_CLKGR1, 3,  0,          CLK_GATE_SET_TO_DISABLE),
	...
	GATE(CLK_GATE_VPU,		"gate_vpu",			"div_vpu",		CPM_CLKGR1, 0,  0, 			CLK_GATE_SET_TO_DISABLE),
...
{
	int i;
	for (i = 0; i < ARRAY_SIZE(t23_clk_div_table); i++) {
		t23_clk_div_table[i].val = i;
		t23_clk_div_table[i].div = (i + 1) * 2;
	}
...
/* Register t23 clocks. */
static void __init t23_clk_init(struct device_node *np, void __iomem *base)
{
	void __iomem *reg_base;

	printk("t23 Clock Power Management Unit init!\n");
	...
	ingenic_clk_of_register_fixed_ext(ctx, t23_fixed_rate_ext_clks,
			                        ARRAY_SIZE(t23_fixed_rate_ext_clks), ext_clk_match);

	/* Register PLLs. */
	ingenic_clk_register_pll(ctx, t23_pll_clks,
				ARRAY_SIZE(t23_pll_clks), reg_base);


	/* Register Muxs */
	ingenic_clk_register_mux(ctx, t23_mux_clks, ARRAY_SIZE(t23_mux_clks));

	/* Register Bus Divs */
	ingenic_clk_register_bus_div(ctx, t23_bus_div_clks, ARRAY_SIZE(t23_bus_div_clks));

	/* Register Divs */
	clk_div_table_generate();
	ingenic_clk_register_cgu_div(ctx, t23_div_clks, ARRAY_SIZE(t23_div_clks));

	/* Register Fractional Divs */
	ingenic_clk_register_fra_div(ctx, t23_fdiv_clks, ARRAY_SIZE(t23_fdiv_clks));

	/* Register Gates */
	ingenic_clk_register_gate(ctx, t23_gate_clks, ARRAY_SIZE(t23_gate_clks));

	/* Register Powers */
//	ingenic_power_register_gate(ctx, t23_gate_power, ARRAY_SIZE(t23_gate_power));
...
CLK_OF_DECLARE(t23_clk, "ingenic,t23-clocks", t23_clk_init);
