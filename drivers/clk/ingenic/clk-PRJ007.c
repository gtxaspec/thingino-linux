 ...
 *      FIXED CLK
 ********************************************************************************/
static struct ingenic_fixed_rate_clock PRJ007_fixed_rate_ext_clks[] __initdata = {
...
static struct ingenic_pll_rate_table PRJ007_pll_rate_table[] = {
...
static struct ingenic_pll_clock PRJ007_pll_clks[] __initdata = {
	PLL(CLK_PLL_APLL, "apll", "ext", &apll_hwdesc, PRJ007_pll_rate_table),
	PLL(CLK_PLL_MPLL, "mpll", "ext", &mpll_hwdesc, PRJ007_pll_rate_table),
	PLL(CLK_PLL_VPLL, "vpll", "ext", &vpll_hwdesc, PRJ007_pll_rate_table),
};
...
static struct ingenic_mux_clock PRJ007_mux_clks[] __initdata = {
...
static struct ingenic_bus_clock PRJ007_bus_div_clks[] __initdata = {
...
static struct clk_div_table PRJ007_clk_div_table[256] = {};
static struct clk_div_table PRJ007_sfc_clk_div_table[256] = {};

static struct ingenic_div_clock PRJ007_div_clks[] __initdata = {
	...
	DIV(CLK_DIV_SFC,		"div_sfc0",		"mux_sfc0",		CPM_SFCCDR,	    8,  0, PRJ007_sfc_clk_div_table),
	DIV(CLK_DIV_SFC1,		"div_sfc1",		"mux_sfc1",		CPM_SFC1CDR,	8,  0, PRJ007_sfc_clk_div_table),
	...
	DIV(CLK_DIV_MSC0,		"div_msc0",		"mux_msc0",		CPM_MSC0CDR,	8,  0, PRJ007_clk_div_table),
	DIV(CLK_DIV_MSC1,		"div_msc1",		"mux_msc1",		CPM_MSC1CDR,	8,  0, PRJ007_clk_div_table),
...
static struct ingenic_fra_div_clock PRJ007_fdiv_clks[] __initdata = {
 ...
 *      GATE
 ********************************************************************************/
static struct ingenic_gate_clock PRJ007_gate_clks[] __initdata = {
	...
	GATE(CLK_GATE_RISCV,    "gate_riscv",		"div_riscv",	CPM_CLKGR,   22, 0, 			CLK_GATE_HIWORD_MASK),
#else
	...
	GATE(CLK_GATE_RTC,	    "gate_rtc",		    "div_apb",	    CPM_CLKGR,   25, 0, 			CLK_GATE_SET_TO_DISABLE),
	GATE(CLK_GATE_TCU,	    "gate_tcu",		    "div_apb",	    CPM_CLKGR,   26, 0, 			CLK_GATE_SET_TO_DISABLE),
...
{
	int i;
	for (i = 0; i < ARRAY_SIZE(PRJ007_clk_div_table); i++) {
		PRJ007_clk_div_table[i].val = i;
		PRJ007_clk_div_table[i].div = (i + 1) * 4;
	}

}

static void clk_div_table_sfc(void)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(PRJ007_sfc_clk_div_table); i++) {
		PRJ007_sfc_clk_div_table[i].val = i;
#ifdef CONFIG_ZERATUL
		PRJ007_sfc_clk_div_table[i].div = (i + 1) * 4;
#else
		PRJ007_sfc_clk_div_table[i].div = (i + 1) * 2;
#endif
	}
...
static void __init PRJ007_clk_init(struct device_node *np, void __iomem *base)
{
	...
	ingenic_clk_of_register_fixed_ext(ctx, PRJ007_fixed_rate_ext_clks,
			                        ARRAY_SIZE(PRJ007_fixed_rate_ext_clks), ext_clk_match);

	/* Register PLLs. */
	ingenic_clk_register_pll(ctx, PRJ007_pll_clks,
				ARRAY_SIZE(PRJ007_pll_clks), reg_base);


	/* Register Muxs */
	ingenic_clk_register_mux(ctx, PRJ007_mux_clks, ARRAY_SIZE(PRJ007_mux_clks));

	/* Register Bus Divs */
	ingenic_clk_register_bus_div(ctx, PRJ007_bus_div_clks, ARRAY_SIZE(PRJ007_bus_div_clks));
	ingenic_clk_sleep_init(reg_base, t41_clk_reg_sleep, t41_nr_clk_reg_sleep);
	/* Register Divs */
	clk_div_table_generate();
	clk_div_table_sfc();
	ingenic_clk_register_cgu_div(ctx, PRJ007_div_clks, ARRAY_SIZE(PRJ007_div_clks));

	/* Register Fractional Divs */
	ingenic_clk_register_fra_div(ctx, PRJ007_fdiv_clks, ARRAY_SIZE(PRJ007_fdiv_clks));

	/* Register Gates */
	ingenic_clk_register_gate(ctx, PRJ007_gate_clks, ARRAY_SIZE(PRJ007_gate_clks));

	/* Register Powers */
//	ingenic_power_register_gate(ctx, PRJ007_gate_power, ARRAY_SIZE(PRJ007_gate_power));
...
CLK_OF_DECLARE(PRJ007_clk, "ingenic,PRJ007-clocks", PRJ007_clk_init);
