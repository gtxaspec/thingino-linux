...
static unsigned long ingenic_t41_pll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	struct ingenic_clk_pll *pll = to_clk_pll(hw);
	...
	rate = ((parent_rate / 1000000 ) * (m + 1) * 2) / ( (n + 1) * ( 1 << od0) *  (od1 + 1));
...
static int ingenic_t41_pll_set_rate(struct clk_hw *hw, unsigned long drate, unsigned long prate)
{
	struct ingenic_clk_pll *pll = to_clk_pll(hw);

	unsigned int m, n, od1, od0, rg;
	unsigned int val, i, reg;
	...
	for (i = 0; i < pll->rate_count; i++) {
		if (drate == pll->rate_table[i].rate) {
			...
			rg = pll->rate_table[i].rg;
		}
	}
    reg = pll->con_reg;
    if(((reg & 0xff) == 0x10) || ((reg & 0xff) == 0x14)){
		printk("WARNING : apll and mpll can not support set rate!!!! \n");
        return 0;
    }

    if((reg & 0xff) == 0x10){
        cpccr = readl((CPM_IOBASE | 0xb0000000) + CPM_CPCCR);
        cpccr &=~(3<<30);
        cpccr |=1<<30;
        writel(cpccr, (CPM_IOBASE | 0xb0000000) + CPM_CPCCR);
    }

	writel(0, pll->con_reg);
	val = (m << pll->hwdesc->m_sft) | (n << pll->hwdesc->n_sft) | (od1 << pll->hwdesc->od1_sft) | (od0 << pll->hwdesc->od0_sft)| (rg << pll->hwdesc->rg_sft) | 1;
...
static const struct clk_ops ingenic_t41_pll_clk_ops = {
	.recalc_rate = ingenic_t41_pll_recalc_rate,
	.round_rate = ingenic_pll_round_rate,
	.set_rate = ingenic_t41_pll_set_rate,
};
...
//	init.flags = pll_clk->flags;
	...
	init.num_parents = 1;
	init.ops = &ingenic_t41_pll_clk_ops;
