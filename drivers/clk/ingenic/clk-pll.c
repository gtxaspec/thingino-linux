...
static int ingenic_tx_pll_set_rate(struct clk_hw *hw, unsigned long drate, unsigned long prate)
{
	...
	if (pll->con_reg == pll->lock_reg + CPM_CPMPCR) {
		printk("WARNING : MPLL not support set rate!!!!\n");
		return 0;
	}
	if (pll->con_reg == pll->lock_reg + CPM_CPAPCR) {
		cpccr = readl(pll->lock_reg);
		cpccr &= ~(3 << 30);
		cpccr |= 1 << 30;
		writel(cpccr, pll->lock_reg);

		/* Determine if the MACPHY clock source is APLL and if the APLL frequency is a multiple of 25MHz. */
		if((!(readl(pll->lock_reg + 0x54) >> 30)) && (pll->rate_count % 25000000))
			printk("WARNING : Please check the MACPHY clock!!!\n");
	}
	...
	if (i == pll->rate_count) {
		printk("WARNING : PLL rate %lu not found in rate_table!\n", drate);
		return 0;
	}
