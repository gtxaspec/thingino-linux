...
#include <soc/cpm.h>
#include <soc/base.h>
#include <dt-bindings/clock/ingenic-c100.h>
 ...
 *      FIXED CLK
 ********************************************************************************/
static struct ingenic_fixed_rate_clock c100_fixed_rate_ext_clks[] __initdata = {
...
static struct ingenic_pll_rate_table c100_pll_rate_table[] = {
...
static struct ingenic_pll_clock c100_pll_clks[] __initdata = {
	PLL(CLK_PLL_APLL, "apll", "ext", &apll_hwdesc, c100_pll_rate_table),
	PLL(CLK_PLL_MPLL, "mpll", "ext", &mpll_hwdesc, c100_pll_rate_table),
	PLL(CLK_PLL_VPLL, "vpll", "ext", &vpll_hwdesc, c100_pll_rate_table),
};
...
static struct ingenic_mux_clock c100_mux_clks[] __initdata = {
...
static struct ingenic_bus_clock c100_bus_div_clks[] __initdata = {
...
static struct clk_div_table c100_clk_div_table[256] = {};

static struct ingenic_div_clock c100_div_clks[] __initdata = {
	...
	DIV(CLK_DIV_MSC0,		"div_msc0",		"mux_msc0",		CPM_MSC0CDR,	8,  0, c100_clk_div_table),
	DIV(CLK_DIV_MSC1,		"div_msc1",		"mux_msc1",		CPM_MSC1CDR,	8,  0, c100_clk_div_table),
...
static struct ingenic_fra_div_clock c100_fdiv_clks[] __initdata = {
 ...
 *      GATE
 ********************************************************************************/
static struct ingenic_gate_clock c100_gate_clks[] __initdata = {
...
{
	int i;
	for (i = 0; i < ARRAY_SIZE(c100_clk_div_table); i++) {
		c100_clk_div_table[i].val = i;
		c100_clk_div_table[i].div = (i + 1) * 2;
	}
...
/* Register c100 clocks. */
static void __init c100_clk_init(struct device_node *np, void __iomem *base)
{
	void __iomem *reg_base;

	printk("c100 Clock Power Management Unit init!\n");
	...
	ingenic_clk_of_register_fixed_ext(ctx, c100_fixed_rate_ext_clks,
			                        ARRAY_SIZE(c100_fixed_rate_ext_clks), ext_clk_match);

	/* Register PLLs. */
	ingenic_clk_register_pll(ctx, c100_pll_clks,
				ARRAY_SIZE(c100_pll_clks), reg_base);


	/* Register Muxs */
	ingenic_clk_register_mux(ctx, c100_mux_clks, ARRAY_SIZE(c100_mux_clks));

	/* Register Bus Divs */
	ingenic_clk_register_bus_div(ctx, c100_bus_div_clks, ARRAY_SIZE(c100_bus_div_clks));

	/* Register Divs */
	clk_div_table_generate();
	ingenic_clk_register_cgu_div(ctx, c100_div_clks, ARRAY_SIZE(c100_div_clks));

	/* Register Fractional Divs */
	ingenic_clk_register_fra_div(ctx, c100_fdiv_clks, ARRAY_SIZE(c100_fdiv_clks));

	/* Register Gates */
	ingenic_clk_register_gate(ctx, c100_gate_clks, ARRAY_SIZE(c100_gate_clks));

	/* Register Powers */
//	ingenic_power_register_gate(ctx, c100_gate_power, ARRAY_SIZE(c100_gate_power));
...
CLK_OF_DECLARE(c100_clk, "ingenic,c100-clocks", c100_clk_init);
