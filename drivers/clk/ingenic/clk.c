				      ...
				      unsigned int num_regs)
{
	...
	for (; num_regs > 0; --num_regs, ++rd) {
		...
		if(!rd->ce && !rd->busy) {
			...
			if(rd->offset == CPM_ISPCDR || rd->offset == CPM_ISPACDR || rd->offset == CPM_TNPUCDR){
				if(rd->value & (1 << rd->stop)){
					rd->value &= ~(1 << rd->stop);
				}
			}

			if(rd->offset == CPM_TNPUCDR){
				val = readl(base + CPM_CLKGR1);
				writel(val & ~(1 << (CLK_GATE_TNPU % 32)), base + CPM_CLKGR1);
			}
			...
			cpm_outl(rd->value | (1 << rd->ce), rd->offset);
			timeout = 0xffffff;
			while(--timeout && cpm_inl(rd->offset) & (1 << rd->busy));
			if(!timeout) {
