...
static struct ingenic_gate_clock t41_gate_clks[] __initdata = {
	...
	GATE(CLK_GATE_IVDC,	    "gate_ivdc",		"div_ispa",	    CPM_CLKGR1,      17, 0,  		        CLK_GATE_SET_TO_DISABLE),
...
static ssize_t clock_set(struct file *file, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	if (!strncmp(buf, "pll_set", sizeof("pll_set")-1)) {
		...
		if (2 != ret) {
            printk("**********use help**********\n");
            printk("echo pll_set apll 1104000000 > /proc/jz/clock/clocks, to set apll/vpll\n");
			return EFAULT;
        }
