...
{
	struct sfc_transfer transfer[7];
	...
	switch(cmd) {
	case SFC_READ_SR:
		mutex_lock(&flash_write_pro->lock);
		for (i = 0; i < 3; i++) {
			transfer[1+i].tx_buf = &w_cmd[1+i];
			transfer[1+i].tx_buf1 = NULL;
			transfer[1+i].rx_buf = sr+i;
			transfer[1+i].len = 1;
			ret = jz_sfc_pio_txrx(flash_write_pro, &transfer[1+i]);
			if(ret != transfer[1+i].len)
				dev_err(flash_write_pro->dev,"the transfer length is error,%d,%s\n",__LINE__,__func__);
		}
		printk("===%s [%d]===>sr1 = %x || sr2 = %x || sr3 = %x  \n",__func__, __LINE__, sr[0], sr[1], sr[2]);
		mutex_unlock(&flash_write_pro->lock);
	...
	case SFC_WRITE_SR:
		...
		transfer[4].tx_buf = &w_cmd[4];
		transfer[4].tx_buf1 = &tmp;
		transfer[4].rx_buf = NULL;
		transfer[4].len = 1;

		ret = jz_sfc_pio_txrx(flash_write_pro, &transfer[4]);
		if(ret != transfer[4].len)
			dev_err(flash_write_pro->dev,"the transfer length is error,%d,%s\n",__LINE__,__func__);
	...
	case SFC_WRITE_SR1:
		...
		transfer[5].tx_buf = &w_cmd[5];
		transfer[5].tx_buf1 = &tmp;
		transfer[5].rx_buf = NULL;
		transfer[5].len = 1;

		ret = jz_sfc_pio_txrx(flash_write_pro, &transfer[5]);
		if(ret != transfer[5].len)
			dev_err(flash_write_pro->dev,"the transfer length is error,%d,%s\n",__LINE__,__func__);
	...
	case SFC_WRITE_SR2:
		...
		transfer[6].tx_buf = &w_cmd[6];
		transfer[6].tx_buf1 = &tmp;
		transfer[6].rx_buf = NULL;
		transfer[6].len = 1;

		ret = jz_sfc_pio_txrx(flash_write_pro, &transfer[6]);
		if(ret != transfer[6].len)
			dev_err(flash_write_pro->dev,"the transfer length is error,%d,%s\n",__LINE__,__func__);
