...
#else
	nor_info->quad_succeed = 0;
	if (read_standard_table.transfer_mode == 0) {
		dev_info(flash->dev, "nor flash now use standard mode!\n");
	} else {
		dev_info(flash->dev, "nor flash now use dual mode!\n");
	}
#endif
