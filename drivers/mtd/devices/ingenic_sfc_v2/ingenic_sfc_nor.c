...
static int ingenic_sfc_major = 238;
static dev_t ingenic_sfc_devid;
static struct cdev ingenic_sfc_cdev;
static struct class *ingenic_sfc_class;
static struct device *ingenic_sfc_device;
struct sfc_flash *flash_write_pro;
...
static int ingenic_spi_norflash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	while (addr < end) {
		ret = sfc_do_erase(flash, addr, instr->mtd->erasesize);
		if (ret) {
...
static int ingenic_spi_norflash_get_params(struct sfc_flash *flash)
{
	...
	for(i = 0; i < burner_params->spi_nor_info_size; i++)
	{
		if(chip_id == spi_nor_info_table[i].id)
		{
			...
			dev_info(flash->dev,"Found Supported nor device, id = %x,name = %s, %dKB Block Erase(BE)\n", chip_id, spi_nor_info_table[i].name, spi_nor_info_table[i].erase_size/1024);
			break;
		}
	}
...
static void nor_create_cdt_table(struct sfc_flash *flash, uint32_t flag)
{
	...
	if(flag == UPDATE_CDT){
		...
		write_cdt(flash->sfc, sfc_cdt, NOR_READ_STANDARD,NOR_STATUS_2_SET_FINISH);
	}
	//dump_cdt(flash->sfc);
}
...
static long ingenic_sfc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SFC_READ_SR2:
		...
		for(i=0;i<3;i++) {
			sfc_nor_read_status(flash_write_pro,w_cmd[i],&sr[i],1);
		}
	...
	case SFC_WRITE_SR:
		...
		mutex_lock(&flash_write_pro->lock);
		ret = sfc_nor_write_status(flash_write_pro,w_cmd[3],&tmp,1);
		if(ret < 0) {
	...
	case SFC_WRITE_SR1:
		get_user(tmp, (uint32_t *)arg);
		mutex_lock(&flash_write_pro->lock);
		ret = sfc_nor_write_status(flash_write_pro,w_cmd[4],&tmp,1);
		if(ret < 0) {
	...
	case SFC_WRITE_SR2:
		get_user(tmp, (uint32_t *)arg);
		mutex_lock(&flash_write_pro->lock);
		ret = sfc_nor_write_status(flash_write_pro,w_cmd[5],&tmp,1);
		if(ret < 0) {
	...
	case SFC_DO_READ:
		...
		switch(trans_config.status) {
			...
			case BUF_READ:
				mutex_lock(&flash_write_pro->lock);
				trans_config.retlen = sfc_read(flash_write_pro, trans_config.addr, trans_config.len, data_buf);
				mutex_unlock(&flash_write_pro->lock);
				break;
			case FREE_BUF:
				kfree(data_buf);
...
int ingenic_sfc_nor_probe(struct sfc_flash *flash)
{
	...
	sprintf(mtdname, "sfc%d_nor", flash->id);
	...
	ingenic_sfc_devid = MKDEV(ingenic_sfc_major, 0);
	register_chrdev_region(ingenic_sfc_devid, 1, "ingenic_sfc");

	ingenic_sfc_cdev.owner = THIS_MODULE;
	cdev_init(&ingenic_sfc_cdev, &ingenic_sfc_ops);
	cdev_add(&ingenic_sfc_cdev, ingenic_sfc_devid, 1);

	ingenic_sfc_class = class_create(THIS_MODULE, "ingenic_sfc");
	if (IS_ERR(ingenic_sfc_class)) {
		unregister_chrdev(ingenic_sfc_major,"ingenic_sfc");
		return -EBUSY;
	}

	ingenic_sfc_device = device_create(ingenic_sfc_class, NULL, ingenic_sfc_devid, NULL, "ingenic_sfc");
	if (IS_ERR(ingenic_sfc_device)){
		class_destroy(ingenic_sfc_class);
		unregister_chrdev(ingenic_sfc_major,"ingenic_sfc");
		return -EBUSY;
	}
