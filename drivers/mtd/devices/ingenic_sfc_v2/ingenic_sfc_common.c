...
static void sfc_dev_hw_init(struct sfc *sfc)
{
	...
	printk("Enter 'DMA Descriptor chain' mode. SFC use ");
	tmp = sfc_readl(sfc, SFC_GLB0);
	tmp |= GLB0_DES_EN;
	sfc_writel(sfc, SFC_GLB0, tmp);
...
#ifdef CPU_FIFO_1
		printk("(fifo = threshold - red_fifo_num)");
#endif
		printk("CPU mode\n");
	}
}
...
static inline void sfc_set_burst(struct sfc *sfc, uint32_t value)
{
	uint32_t tmp;
	tmp = sfc_readl(sfc, SFC_GLB0);
...
#ifdef CPU_FIFO_1	/* read fifo =  threshold - red_fifo_num*/
	int32_t old_num = 0;
	int32_t new_num = 0;
	int32_t red_fifo_num = 1;/* red_fifo_num < 3 */

	align_len = ALIGN(xfer->config.datalen, 4);
	old_num = align_len / (sfc->threshold * 4);
	new_num = align_len / ((sfc->threshold - red_fifo_num) * 4);
	if (align_len % (sfc->threshold * 4))
		old_num++;
	if (align_len % ((sfc->threshold - red_fifo_num) * 4))
		new_num++;

retry:
	if(((align_len - xfer->config.cur_len) / 4) > sfc->threshold - red_fifo_num) {
		fifo_num = sfc->threshold - red_fifo_num;
		last_word = 0;
	} else {
	...
	if (flag_num >= old_num && flag_num < new_num){
		goto retry;
	} else if (flag_num >= new_num) {
		flag_num = 0;
	} else
		;
#else	/* read fifo =  threshold */
	align_len = ALIGN(xfer->config.datalen, 4);
...
#define SFC_TRANSFER_TIMEOUT_WDT	100		//30ms for timeout
