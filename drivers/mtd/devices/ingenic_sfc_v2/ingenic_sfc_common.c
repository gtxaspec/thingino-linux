...
struct sfc *sfc_res_init(struct platform_device *pdev)
{
	...
	board_info->param_offset = 0;
	board_info->use_board_info = 0;
	...
	clk_enable(sfc->clk_gate);
		...
		printk("JZ-SFC,Use SFC QUAD Mode\n");
#else
		printk("JZ-SFC,Use SFC Standard Mode\n");
#endif

#ifdef CONFIG_SFC_DMA_MODE
		printk("JZ-SFC,Use SFC DMA Mode\n");
#else
		printk("JZ-SFC,Use SFC PIO Mode\n");
#endif
