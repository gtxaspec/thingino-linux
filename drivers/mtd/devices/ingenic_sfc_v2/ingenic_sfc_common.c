...
static void sfc_dev_hw_init(struct sfc *sfc)
{
	...
	/* set sfc burst 16 */
	tmp = sfc_readl(sfc, SFC_GLB0);
	tmp &= ~GLB0_BURST_MD_MSK;
	tmp |= GLB0_BURST_INCR16;
	sfc_writel(sfc, SFC_GLB0, tmp);
...
int32_t set_flash_timing(struct sfc *sfc, uint32_t t_hold, uint32_t t_setup, uint32_t t_shslrd, uint32_t t_shslwr)
{
	...
	ns = 1000000000ULL;
	//T32 T33: (2*t_xx + cycle -1) / cycle - 1;
	cycle = do_div(ns, clk_get_rate(sfc->clk_gate));
	cycle = ns;

	tmp = sfc_readl(sfc, SFC_DEV_CONF);
	tmp &= ~(DEV_CONF_THOLD_MSK | DEV_CONF_TSETUP_MSK | DEV_CONF_TSH_MSK);

	c_hold = (2*t_hold + cycle - 1) / cycle;
	if(c_hold > 0)
		c_hold -= 1;
	if(c_hold > 3)
		c_hold = 3;

	c_setup = (2*t_setup + cycle - 1) / cycle;
	if(c_setup > 0)
		c_setup -= 1;
	if(c_setup > 3)
		c_setup = 3;
...
#define SFC_TRANSFER_TIMEOUT	10000	//3000ms for timeout
/* #define SFC_TRANSFER_TIMEOUT_WDT 100	// 3000ms for timeout */
/* #define SFC_WAIT_CNT 30					// 30 * 100 = 3000ms for timeout */
static int32_t sfc_start_transfer(struct sfc *sfc)
{
	int32_t err;
	/* int32_t wdt_en = 0; */
	/* int time = SFC_WAIT_CNT; */
	...
	/* if(wdt_en) */
	/* 	outl((inl(WDT_IOBASE + 0x0c) | (1 << 10)), (WDT_IOBASE + 0x0c)); /1* 0x0c:Watchdog Control Register *1/ */
	sfc_start(sfc);

	//while (time--)
	//{
	//	/* 0x0c:Watchdog Control Register */
	//	wdt_en = inl(WDT_IOBASE + 0x04) & 0x1;
	//	if (wdt_en)
	//		outl((inl(WDT_IOBASE + 0x0c) | (1 << 10)), (WDT_IOBASE + 0x0c)); /* 0x0c:Watchdog Control Register */
	...
	if (!err) {
		...
		sfc_stop(sfc);
		sfc_flush_fifo(sfc);
		printk("err = %d, line:%d Timeout for ACK from SFC device\n",err, __LINE__);
		return -ETIMEDOUT;
	}
	return 0;
}
...
void ingenic_sfc_init_setup(struct sfc *sfc)
{
	...
	if(sfc->src_clk >= 100 * 1000000)
#ifdef CONFIG_ZERATUL
		sfc_smp_delay(sfc, DEV_CONF_SMP_DELAY_315);
#else
		sfc_smp_delay(sfc, DEV_CONF_SMP_DELAY_1);
#endif
	else if(sfc->src_clk >= 80 * 1000000)
		sfc_smp_delay(sfc, DEV_CONF_SMP_DELAY_225);
	else if(sfc->src_clk >= 50 * 1000000)
#ifdef CONFIG_ZERATUL
		sfc_smp_delay(sfc, DEV_CONF_SMP_DELAY_90);
#else
		sfc_smp_delay(sfc, DEV_CONF_SMP_DELAY_180);
#endif
	else
		sfc_smp_delay(sfc,DEV_CONF_SMP_DELAY_0);
}

static int sfc_ingenic_set_driver_strength(struct of_phandle_args *pinmux_spec, unsigned int val)
{
	 int strength,i;
	 if(val == 0)
		 return -1;
	 ...
	 if(!strcmp(pinmux_spec->np->name,"gpa")){
		 for(i = 23; i < 29; i++)
			 jzgpio_set_func(GPIO_PORT_A, strength, i);
	 }else if(!strcmp(pinmux_spec->np->name,"gpc") && pinmux_spec->args[0] == 2){
		 for(i = 2; i < 8; i++)
			 jzgpio_set_func(GPIO_PORT_C, strength, i);
	 }else if(!strcmp(pinmux_spec->np->name,"gpc") && pinmux_spec->args[0] == 11){
		 for(i = 11; i < 17; i++)
			 jzgpio_set_func(GPIO_PORT_C, strength, i);
	 }
...
struct sfc *sfc_res_init(struct platform_device *pdev)
{
	...
	if (err < 0) {
	 ...
	 err = of_property_read_u32(np, "driver-strength", &val);
	 if (err < 0) {
		 dev_err(&pdev->dev, "Cannot get sfc driver strength\n");
		 return ERR_PTR(-ENOENT);
	 }

	 if(!(pinctrl_node = of_parse_phandle(np, "pinctrl-0", 0))){
		 dev_err(&pdev->dev, "Failed to parse pinctrl-0\n");
	 }

	 if(of_parse_phandle_with_fixed_args(pinctrl_node, "ingenic,pinmux", 2, 0, &pinmux_spec)){
		 dev_err(&pdev->dev, "Failed to parse ingenic,pinmux\n");
		 of_node_put(pinctrl_node);
		 return -EINVAL;
	 }

	 err = sfc_ingenic_set_driver_strength(&pinmux_spec,val);
	 if (err < 0) {
		 dev_err(&pdev->dev, "Failed to set sfc driver strength\n");
	 }

	 if (of_property_read_bool(np, "sfc_single_pa"))
	 {
		 jzgpio_set_func(GPIO_PORT_A, GPIO_OUTPUT0, 1 << 25);
		 jzgpio_set_func(GPIO_PORT_A, GPIO_OUTPUT0, 1 << 26);
	 }
