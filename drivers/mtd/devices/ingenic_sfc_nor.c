...
#ifdef CONFIG_SFC_NOR_INTERFACE_USE
/* use "void *" to suppress compile warning */
void *Recoveryflash;
#endif
...
{
	struct sfc_flash *flash = Recoveryflash;

	mutex_lock(&flash->lock);
	sfc_read(flash, from, len, buf);
	mutex_unlock(&flash->lock);
...
#ifdef CONFIG_SFC_NOR_INTERFACE_USE
int recovery_norflash_erase(struct erase_info *instr)
{
	struct sfc_flash *flash = Recoveryflash;
	struct spinor_flashinfo *nor_info = flash->flash_info;
	struct spi_nor_info *spi_nor_info = nor_info->nor_flash_info;
...
int direct_erase_norflash(int offset, int size)
{
	...
	printk("##############erase_flash add=%llu,len=%llu#####\n",instr.addr, instr.len);
