...
#define SIZE_BOOTLOADER  (0x100000 * 32)
#define SIZE_KERNEL (0x100000 * 32)
#define SIZE_ROOTFS (0x100000 * 64)
#define SIZE_DATA (0x100000 * 32)

#define SIZE_ALL (0x100000 * 128)
...
struct mtd_partition jz_mtd_spinand_partition[] = {
	...
	{
		.name =     "zrtdata3",
		.offset =   SIZE_KERNEL + SIZE_BOOTLOADER + SIZE_DATA,
		.size =     SIZE_DATA,
	},
...
static int jz_spi_nandflash_erase_blk(struct jz_spi_nandflash *flash,uint32_t addr)
{
	...
	if(status & E_FALI){
		pr_info("[status = %x ]Erase error,get state error ! %s %s %d \n",status, __FILE__,__func__,__LINE__);
		return -EIO;
	}
	return 0;
}
...
static int32_t jz_spi_nandflash_write_ops(struct jz_spi_nandflash *flash, u_char *buf, int column, int page, size_t len)
{
	print_dbg("step enter [%s] [%d] \n", __func__, __LINE__);
	struct spi_message message;
	...
	memcpy(command_write + 3 , buf, len);
	transfer[0].tx_buf = command_write;
	transfer[0].len = len + 3;
	transfer[0].cs_change = 1;
	spi_message_add_tail(&transfer[0], &message);
	...
	if(status & p_FAIL){
		printk("spi nand write fail %s %s %d \n", __FILE__, __func__, __LINE__);
		ret =-EBADMSG;
		goto nandflash_write_ops_exit;
	}

nandflash_write_ops_exit:
	print_dbg("step out[%s] [%d] \n", __func__, __LINE__);
	return ret;
}
...
{
//	int count = 0;
//	unsigned char *test_buf = buf;
	int ret,rlen;
	int page_size = mtd->writesize;
	struct jz_spi_nandflash *flash;
	size_t rel_rlen = 0;
	uint32_t pageaddr, column;

	flash = container_of(mtd, struct jz_spi_nandflash, mtd);
	mutex_lock(&flash->lock);
	while(len){
		pageaddr = (uint32_t)from / page_size;
		column = (uint32_t) from % page_size;
		rlen = min_t(uint32_t, len, page_size - column);
		ret = jz_spi_nandflash_read_ops(flash, buf, pageaddr, column, rlen, &rel_rlen);
		if(ret < 0){
			printk("[%s] [%s] [%d] read failed [ret = %d] [pageaddr = %u] [column = %u] [wlen = %u] \n", __FILE__, __func__, __LINE__, ret, pageaddr, column, rlen);
			break;
		}
...
static int jz_spinand_write(struct mtd_info *mtd, loff_t to, size_t len,size_t *retlen, const u_char *buf)
{
	...
	*retlen = 0;
	...
	if(to & (mtd->writesize - 1)){
		print_dbg("wirte add don't align ,error ! %s %s %d \n",__FILE__,__func__,__LINE__);
		ret=-EINVAL;
	...
	while(len){
		print_dbg("step [%s] [%d] \n", __func__, __LINE__);
		pageaddr = (uint32_t)to / page_size;
		column = (uint32_t) to % page_size;
		wlen = min_t(uint32_t, page_size - column, len);
		ret = jz_spi_nandflash_write_ops(flash, buf, column, pageaddr, wlen);
		if(ret){
			printk("[%s] [%s] [%d] write failed [ret = %d] [pageaddr = %u] [column = %u] [wlen = %u] \n", __FILE__, __func__, __LINE__, ret, pageaddr, column, wlen);
			break;
		}
		*retlen += wlen;
		len -= wlen;
...
static int jz_spinand_read_oob(struct mtd_info *mtd,loff_t from,struct mtd_oob_ops *ops)
{
	struct jz_spi_nandflash *flash;
	...
	mutex_lock(&flash->lock);
	if(ops->datbuf){
		ret = jz_spinand_read(mtd, from, ops->len, &ops->retlen, ops->datbuf);
		if(ret < 0){
			printk("[%s] [%s] [%d] spi nand read data error ,ret = %d \n", __FILE__, __func__, __LINE__, ret);
		}
	}

	if(ops->oobbuf){
		ret = jz_spi_nandflash_read_ops(flash, ops->oobbuf, pageaddr, mtd->writesize + ops->ooboffs, ops->ooblen, &oob_rlen);
		if(ret < 0){
			printk("[%s] [%s] [%d] spi nand read oob error ,ret = %d \n", __FILE__, __func__, __LINE__, ret);
		}
	}
...
static int jz_spinand_write_oob(struct mtd_info *mtd, loff_t addr, struct mtd_oob_ops *ops)
{
	...
	flash = container_of(mtd, struct jz_spi_nandflash, mtd);
	mutex_lock(&flash->lock);
	ret = jz_spi_nandflash_write_ops(flash, ops->oobbuf, mtd->writesize, oob_addr / mtd->writesize, ops->ooblen);
	if(ret){
		printk("[%s] [%s] [%d] jz_spinand_write_oob failed! ret = %d \n", __FILE__, __func__, __LINE__, ret);
		goto write_oob_exit;
	}
...
static int jz_spi_nandflash_probe(struct spi_device *spi)
{
	...
	spi_nandflash->mtd.bitflip_threshold = spi_nandflash->mtd.ecc_strength = spi_flash->ecc_strength - 1; //(0x4 - 1)
