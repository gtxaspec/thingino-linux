				      ...
				      int extra_mem_size)
{
	...
	if (*s == ',') {
		if (size == SIZE_REMAINING) {
			...
			if (*next_char != '@') {
				printk("mtdparts: no partitions allowed after a fill-up partition without an offset\n");
				return ERR_PTR(-EINVAL);
			}
		}
				    ...
				    struct mtd_part_parser_data *data)
{
	...
	char *tmp = cmdline;
	char cmdline_actual[1024];
	char colon_position=0;

	if(cmdline) {
		strcpy(cmdline_actual,cmdline);
		for(;*tmp != '\0';) {
			if(*(tmp) == ';')
				break;
			colon_position++;
		...
		if(master->sfc_index == 0) {
			*(cmdline_actual+colon_position) = '\0';
		} else if(master->sfc_index == 1) {
			for(i=0;*tmp != '\0';i++) {
				tmp++;
				*(cmdline_actual+i) = *tmp;
			}
		}
	}
	/* parse command line */
	if (!cmdline_parsed) {
		err = mtdpart_setup_real(cmdline_actual);
		if (err)
			return err;
	}
