...
#ifdef CONFIG_SKIP_SQUASHFS_BAD_BLOCK
#define MAX_PARTITION_MAPPING 16
static struct part_map *static_part_mapping[MAX_PARTITION_MAPPING];
static struct part_map **part_mapping = NULL;
...
static loff_t ajust_offset(struct mtd_info *mtd, loff_t from)
{
	...
	if(mtd->index >= part_mapping_count || part_mapping[mtd->index] == NULL)
		return from;

	/* remap from logic block to physical block */
	logic_b = from >> this->bbt_erase_shift;
	if ( logic_b < part_mapping[mtd->index]->nBlock )
	{
		phys_b = part_mapping[mtd->index]->map_table[logic_b];
...
static int part_create_partition_mapping ( struct mtd_info *part_mtd )
{
    ...
    if(part_mtd->index >= part_mapping_count || part_mapping[part_mtd->index] != NULL)
    {
    ...
    part_mapping[part_mtd->index] = map_part;
...
static void part_del_partition_mapping( struct mtd_info *part_mtd )
{
    ...
    if(part_mtd->index >= part_mapping_count || part_mapping[part_mtd->index] == NULL)
    {
        printk("partition mapping is empty!");
        return;
    }

	map_part = part_mapping[part_mtd->index];

	kfree(map_part->map_table);
	kfree(map_part);
	part_mapping[part_mtd->index] = NULL;
}
...
#endif /* CONFIG_SKIP_SQUASHFS_BAD_BLOCK */
/*
...
#ifdef CONFIG_SKIP_SQUASHFS_BAD_BLOCK
	if(nbparts > MAX_PARTITION_MAPPING){
		part_mapping = kmalloc(sizeof(void *) * nbparts, GFP_KERNEL);
		if(part_mapping == NULL)
			printk(KERN_NOTICE "Failed to malloc %d part_mapping\n", nbparts);
	}
	if(part_mapping == NULL){
		part_mapping = static_part_mapping;
        part_mapping_count = MAX_PARTITION_MAPPING;
	}else{
        part_mapping_count = nbparts;
	}
	memset(part_mapping, 0, sizeof(struct part_map *)*part_mapping_count);
#endif
	for (i = 0; i < nbparts; i++) {
