						...
						bool is_usb_reset)
{
	...
	if (!is_usb_reset) {
		...
		/* all endpint should be shutdown */
			       ...
			       const struct usb_endpoint_descriptor *desc)
{
	...
	if (dir_in && hsotg->dedicated_fifos && !hs_ep->fifo_index) {
		...
		/*printk("ep:%d,fifi_index:%d\n",index,fifo_index);*/
		/* add ywx : 解决双码流切换摄像头开流顺序导致视频黑屏 */
						...
						struct dwc2_hsotg_ep *hs_ep)
{
	...
	if (hs_ep->dir_in) {
		if (hsotg->dedicated_fifos || hs_ep->periodic) {
			...
			if (dwc2_hsotg_wait_bit_set(hsotg, GINTSTS,
				...
				dev_warn(hsotg->dev,
					 "%s: timeout GINTSTS.NAKEFF\n", __func__);

		}
	} else {
		/*Mask GINTMSK_GOUTNAKEFF interrupt */
		...
		if (!using_dma(hsotg)) {
			...
			if (dwc2_hsotg_wait_bit_set(hsotg, GINTSTS,
				...
				/*
				 * pop GLOBAL OUT_NAK status packet from RxFIFO
				 * to assert GOUTNakEff interrupt
				 */
				dwc2_readl(hsotg->regs + GRXSTSP);
			}
		}
	...
	if (hs_ep->dir_in) {
		...
		/* Clear Global In NP Nak in share FIFO for non periodic ep */
		if (!hsotg->dedicated_fifos && !hs_ep->periodic)
...
int dwc2_hsotg_suspend(struct dwc2_hsotg *hsotg)
{
	...
	if (hsotg->driver) {
		...
		/* don't try to disable ep0 in dwc2_hsotg_suspend */
		for (ep = 1; ep < hsotg->num_of_eps; ep++) {
			if (hsotg->eps_in[ep])
				dwc2_hsotg_ep_disable_lock(&hsotg->eps_in[ep]->ep);
			if (hsotg->eps_out[ep])
