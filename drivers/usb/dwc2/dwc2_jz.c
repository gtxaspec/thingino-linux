...
#define OTG_CLK_NAME		"gate_otg1"
#define VBUS_REG_NAME		"vbus"
#define DIV_USB_CLK_NAME	"otg_phy"
...
static void  platform_dwc2_release(struct device *dev)
{
	return ;
}

struct platform_device jz_dwc2_device = {
	.name   = "jz-dwc2",
	.id = -1,
	.resource   = jz_dwc_otg_resources,
	.dev = {
		.release = platform_dwc2_release,
	},
...
static void jz_usb_release(struct device *dev)
{
	return;
}
...
static int dwc2_jz_probe(struct platform_device *pdev) {
	...
	dwc2->dev.release = jz_usb_release;
	...
	jz->div_clk = devm_clk_get(&pdev->dev, DIV_USB_CLK_NAME);
	if (IS_ERR(jz->div_clk)) {
		dev_warn(&pdev->dev, "div clk gate get error\n");
		jz->div_clk = NULL;
	}
	...
	if (jz->div_clk) {
		clk_set_rate(jz->div_clk, 24000000);
		clk_enable(jz->div_clk);
	}
	clk_enable(jz->clk);
	if (jz->pwr_clk)
...
fail_reuqest_irq:
	...
	platform_driver_unregister(&jz->dwc2);
#endif
...
{
	int ret = 0;
	ret = platform_device_register(&jz_dwc2_device);
	if(ret){
		printk("Failed to insmod dwc2 device driver!!!\n");
		return ret;
	}
	dwc2_init();
...
static void __exit dwc2_jz_exit(void)
{
	...
	platform_device_unregister(&jz_dwc2_device);
	jz_otg_ctr_reset();
}
