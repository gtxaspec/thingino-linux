...
static int iphy_init(struct usb_phy *x)
{
	...
	//u8 reg;
        ...
        printk("ingenic usb phy_init otgid:%d\n",iphy->otgid);
	...
	spin_lock_irqsave(&iphy->phy_lock, flags);
        ...
        if(2 == iphy->otgid){
                /*    usb port 2     */
                usbpcr1 = inno_usbphy_read(x, CPM_USB2PCR1);
                usbpcr1 |= USBPCR1_DPPULLDOWN | USBPCR1_DMPULLDOWN;
                inno_usbphy_write(x, usbpcr1, CPM_USB2PCR1);

                usbpcr = inno_usbphy_read(x, CPM_USB2PCR);
...
#endif
                inno_usbphy_write(x, usbpcr, CPM_USB2PCR);

                inno_usbphy_update_bits(x, CPM_USB2PCR1, USBPCR1_PORT_RST, USBPCR1_PORT_RST);

                inno_usbphy_update_bits(x, CPM_USB2RDT, USBRDT_UTMI_RST, 0);
                inno_usbphy_update_bits(x, CPM_USB2PCR, USBPCR_POR, USBPCR_POR);
                inno_usbphy_update_bits(x, CPM_SRBC0, SRBC_USB2_SR, SRBC_USB2_SR);
                udelay(5);
                inno_usbphy_update_bits(x, CPM_USB2PCR, USBPCR_POR, 0);
                udelay(10);
                inno_usbphy_update_bits(x, CPM_OPCR, OPCR_USB_SPENDN2, OPCR_USB_SPENDN2);
                udelay(550);
                inno_usbphy_update_bits(x, CPM_USB2RDT, USBRDT_UTMI_RST, USBRDT_UTMI_RST);
                udelay(10);
                inno_usbphy_update_bits(x, CPM_SRBC0, SRBC_USB2_SR, 0);

#if 0
                //Edwin.wen@20220920 Enhance DP DM pins drive capability.
                usb_phy_writeb(0xf,iphy->base + 0x830);
                usb_phy_writeb(0x1c,iphy->base + 0x924);
                usb_phy_writeb(0x69,iphy->base + 0x840);
                usb_phy_writeb(0x80,iphy->base + 0x864);
#endif

        }else if(1 == iphy->otgid){

                /*    usb port 1     */
                usbpcr1 = inno_usbphy_read(x, CPM_USB1PCR1);
                usbpcr1 |= USBPCR1_DPPULLDOWN | USBPCR1_DMPULLDOWN;
                inno_usbphy_write(x, usbpcr1, CPM_USB1PCR1);

                usbpcr = inno_usbphy_read(x, CPM_USB1PCR);
                usbpcr &= ~USBPCR_IDPULLUP_MASK;
#if IS_ENABLED(CONFIG_USB_DWC2_HOST)
                usbpcr |= USBPCR_USB_MODE | USBPCR_IDPULLUP_OTG;
#elif IS_ENABLED(CONFIG_USB_DWC2_PERIPHERAL)
                usbpcr &= ~USBPCR_USB_MODE;
#endif
                inno_usbphy_write(x, usbpcr, CPM_USB1PCR);

                inno_usbphy_update_bits(x, CPM_USB1PCR1, USBPCR1_PORT_RST, USBPCR1_PORT_RST);

                inno_usbphy_update_bits(x, CPM_USB1RDT, USBRDT_UTMI_RST, 0);
                inno_usbphy_update_bits(x, CPM_USB1PCR, USBPCR_POR, USBPCR_POR);
                inno_usbphy_update_bits(x, CPM_SRBC0, SRBC_USB1_SR, SRBC_USB1_SR);
                udelay(5);
                inno_usbphy_update_bits(x, CPM_USB1PCR, USBPCR_POR, 0);
                udelay(10);
                inno_usbphy_update_bits(x, CPM_OPCR, OPCR_USB_SPENDN1, OPCR_USB_SPENDN1);
                udelay(550);
                inno_usbphy_update_bits(x, CPM_USB1RDT, USBRDT_UTMI_RST, USBRDT_UTMI_RST);
                udelay(10);
                inno_usbphy_update_bits(x, CPM_SRBC0, SRBC_USB1_SR, 0);

#if 0
                //Edwin.wen@20220920 Enhance DP DM pins drive capability.
                usb_phy_writeb(0xf,iphy->base + 0x430);
                usb_phy_writeb(0x1c,iphy->base + 0x524);
                usb_phy_writeb(0x69,iphy->base + 0x440);
                usb_phy_writeb(0x80,iphy->base + 0x464);
#endif

        }else{

                /*    usb port 0   */
                usbpcr1 = inno_usbphy_read(x, CPM_USB0PCR1);
                usbpcr1 |= USBPCR1_DPPULLDOWN | USBPCR1_DMPULLDOWN;
                inno_usbphy_write(x, usbpcr1, CPM_USB0PCR1);

                usbpcr = inno_usbphy_read(x, CPM_USB0PCR);
                usbpcr &= ~USBPCR_IDPULLUP_MASK;
#if IS_ENABLED(CONFIG_USB_DWC2_HOST)
                usbpcr |= USBPCR_USB_MODE | USBPCR_IDPULLUP_OTG;
#elif IS_ENABLED(CONFIG_USB_DWC2_PERIPHERAL)
                usbpcr &= ~USBPCR_USB_MODE;
#endif
                inno_usbphy_write(x, usbpcr, CPM_USB0PCR);

                inno_usbphy_update_bits(x, CPM_USB0PCR1, USBPCR1_PORT_RST, USBPCR1_PORT_RST);

                inno_usbphy_update_bits(x, CPM_USB0RDT, USBRDT_UTMI_RST, 0);
                inno_usbphy_update_bits(x, CPM_USB0PCR, USBPCR_POR, USBPCR_POR);
                inno_usbphy_update_bits(x, CPM_SRBC0, SRBC_USB0_SR, SRBC_USB0_SR);

                udelay(5);
                inno_usbphy_update_bits(x, CPM_USB0PCR, USBPCR_POR, 0);
                udelay(10);
                inno_usbphy_update_bits(x, CPM_OPCR, OPCR_USB_SPENDN, OPCR_USB_SPENDN);
                udelay(550);
                inno_usbphy_update_bits(x, CPM_USB0RDT, USBRDT_UTMI_RST, USBRDT_UTMI_RST);
                udelay(10);
                inno_usbphy_update_bits(x, CPM_SRBC0, SRBC_USB0_SR, 0);

#if 0
                //Edwin.wen@20220920 Enhance DP DM pins drive capability.
                usb_phy_writeb(0xf,iphy->base + 0x30);
                usb_phy_writeb(0x1c,iphy->base + 0x124);
                usb_phy_writeb(0x69,iphy->base + 0x40);
                usb_phy_writeb(0x80,iphy->base + 0x64);
#endif
        }
...
static int iphy_set_suspend(struct usb_phy *x, int suspend)
{
	...
	spin_lock_irqsave(&iphy->phy_lock, flags);
        ...
        pr_debug("usb%d suspend CPM_OPCR:%x\n",iphy->otgid,inno_usbphy_read(x,CPM_OPCR));
        if(suspend){
                if(2 == iphy->otgid){
                        inno_usbphy_update_bits(x, CPM_OPCR, OPCR_USB_SPENDN2,
                        suspend ? 0 : OPCR_USB_SPENDN2);
                }else if(1 == iphy->otgid){
                        inno_usbphy_update_bits(x, CPM_OPCR, OPCR_USB_SPENDN1,
                                              suspend ? 0 : OPCR_USB_SPENDN1);
                }else if(0 == iphy->otgid){
...
#if 0 //Edwin.Wen@20230424 comment this,and rewrite suspend code.
#if IS_ENABLED(CONFIG_USB_DWC2_PERIPHERAL)
	if(suspend){
                if(2 == iphy->otgid){
                        dev_err(x->dev,"%s,otg2\n",__func__);
#if 0

                        cpm_usbpcr1 = inno_usbphy_read(x, CPM_USB2PCR1);
                        cpm_usbpcr1 &= ~(1 << 30);
                        inno_usbphy_write(x, cpm_usbpcr1, CPM_USB2PCR1);

                        usbrdt = inno_usbphy_read(x, CPM_USB2RDT);
                        ...
                        cpm_clkgr |= (1 << CPM_A1_CLKGR0_OTG2_GATE);
                        inno_usbphy_write(x,cpm_clkgr,CPM_CLKGR);
#endif
                }else if(1 == iphy->otgid){

                        dev_err(x->dev,"%s,otg1\n",__func__);
                        cpm_usbpcr1 = inno_usbphy_read(x, CPM_USB1PCR1);
                        cpm_usbpcr1 &= ~(1 << 30);
                        inno_usbphy_write(x, cpm_usbpcr1, CPM_USB1PCR1);

                        usbrdt = inno_usbphy_read(x, CPM_USB1RDT);
                        usbrdt &= ~USBRDT_RESUME_SPEED_MSK;
                        speed = (otg && otg->gadget) ? otg->gadget->speed : USB_SPEED_FULL;
                        if(speed == USB_SPEED_FULL)
                                usbrdt |= USBRDT_RESUME_SPEED_FULL;
                        ...
                        //TODO: the function be in suspend.
                        //usbrdt |= USBRDT_RESUME_INTEEN;
                        inno_usbphy_write(x, usbrdt, CPM_USB1RDT);

                        usb_phy_writeb(0x8,iphy->base + PHY_SUSPEND_LPM);

                        cpm_opcr =inno_usbphy_read(x,CPM_OPCR);
                        cpm_opcr &= ~OPCR_USB_SPENDN1;
                        inno_usbphy_write(x,cpm_opcr,CPM_OPCR);
                        udelay(10);
                        cpm_opcr =inno_usbphy_read(x,CPM_OPCR);
                        cpm_opcr |= OPCR_USB_PHY_GATE;
                        inno_usbphy_write(x,cpm_opcr,CPM_OPCR);
                        cpm_clkgr =inno_usbphy_read(x,CPM_CLKGR);
                        cpm_clkgr |= (1 << CPM_A1_CLKGR0_OTG1_GATE);
                        inno_usbphy_write(x,cpm_clkgr,CPM_CLKGR);

                }else{

                        dev_err(x->dev,"%s,otg0\n",__func__);
                        cpm_usbpcr1 = inno_usbphy_read(x, CPM_USB0PCR1);
                        cpm_usbpcr1 &= ~(1 << 30);
                        inno_usbphy_write(x, cpm_usbpcr1, CPM_USB0PCR1);

                        usbrdt = inno_usbphy_read(x, CPM_USB0RDT);
                        usbrdt &= ~USBRDT_RESUME_SPEED_MSK;
                        speed = (otg && otg->gadget) ? otg->gadget->speed : USB_SPEED_FULL;
                        if(speed == USB_SPEED_FULL)
                                usbrdt |= USBRDT_RESUME_SPEED_FULL;
                        ...
                        //TODO: the function be in suspend.
                        //usbrdt |= USBRDT_RESUME_INTEEN;
                        inno_usbphy_write(x, usbrdt, CPM_USB0RDT);
                        ...
                        cpm_clkgr =inno_usbphy_read(x,CPM_CLKGR);
                        cpm_clkgr |= (1 << CPM_A1_CLKGR0_OTG0_GATE);
                        inno_usbphy_write(x,cpm_clkgr,CPM_CLKGR);
...
static int iphy_set_wakeup(struct usb_phy *x, bool enabled)
{
	...
	if (enabled) {
		...
		inno_usbphy_update_bits(x, CPM_USB0RDT, USBRDT_RESUME_INTEEN|USBRDT_RESUME_INTERCLR,
	...
	if (enabled) {
		...
		if (otg && otg->state >= OTG_STATE_A_IDLE)
			...
			speed = (otg && otg->gadget) ? otg->gadget->speed : USB_SPEED_FULL;
		inno_usbphy_update_bits(x, CPM_USB0RDT, USBRDT_RESUME_INTEEN, USBRDT_RESUME_INTEEN);
	} else {
		/*disable usb resume interrupt*/
		inno_usbphy_update_bits(x, CPM_USB0RDT, USBRDT_RESUME_INTEEN|USBRDT_RESUME_INTERCLR,
				USBRDT_RESUME_INTERCLR);
