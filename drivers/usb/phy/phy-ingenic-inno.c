...
static int iphy_init(struct usb_phy *x)
{
	...
	udelay(10);
	inno_usbphy_update_bits(x, CPM_OPCR, OPCR_USB_SPENDN0, OPCR_USB_SPENDN0);
	udelay(550);
...
static int iphy_set_suspend(struct usb_phy *x, int suspend)
{
	...
	inno_usbphy_update_bits(x, CPM_OPCR, OPCR_USB_SPENDN0,
			suspend ? 0 : OPCR_USB_SPENDN0);
	if (!suspend) {
