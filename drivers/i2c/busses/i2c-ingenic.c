...
#define I2C_HHCNT		(0x24)
#define I2C_HLCNT		(0x28)
#endif
...
static int i2c_set_speed(struct i2c_ingenic *i2c, int rate)
{
	...
	if (rate <= 100000) {
		...
		i2c_writel(i2c, I2C_HHCNT, I2CFHCNT_ADJUST(cnt_high));
		i2c_writel(i2c, I2C_HLCNT, I2CFLCNT_ADJUST(cnt_low));
	}
#endif
...
static int i2c_ingenic_probe(struct platform_device *pdev)
{
	...
	if(i2c->id == 0){
		...
		//dev_info(&pdev->dev, "i2c%d clk is %d.\n", i2c->id, i2c->speed_hz);
#endif
	} else if(i2c->id == 1){
	...
	if(i2c->id == 0){
		...
		clk_disable_unprepare(i2c->clk);
#else
		dev_info(&pdev->dev, "i2c%d skip enable.\n", i2c->id);
#endif
	} else if(i2c->id == 1){
#ifndef CONFIG_I2C1_BOOT_DEFAULT
		i2c_ingenic_enable(i2c, 1);
...
#endif
	dev_info(&pdev->dev, "register i2c%d success, speed is %d.\n", i2c->id, i2c->speed_hz);
	return 0;
}
...
static const struct of_device_id ingenic_i2c_dt_match[] = {
	...
	{ .compatible = "ingenic,PRJ007-i2c" },
	{ .compatible = "ingenic,PRJ008-i2c" },
	{},
};
