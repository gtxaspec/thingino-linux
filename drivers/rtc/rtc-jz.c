...
static int write_rtc_reg(struct jz_rtc *rtc, struct set_reg *prv)
{
	int timeout = 1000;
	do{
		jzrtc_writel(rtc, prv->reg, prv->val);
	}while((prv->val != jzrtc_readl(rtc, prv->reg)) && timeout--);
...
static unsigned int read_rtc_reg(struct jz_rtc *rtc,struct set_reg *prv)
{
	return jzrtc_readl(rtc, prv->reg);
}

static int ctrl_rtc_reg(struct device *dev,unsigned long arg)
{
	struct set_reg rv;
	int ret = -1;
	struct jz_rtc *rtc = dev_get_drvdata(dev);
...
static void jz_rtc_enable(struct jz_rtc *rtc)
{
	 ...
	 */
#if defined(CONFIG_SOC_PRJ007) && (defined (CONFIG_PM) || defined (CONFIG_ZERATUL))
	unsigned int reg = 0;
...
#endif
	//cpm_set_clock(DIV_RTCCLK, 32768); //later to know if we need to set it ,it may decided by hardware.
	cfc = HSPR_RTCV;
...
static int jz_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	jzrtc_writel(rtc, RTC_HWCR, (0x1a55a5a5 << 3) | 1);
	printk("alarm suspend count = %d\n", rtc->sleep_count++);
#elif defined (SUSPEND_KEY_WAKEUP)
	jzrtc_writel(rtc, RTC_HWCR, (0x1a55a5a5 << 3));
