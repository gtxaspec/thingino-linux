			      ...
			      struct stmmac_dma_cfg *dma_cfg, int atds)
{
	...
	writel(value | XGMAC_INTCLEVEL, ioaddr + XGMAC_DMA_MODE);
}
...
static void dwxgmac2_dma_axi(void __iomem *ioaddr, struct stmmac_axi *axi)
{
	...
	writel(value, ioaddr + XGMAC_DMA_SYSBUS_MODE);
//	writel(XGMAC_TDPS | 0xc0000000, ioaddr + XGMAC_TX_EDMA_CTRL);
				  ...
				  struct stmmac_extra_stats *x, u32 chan)
{
	...
	/* TX/RX NORMAL interrupts, when intc type is level (config 0x3000 bit12), XGMAC_NIS could not occur */
				    ...
				    struct dma_features *dma_cap)
{
	...
	dma_cap->half_duplex = (hw_cap & XGMAC_HW_FEAT_HDSEL) >> 3;
...
static void dwxgmac2_rx_watchdog(void __iomem *ioaddr, u32 riwt, u32 nchan)
{
	...
	for (i = 0; i < nchan; i++)
		writel((riwt & XGMAC_RWT) | DMA_RIWT_UNIT_2048, ioaddr + XGMAC_DMA_CH_Rx_WATCHDOG(i)); //by xhshen, Increase watchdog delay time
}
