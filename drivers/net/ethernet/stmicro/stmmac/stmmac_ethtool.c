...
static int stmmac_ethtool_get_settings(struct net_device *net_dev,
				struct ethtool_cmd *cmd)
{
    struct stmmac_priv *priv = netdev_priv(net_dev);
    if (!priv)
    return -ENODEV;

    /* /sys/class/net/eth0/speed     /kernel/net/core/net-sysfs.c callback */
...
static const struct ethtool_ops stmmac_ethtool_ops = {
	...
	.get_settings = stmmac_ethtool_get_settings,
};
