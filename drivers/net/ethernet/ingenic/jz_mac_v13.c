...
static int  jz_mii_bus_probe(struct platform_device *pdev)
{
	...
	clk_gate = clk_get(NULL, "gate_gmac");
	clk_div = clk_get(NULL, "div_gmac");

	if (clk_enable(clk_gate) < 0) {
		printk("enable mac clk gate failed\n");
		goto out_err_alloc;
	}
	if (clk_enable(clk_div) < 0) {
		printk("enable gmac clk div failed\n");
		clk_put(clk_gate);
		goto out_err_alloc;
	}
#ifdef CONFIG_FPGA_TEST
	clk_set_rate(clk_div, 50000000);
#else
	clk_set_rate(clk_div, 25000000);
#endif
	/* //	synopGMAC_multicast_enable(gmacdev); */
