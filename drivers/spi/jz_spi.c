...
static int jz_spi_dma_txrx(struct spi_device *spi, struct spi_transfer *t)
{
	...
	set_tx_trigger(hw, 0); //The transfer is steady if the trigger number is used
	...
	if (dma_map_sg(hw->txchan->device->dev,hw->sg_tx, 1, DMA_TO_DEVICE) != 1) {
		dev_err(&spi->dev, "dma_map_sg tx error\n");
		printk("%s LINE %d: %s\n", __func__, __LINE__, __FILE__);
		goto err_tx_sgmap;
	}

	txdesc = txchan->device->device_prep_slave_sg(txchan, hw->sg_tx, 1, DMA_TO_DEVICE, DMA_PREP_INTERRUPT | DMA_CTRL_ACK,NULL);
	if(!txdesc) {
	...
	set_rx_trigger(hw, 0); //the rx trigger is steady for tranfer
	...
	if (dma_map_sg(hw->rxchan->device->dev,hw->sg_rx, 1, DMA_FROM_DEVICE) != 1) {
