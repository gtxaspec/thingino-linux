...
static int jz_spi_set_clk(struct spi_device *spi, u32 hz)
{
	...
	cgv = cpm_rate_a / (4 * hz) - 1;
	if (cgv < 0){
		printk("spi clk set %ld not support, max src_rate = %ld\n", hz, cpm_rate_a/4);
                return -1;
	}

	/* The cgu_ssi freq is 200M and the SPI available divisions include 4(50M),8,16(12.5M),20,32(6.25M),40,50,100 and 200(1M) */
...
{
	/* Freq of cgu_ssi clock is set to 200M, which ensure max freq of SPI is 50M */
        clk_set_rate(hw->clk, hw->src_clk * 4);
