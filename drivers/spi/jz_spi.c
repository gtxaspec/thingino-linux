...
static int jz_spi_set_clk(struct spi_device *spi, u32 hz)
{
	...
	cgv = (cpm_rate / (hz * 2)) - 1;
	if(cgv < 0) {
		printk("spi clk set %ld not support, cpm_rate = %ld\n", hz, cpm_rate);
		return -1;
	}
...
static int jz_spi_pio_polling_txrx(struct spi_device *spi, struct spi_transfer *t)
{
	struct jz_spi *hw = spi_master_get_devdata(spi->master);
	...
	before_status = spi_readl(hw, SSI_SR);
	/* fill the txfifo with CPU Mode */
	retlen = cpu_write_txfifo(hw, entries);
	if (!retlen) {
		dev_info(hw->dev,"cpu_write_txfifo error!\n");
		return -1;
	}
	/* polling cpu finish */
	while((get_rxfifo_count(hw) != entries) && timeout--){
	}
	if(timeout <= 0){
		printk("SSI_SR: before 0x%08x; after %08x\n", before_status, spi_readl(hw, SSI_SR));
		dump_spi_reg(hw);
		dev_info(hw->dev,"cpu_write_txfifo timeout! rxfifo = %d, txfifo = %d entries = %d\n", get_rxfifo_count(hw), get_txfifo_count(hw),  entries);
		ret = -1;
	}else{
...
#endif
	ret = wait_for_completion_interruptible_timeout(&hw->done_rx_dma, 200 * HZ);
	if (ret <= 0) {
		dump_spi_reg(hw);
		printk("The spi dma_callback wait timeout\n");
...
static int jz_spi_dma_txrx_new(struct spi_device *spi, struct spi_transfer *t)
{
	int ret;
	int offset = 0;
	struct jz_spi *hw = spi_master_get_devdata(spi->master);
	...
	if(t->tx_buf && t->rx_buf){
		printk("SPI Can't support synchronous transfer\n");
		return -EFAULT;
	}

	memcpy(&tmp, t, sizeof(tmp));
	disable_irq(hw->irq);
	if(!(spi_readl(hw, SSI_CR0) & CR0_SSIE)){
		disable_tx_intr(hw);
		disable_rx_intr(hw);
		...
		clear_errors(hw);
		start_transmit(hw);
		set_tx_trigger(hw, DMA_BRUST_MAX_LENGTH);
		set_rx_trigger(hw, DMA_BRUST_MAX_LENGTH);
	...
	if(t->tx_buf){
		...
		if(t->len <= 2*DMA_BRUST_MAX_LENGTH - ((unsigned int)t->tx_buf % DMA_BRUST_MAX_LENGTH)){
			ret = jz_spi_pio_polling_txrx(spi, t);
			goto end;
		}
		...
		if((unsigned int)t->tx_buf % DMA_BRUST_MAX_LENGTH){
			...
			ret = jz_spi_pio_polling_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI:Failed to transfer first data!\n");
				goto end;
			}
			offset = tmp.len;
		}
		...
		if(tmp.len){
			ret = jz_spi_dma_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI:Failed to transfer second data!\n");
				goto end;
			}
			offset += tmp.len;
		}
		...
		/* little data */
		if(t->len <= 2*DMA_BRUST_MAX_LENGTH - ((unsigned int)t->rx_buf % DMA_BRUST_MAX_LENGTH)){
			ret = jz_spi_pio_polling_txrx(spi, t);
			goto end;
		}
...
static int jz_spi_pio_txrx(struct spi_device *spi, struct spi_transfer *t)
{
	...
	ret = wait_for_completion_interruptible_timeout(&hw->done, msecs_to_jiffies(1000));
	if (ret <= 0) {
		printk("The tx wait timeout\n");
		dump_spi_reg(hw);
...
static int jz_spi_setupxfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if (hw->use_dma) {
		hw->txrx_bufs = &jz_spi_dma_txrx_new;
...
{
	clk_set_rate(hw->clk, hw->src_clk * 2);
	clk_enable(hw->clk);
...
static int __init jz_spi_probe(struct platform_device *pdev)
{
	...
	if (hw->use_dma) {
		...
		hw->buffer = dma_alloc_noncoherent(&pdev->dev, BUFFER_SIZE,
