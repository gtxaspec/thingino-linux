...
//    printk("%s[%d]: \n",__func__,__LINE__);
	...
	ingspi->dma_tx_unit = t->len / ingspi->transfer_unit_size;
	ingspi->dma_rx_unit = t->len / ingspi->transfer_unit_size;
	/* set tx dma trigger */
	for (i = 0; i < ARRAY_SIZE(dma_ds); i++) {
		if ((t->len % (dma_ds[i] * tx_config.dst_addr_width)) == 0)
			break;
	}

	if (i < ARRAY_SIZE(dma_ds)) {
		ingspi->tx_trigger = dma_ds[i];
		ingspi->rx_trigger = dma_ds[i];
	} else {
		print_dbg("DMA tx block_size force to defaut set!!!");
		ingspi->tx_trigger = INGENIC_SSI_DMA_BURST_LENGTH;
		ingspi->rx_trigger = INGENIC_SSI_DMA_BURST_LENGTH;
	}

	tx_config.dst_maxburst = ingspi->tx_trigger;
	tx_config.src_maxburst = ingspi->tx_trigger;
	rx_config.src_maxburst = ingspi->rx_trigger;
	rx_config.dst_maxburst = ingspi->rx_trigger;
...
//	enable_receive(ingspi);
	...
	if (!(ingspi->rw_mode & R_MODE)) {
		...
		print_dbg("t->len: %d, ingspi->dma_tx_unit = %d, tx fifo width: %d, set tx trigger value to %d\n",
										t->len, ingspi->dma_tx_unit, ingspi->txfifo_width, ingspi->tx_trigger);
		/*dump_spi_reg(ingspi);*/
		enable_receive(ingspi);
		ret = wait_for_completion_interruptible_timeout(&ingspi->done_tx_dma, 60 * HZ);
		if (ret <= 0) {
	...
	/* */
	spi_surplus_enable(ingspi);
	spi_surplus_setcnt(ingspi, ingspi->dma_rx_unit, ingspi->rx_trigger);
	spi_surplus_loadcnt(ingspi);
	...
	print_dbg("t->len: %d, ingspi->dma_rx_unit = %d, rx fifo width: %d, set rx trigger value to %d\n",
										t->len, ingspi->dma_rx_unit, ingspi->rxfifo_width, ingspi->rx_trigger);
	/*dump_spi_reg(ingspi);*/
	enable_receive(ingspi);
...
static int ingenic_spi_probe(struct platform_device *pdev)
{
    ...
    printk("%s[%d]: ingspi->iomem = %p\n",__func__,__LINE__,ingspi->iomem);
