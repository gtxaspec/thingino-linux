...
static void ingenic_spi_chipsel(struct spi_device *spi, int value)
{
	...
	spi->mode = ingspi->pdata->board_info->mode;
...
static int ingenic_spi_clk_set_rate(struct spi_device *spi, unsigned long rate)
{
	...
	if(ingspi->set_clk_flag == 1)
		return 0;

	cur_rate = ingenic_spi_clk_get_rate(spi);
	if(cur_rate == rate){
		ingspi->set_clk_flag = 1;
		return 0;
	}
	spin_lock(&ingspi->lock);
...
extern void dump_dma_desc_test(struct dma_chan *chan);
...
#define DMA_BRUST_MAX_LENGTH 16
static int ingenic_spi_pio_polling_txrx(struct spi_device *spi, struct spi_transfer *t)
{
	struct ingenic_spi *ingspi = spi_master_get_devdata(spi->master);
	struct ingenic_intr_cnt *g_ingenic_intr = ingspi->g_ingenic_intr;
	u32 entries, unit_size, retlen;
	int ret = 0, timeout = 100000;
	unsigned int before_status = 0;
	...
	if (unit_size == SPI_8BITS)
		...
		entries = ingspi->len >> 1;
	else if (unit_size == SPI_32BITS )
		entries = ingspi->len >> 2;
	else {
	...
	flush_rxfifo(ingspi);
	before_status = spi_readl(ingspi, SSI_SR);
	/* fill the txfifo with CPU Mode */
	retlen = cpu_write_txfifo(ingspi, entries);
	if (!retlen) {
		dev_info(ingspi->dev,"cpu_write_txfifo error!\n");
		return -1;
	}
	/* polling cpu finish */
	while((get_rxfifo_count(ingspi) != entries) && timeout--){
	}
	if(timeout <= 0){
		printk("SSI_SR: before 0x%08x; after %08x\n", before_status, spi_readl(ingspi, SSI_SR));
		dump_spi_reg(ingspi);
		dev_info(ingspi->dev,"cpu_write_txfifo timeout! rxfifo = %d, txfifo = %d entries = %d\n", get_rxfifo_count(ingspi), get_txfifo_count(ingspi),  entries);
		ret = -1;
	}else{
...
static int ingenic_spi_dma_txrx(struct spi_device *spi, struct spi_transfer *t)
{
	...
	/* all transfer start whth tx , end whit rx ??? */
	...
	if(i < ARRAY_SIZE(dma_ds)){
		...
		//printk("DMA tx block_size force to defaut set!!!");
	...
	if(dma_map_sg(ingspi->txchan->device->dev, ingspi->sg_tx, 1, DMA_TO_DEVICE) != 1) {
		dev_err(&spi->dev, "dma_map_sg tx error\n");
		printk("%s LINE %d: %s\n", __func__, __LINE__, __FILE__);
		goto err_tx_sgmap;
	}

	txdesc = txchan->device->device_prep_slave_sg(txchan, ingspi->sg_tx, 1, DMA_TO_DEVICE, DMA_PREP_INTERRUPT | DMA_CTRL_ACK, NULL);
	if(!txdesc) {
...
//		enable_tx_error_intr(ingspi);
		...
		ret = wait_for_completion_interruptible_timeout(&ingspi->done_tx_dma, 200 * HZ);
		if(ret <= 0){
	...
	if(dma_map_sg(ingspi->rxchan->device->dev, ingspi->sg_rx, 1, DMA_FROM_DEVICE) != 1) {
		dev_err(&spi->dev, "dma_map_sg rx error\n");
		goto err_rx_sgmap;
	}

	rxdesc = rxchan->device->device_prep_slave_sg(rxchan, ingspi->sg_rx, 1, DMA_FROM_DEVICE, DMA_PREP_INTERRUPT | DMA_CTRL_ACK, NULL);
	if(!rxdesc) {
		dev_err(&spi->dev, "device_prep_slave_sg error\n");
		printk("%s LINE %d: %s\n", __func__, __LINE__, __FILE__);
		goto err_rxdesc;
...
//	enable_tx_error_intr(ingspi);
	...
	if(ret <= 0) {
		dump_spi_reg(ingspi);
		dump_dma_desc_test(txchan);
		dump_dma_desc_test(rxchan);
		printk("SPI:ERROR The spi dma_callback wait timeout\n");
		printk("The length of data is %d\n", t->len);
		goto err_rxdesc;
	}
...
static int ingenic_spi_dma_txrx_new(struct spi_device *spi, struct spi_transfer *t)
{
	int ret, offset = 0;
	struct ingenic_spi *ingspi = spi_master_get_devdata(spi->master);
	struct dma_chan *rxchan = ingspi->rxchan;
	...
	disable_irq(ingspi->irq);
	if(!(spi_readl(ingspi, SSI_CR0) & CR0_SSIE)){
		disable_tx_intr(ingspi);
		disable_rx_intr(ingspi);
		...
		clear_errors(ingspi);
		start_transmit(ingspi);
		set_tx_trigger(ingspi, DMA_BRUST_MAX_LENGTH);
		set_rx_trigger(ingspi, DMA_BRUST_MAX_LENGTH);
	...
	if(t->tx_buf){
		/* little data */
		if(t->len < 2 * DMA_BRUST_MAX_LENGTH - ((unsigned int)t->tx_buf % DMA_BRUST_MAX_LENGTH)){
//			printk("0, pio mode tx_len = %d thre = %d \n", t->len, (2 * DMA_BRUST_MAX_LENGTH - ((unsigned int)t->tx_buf % DMA_BRUST_MAX_LENGTH)));
			ret = ingenic_spi_pio_polling_txrx(spi, t);
			goto end;
		}

		/*not aligend*/
		if((unsigned int)t->tx_buf % DMA_BRUST_MAX_LENGTH){
			tmp.tx_buf = t->tx_buf;
			tmp.len = DMA_BRUST_MAX_LENGTH - ((unsigned int )t->tx_buf % DMA_BRUST_MAX_LENGTH);
//			printk("1, tx_buf = %p tx_len = %d\n",tmp.tx_buf, tmp.len);
			ret = ingenic_spi_pio_polling_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI: ERROR. tx buf transfer first data failed !\n");
				goto end;
			}
			offset = tmp.len;
		}
		...
		/* aligned */
		tmp.tx_buf = t->tx_buf + offset;
		tmp.len = (t->len - offset) / DMA_BRUST_MAX_LENGTH * DMA_BRUST_MAX_LENGTH;
		if(tmp.len){
//			printk("2, tx_len = %d t->len = %d  offset = %d \n", tmp.len, t->len, offset);
			ret = ingenic_spi_dma_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI: ERROR. tx buf aligned data failed !\n");
				goto end;
			}
			offset += tmp.len;
		}
...
//			printk("3, tx_len = %d\n", tmp.len);
			ret = ingenic_spi_pio_polling_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI: ERROR. tx buf transfer tail data failed !\n");
				goto end;
			}
		}
	}else{
		/* spi rx transfer */
		/* little data */
		if(t->len < DMA_BRUST_MAX_LENGTH - ((unsigned int)t->len % DMA_BRUST_MAX_LENGTH)){
			ret = ingenic_spi_pio_polling_txrx(spi, t);
			goto end;
		}

		/* not aligned */
		if((unsigned int)t->rx_buf % DMA_BRUST_MAX_LENGTH){
			tmp.rx_buf = t->rx_buf;
			tmp.len = DMA_BRUST_MAX_LENGTH - ((unsigned int)t->rx_buf % DMA_BRUST_MAX_LENGTH);
//			printk("1,rx_buf = %p rx_len = %d\n",tmp.rx_buf, tmp.len);
			ret = ingenic_spi_pio_polling_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI: ERROR. rx buf transfer first data failed !\n");
				goto end;
			}
			offset = tmp.len;
		}
		...
		/* aligned data */
		tmp.rx_buf = t->rx_buf + offset;
		tmp.len = (t->len - offset) / DMA_BRUST_MAX_LENGTH * DMA_BRUST_MAX_LENGTH;
		if(tmp.len){
//			printk("2, rx_len = %d\n", tmp.len);
			ret = ingenic_spi_dma_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI: ERROR. rx buf aligned data failed !\n");
				goto end;
			}
			offset += tmp.len;
		}
...
//			printk("3, rx_len = %d\n", tmp.len);
			ret = ingenic_spi_pio_polling_txrx(spi, &tmp);
			if(ret != tmp.len){
				printk("SPI: ERROR. rx buf transfer tail data failed !\n");
				goto end;
			}
		}
	}
...
static int ingenic_spi_setupxfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if (ingspi->use_dma) {
		ingspi->txrx_bufs = &ingenic_spi_dma_txrx_new;
...
static struct ingenic_spi_info *ingenic_spi_parse_dt(struct ingenic_spi *ingspi)
{
	...
	struct spi_board_info *board_info = devm_kzalloc(dev, sizeof(*board_info), GFP_KERNEL);
	if(!board_info){
		printk("[%d] board info devm_kzalloc failed \n", __LINE__);
	...
	if (of_property_read_u32(dev->of_node, "mode", &value)) {
		dev_warn(dev, "device mode not specified\n");
		board_info->mode = 0;
	} else {
		board_info->mode = 0xB;
	}
	isi->board_info = board_info;
	/*printk("%s[%d]: isi->board_info->mode = %x\n",__func__,__LINE__, isi->board_info->mode);*/
...
static int ingenic_spi_configure_dma(struct ingenic_spi *ingspi)
{
	...
	ingspi->buffer = dma_alloc_noncoherent(dev, BUFFER_SIZE,
...
static int ingenic_spi_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "INGENIC SSI Controller for SPI channel %d driver register. version v1.0 \n",ingspi->chnl);
	return 0;
