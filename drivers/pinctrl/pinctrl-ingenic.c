...
/**************************************************************
 * Set GPIO Function
 *  func:       PA06-PA19 can set DS3,DS2,DS1,DS0, other only set DS1,DS0
 *	chip:		gpio chip
 *	pins:		pins bitmap
		...
		u32 pin, u16 strength)
{
    if((0 == chip->idx) && (pin<=19)) {
        ...
        if(strength & BIT(3))
		    ingenic_gpio_writel(chip, PxPDS3S, BIT(pin));
	    else
		    ingenic_gpio_writel(chip, PxPDS3C, BIT(pin));
    } else {
	    if(strength > 3)
		    strength = 3;
		...
		u32 pin)
{
    ...
    if((0 == chip->idx) && (pin<=19)) {
	    ...
	    strength |= (ingenic_gpio_readl(chip, PxPDS3) & BIT(pin)) ? (1 << 3) : 0;
    } else {
	    strength |= (ingenic_gpio_readl(chip, PxPDS0) & BIT(pin)) ? (1 << 0) : 0;
	    strength |= (ingenic_gpio_readl(chip, PxPDS1) & BIT(pin)) ? (1 << 1) : 0;
    }
