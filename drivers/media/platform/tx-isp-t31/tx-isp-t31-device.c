...
#include "../../../../drivers/tx-isp-t31/include/fast_start_common.h"
...
unsigned long tp_fv_thr = 0;
EXPORT_SYMBOL(tp_fv_thr);
...
long tp_explosion_threshold = 255;
EXPORT_SYMBOL(tp_explosion_threshold);
...
static int __init ir_switch_parse(char *str)
{
	...
	p = strstr(str_t, "fv_thr=");
	if(p != NULL) {
		tp_fv_thr = simple_strtoul(p+strlen("fv_thr="), NULL, 10);
	}
...
int read_iq(void)
{
	...
	snprintf(file_name, sizeof(file_name), "/etc/sensor/%s-t31.bin",get_sensor_name());
...
static int __init tx_isp_t31_init(void)
{
...
static void __exit tx_isp_t31_exit(void)
{
	tx_isp_exit();
}

#ifdef CONFIG_BUILT_IN_SENSOR_SETTING
subsys_initcall(tx_isp_t31_init);
#else
module_init(tx_isp_t31_init);
#endif
module_exit(tx_isp_t31_exit);
