...
#include "../../../arch/mips/xburst2/lib/isp-t41/include_double/fast_start_common.h"
...
#define SENSOR1_LOADADDR             (0x3400000)
#define DECOMPRESS_SENSOR_LOADADDR  (0x3600000)
#define DECOMPRESS_SENSOR1_LOADADDR  (0x3700000)
#define SENSOR_SETTING_ADDR		    (DECOMPRESS_SENSOR_LOADADDR)
#define SENSOR1_SETTING_ADDR		    (DECOMPRESS_SENSOR1_LOADADDR)

#define AE_TABLE_OFFSET             (1*1024*4 * 5)
#define AE_TABLE_ADDR               (0x3100000 + AE_TABLE_OFFSET)

#define AE1_TABLE_OFFSET            (1*1024*4 * 6)
#define AE1_TABLE_ADDR              (0x3100000 + AE1_TABLE_OFFSET)
...
static void *g_setting1_info = NULL;
static void *g_setting_info_for_etc = NULL;
static void *g_setting1_info_for_etc = NULL;
static void *g_setting_data = NULL;
static void *g_setting1_data = NULL;
void *setting_mem = NULL;
void *setting1_mem = NULL;
void *setting_info = NULL;
void *setting1_info = NULL;
...
int read_iq(void)
{
       ...
       if(sensor_number == 2){
           snprintf(file1_name, sizeof(file1_name), "/etc/sensor/%s-t41.bin",get_sensor1_name());
	   }
       ...
       if(sensor_number == 2){
           fd1 = filp_open(file1_name, O_CREAT | O_WRONLY | O_TRUNC, 00766);
           if (fd1 < 0) {
               printk("Failed to open %s\n",file1_name);
               return -1;;
           }
       ...
       if(sensor_number == 2){
       		g_setting1_info_for_etc = get_sensor1_setting();
	   }
       old_fs = get_fs();
       set_fs(KERNEL_DS);
       pos = &(fd->f_pos);
       ...
       if(sensor_number == 2){
       		...
       		set_fs(KERNEL_DS);
       		pos = &(fd1->f_pos);
       		vfs_write(fd1,g_setting1_info_for_etc , get_sensor1_setting_len(), pos);
       		filp_close(fd1, NULL);
       		set_fs(old_fs);
	}
}
...
int sensor1_setting_init(void)
{
    ...
    setting_info = (void *)ioremap(SENSOR1_LOADADDR, SENSOR_SETTING_INFO_SIZE);
    if(setting_info == NULL) {
        printk("sensor setting  info mmap failed\n");
        return -2;
    }

    printk("Calibration1 ADDR = %p\n", setting_mem);

    g_setting1_data = vmalloc(SENSOR_SETTING_MAX_SIZE);
    if(g_setting1_data == NULL) {
        printk("sensor setting malloc failed\n");
        return -3;
    }

    g_setting1_info = vmalloc(sizeof(struct sensor_setting_info));
    if(g_setting1_info == NULL) {
        printk("sensor1 setting malloc failed\n");
        return -3;
    }
    memcpy(g_setting1_data, setting_mem, SENSOR_SETTING_MAX_SIZE);
    memcpy(g_setting1_info, setting_info, SENSOR_SETTING_INFO_SIZE);
    return 0;
}
...
int get_sensor1_setting_len(void)
{
    ...
    info = (struct sensor_setting_info *)(g_setting1_info);
    printk("Calibration len = %d\n", info->len);
    if(info->len > SENSOR_SETTING_MAX_SIZE)
        return SENSOR_SETTING_MAX_SIZE;
    return info->len;
