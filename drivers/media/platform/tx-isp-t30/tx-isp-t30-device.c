...
static int __init tx_isp_t30_init(void)
{
	return tx_isp_init();
}

static void __exit tx_isp_t30_exit(void)
{
	tx_isp_exit();
}

#ifdef CONFIG_BUILT_IN_SENSOR_SETTING
subsys_initcall(tx_isp_t30_init);
#else
module_init(tx_isp_t30_init);
#endif
module_exit(tx_isp_t30_exit);
