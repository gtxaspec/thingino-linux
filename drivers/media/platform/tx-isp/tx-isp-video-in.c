...
static struct tx_isp_video_in_device *g_vi = NULL;
static int is_sensor_register = 0;
...
static long subdev_core_ops_register_sensor(struct tx_isp_video_in_device *vi, void *arg)
{
	...
	if(++is_sensor_register == 2) {
		if(strcmp(info->name, get_sensor_name()) == 0) {
			return ISP_SUCCESS;
		}
	}
...
int start_tx_isp_video_in_device()
{
	int ret = 0;

	if(g_vi == NULL) {
		printk("%s g_vi is NULL\n", __func__);
		return -1;
	}

	struct v4l2_tx_isp_sensor_register_info sensor_register_info;
	memset(&sensor_register_info, 0, sizeof(struct v4l2_tx_isp_sensor_register_info));
	...
	struct tx_isp_notify_argument arg;
	atomic_set(&g_vi->state, TX_ISP_STATE_START);
	g_vi->sd.v4l2_dev->notify(&g_vi->sd, TX_ISP_NOTIFY_GET_PIPELINE, &arg);
	if(arg.ret != ISP_SUCCESS)
		return -ISP_ERROR;
	g_vi->p = (struct tx_isp_media_pipeline *)arg.value;

	ret = subdev_core_ops_register_sensor(g_vi, &sensor_register_info);
	if(ret < 0) {
		printk("subdev_core_ops_register_sensor error\n");
		return ret;
	}
	...
	while(1) {
		...
		ret = subdev_core_ops_enum_input(g_vi, &input);
		if(ret < 0) {
			printk("subdev_core_ops_enum_input end\n");
			break;
		}

		if(strcmp(sensor_register_info.name, input.name) == 0) {
			break;
		}
	}

	ret = subdev_core_ops_set_input(g_vi, &input.index);
	if(ret < 0) {
		printk("subdev_core_ops_set_input error\n");
		return ret;
	}

	int index = -1;
	ret = subdev_core_ops_get_input(g_vi, &index);
	if(ret < 0) {
		printk("subdev_core_ops_get_input error\n");
		return ret;
	}

	if(index == -1) {
		printk("sensor hasn't been added!");
		return -1;
	}

	ret = subdev_core_ops_streamon(g_vi, 1);
	if(ret < 0) {
		printk("subdev_core_ops_streamon error\n");
		return ret;
	}
