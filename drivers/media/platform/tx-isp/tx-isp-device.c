...
static const struct proc_ops zrt_proc_fops = {
	.proc_ioctl = zeratul_proc_ioctl,
};
static const struct proc_ops zrt_variable_proc_fops = {
	.proc_ioctl = zeratul_variable_proc_ioctl,
};
...
static int zeratul_proc_create(void)
{
	...
	if (!p) {
		printk("[%s %d]create_proc_entry for common reset failed.\n",__func__,__LINE__);
		return -ENODEV;
	}
...
int __init fast_dma_declare_contiguous(phys_addr_t base, phys_addr_t size)
{
...
{
	int ret = 0;
	ret = free_reserved_area(__va(base), __va(base + size), -1, "IQ_DMA");
	if(ret){
...
int sensor_setting_init(void)
{
	...
	/* printk("===head_remap sensor_cnt:%d\n",(struct sensor_calibration_head *)head_remap->info.sensor_cnt); */
	//TD not free
...
static const struct proc_ops iq_data_proc_fops ={
	.proc_read = iq_addr_fops_read,
};
...
int32_t get_sensor_section_num(int sensor_index, int iq_index)
{
	...
	return num;
failed_head:
	printk("[%s %d] get sensor%d iq%d failed\n",__func__,__LINE__,sensor_index,iq_index);
	return num;
}
/*
 *
...
static int __init tx_isp_PRJ008_init(void)
{
	...
	if (sensor_setting_fw == -1 || riscv_fw == -1) {
		printk("tx-isp-PRJ not init(%d,%d)\n",sensor_setting_fw, riscv_fw);
	}else{
		retval = tx_isp_init();
	}
#ifdef CONFIG_BLK_DEV_INITRD
