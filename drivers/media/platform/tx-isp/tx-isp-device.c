...

	{
		...
		for (i = 0; i < 12; i++) {
			if (i == 0) {
				...
				snprintf(key, sizeof(key), "init_vw_ch%d=", i);
				p = strstr(str_t, key);
				if (p != NULL)
					frame_channel_width[i] = simple_strtoul(p + strlen(key), NULL, 10);
				snprintf(key, sizeof(key), "init_vh_ch%d=", i);
				p = strstr(str_t, key);
				if (p != NULL)
					frame_channel_height[i] = simple_strtoul(p + strlen(key), NULL, 10);
				snprintf(key, sizeof(key), "nrvbs_ch%d=", i);
				p = strstr(str_t, key);
				if (p != NULL)
					frame_channel_nrvbs[i] = simple_strtoul(p + strlen(key), NULL, 10);
			}
