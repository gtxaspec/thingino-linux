...
#if FIXED_INTERVAL_VIC_RESET
static int frame_num = 0;
static int vic_reset()
{
	frame_num++;

	if(frame_num % 40 == 0) {
		unsigned int vic_tmp = *(volatile unsigned int *)0xB3300000;
		vic_tmp |= (0x1 << 4);
		*(volatile unsigned int *)0xB3300000 = vic_tmp;
	}
	return 0;
}
#endif

#if TX_ISP_FAST_START && TX_ISP_NRVBS_EN && TX_ISP_FRAME_BUFFER_IGNORE_CHOICE
extern int tx_isp_frame_channel_fast_qbuf(void);
#endif
...
#if TX_ISP_FAST_START && TX_ISP_NRVBS_EN && TX_ISP_FRAME_BUFFER_IGNORE_CHOICE
						ret = tx_isp_frame_channel_fast_qbuf();
						if(ret >= 0) {
							if(chan->dma_state != 1){
								isp_enable_channel(chan);
							}
