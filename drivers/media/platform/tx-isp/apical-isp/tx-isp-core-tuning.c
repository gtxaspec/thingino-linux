...
static image_tuning_vdrv_t *g_tuning = NULL;
extern int get_z_value_at_night_mode(void);
#endif
...
#if TX_ISP_FAST_START
	if(dn == ISP_CORE_RUNING_MODE_NIGHT_MODE){
		get_z_value_at_night_mode();
	}
#endif
...
static inline int apical_isp_day_or_night_s_ctrl(struct tx_isp_core_device *core, struct v4l2_control *control, int mode)
{
	struct video_device *video = core->tun;
	TXispPrivParamManage *param = core->param;
	image_tuning_vdrv_t *tuning = video_get_drvdata(video);
...
static inline int apical_isp_fps_s_control(struct tx_isp_core_device *core, struct v4l2_control *control, int mode)
{
	struct video_device *video = core->tun;
	image_tuning_vdrv_t *tuning = video_get_drvdata(video);
...
static int apical_isp_ae_coeff_g_ctrl(struct tx_isp_core_device *core, struct v4l2_control *control)
{
	unsigned char status = 0;
	struct video_device *video = core->tun;
	TXispPrivParamManage *param = core->param;
...
static int apical_isp_core_ops_g_ctrl(struct tx_isp_core_device *core, struct v4l2_control *ctrl)
{
	...
	switch(ctrl->id){
		...
		case IMAGE_TUNING_CID_HILIGHT_DEPRESS_STRENGTH:
			...
			break;
		case IMAGE_TUNING_CID_AE_COEFF:
			ret = apical_isp_ae_coeff_g_ctrl(core, ctrl);
			break;
...
static int apical_isp_core_ops_s_ctrl(struct tx_isp_core_device *core, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) {
		...
		case IMAGE_TUNING_CID_FC_ATTR:
			...
			break;
		case IMAGE_TUNING_CID_CONTROL_FPS:
			ret = apical_isp_fps_s_control(core, ctrl, 1);
			break;
		case IMAGE_TUNING_CID_DAY_OR_NIGHT:
			ret = apical_isp_day_or_night_s_ctrl(core, ctrl, 1);
			break;
		case IMAGE_TUNING_CID_HVFLIP:
...
int isp_core_tuning_switch_day_or_night(int day)
{
	...
	struct tx_isp_core_device *core = sd_to_tx_isp_core_device(g_tuning->parent);
	struct v4l2_control control = {0};

	if(day == 0) {
		control.value = SENSOR_NIGHT_FPS << 16 | 1;

		ret = apical_isp_fps_s_control(core, &control, 0);
		if(ret < 0) {
			printk("%s apical_isp_fps_s_control failed\n", __func__);
			return -1;
		}
	}

	control.value = day ? ISP_CORE_RUNING_MODE_DAY_MODE : ISP_CORE_RUNING_MODE_NIGHT_MODE;
	ret = apical_isp_day_or_night_s_ctrl(core, &control, 0);
	if(ret < 0) {
		printk("%s apical_isp_day_or_night_s_ctrl failed\n", __func__);
		return -1;
	}
