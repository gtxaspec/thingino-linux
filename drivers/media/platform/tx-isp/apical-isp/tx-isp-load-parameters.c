...
#ifndef CONFIG_BUILT_IN_SENSOR_SETTING
	/* open file */
...
#else
	if(manager->data == NULL){
		manager->data = kmalloc(get_sensor_setting_len(), GFP_KERNEL);
		if(manager->data == NULL){
			printk("%s[%d]: Failed to alloc %lld KB buffer!\n",__func__,__LINE__, fsize >> 10);
			ret = -1;
			goto failed_malloc_data;
		}
		manager->data_size = get_sensor_setting_len();
	}
	printk("Load Sensor Setting DATE:%s MD5:%s\n", get_sensor_setting_date(), get_sensor_setting_md5());
	memcpy(manager->data, get_sensor_setting(), get_sensor_setting_len());
#endif
