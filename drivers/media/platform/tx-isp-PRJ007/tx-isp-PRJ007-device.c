...
#include "tx-isp-PRJ007-device.h"
#include <jz_proc.h>
...
#define ZRT_IOCTL_READ	_IOR(ZRT_IOCTL_TYPE, 1, int)
//#define ZRT_IOCTL_WRITE _IOW(ZRT_IOCTL_TYPE, 2, int)
...
int sensor_iq_bin_init(int s, int it, int st)
{
	...
	if(iq_ft->sections_cnt > 0){
		iq_paddr = iq_ft->vmap.section[st].offset + SENSOR_SETTING_ADDR;
		iq_size = iq_ft->vmap.section[st].size;
		iq_remap = ioremap(iq_paddr, iq_size);
		//malloc
		iq_tx_isp->iq.section[st].vaddr = vmalloc(iq_size);
		//copy
		memcpy(iq_tx_isp->iq.section[st].vaddr,
		       iq_remap,
		       iq_size);
...
int sensor_setting_init(void)
{
	...
	head_remap = ioremap(SENSOR_ATTR_LOADADDR, SENSOR_CALIBRATION_HEAD_SIZE);
	//TD not free
...
static int __init tx_isp_PRJ007_init(void)
{
	int retval = 0;
	if (sensor_setting_fw == -1 || riscv_fw == -1) {
		printk("tx-isp-PRJ007 not init(%d,%d)\n",sensor_setting_fw, riscv_fw);
	}else{
		retval = tx_isp_init();
	}
#ifdef CONFIG_BLK_DEV_INITRD
...
static void __exit tx_isp_PRJ007_exit(void)
{
	tx_isp_exit();
#ifdef CONFIG_ZERATUL
...
#ifdef CONFIG_ZERATUL
#ifdef CONFIG_BUILT_IN_SENSOR_SETTING
subsys_initcall(tx_isp_PRJ007_init);
#else
module_init(tx_isp_PRJ007_init);
#endif/*CONFIG_BUILT_IN_SENSOR_SETTING*/

module_exit(tx_isp_PRJ007_exit);
#endif/*CONFIG_ZERATUL*/
