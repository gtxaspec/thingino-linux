...
void dump_dma_desc_test(struct dma_chan *chan)
{
	...
	struct dma_desc *desc = dmac->desc;
	if(dmac->sg_len){
		printk("DMA chan id = %d\n", dmac->id);
		printk("DMA Command 0x%08x\n", readl(dmac->iomem + CH_DCM));
		printk("DMA cnt %d\n", readl(dmac->iomem + CH_DTC));
		printk("DMA dst begin address 0x%08x\n", desc[0].dta);
		printk("DMA dst current address 0x%08x\n", readl(dmac->iomem + CH_DTA));
		printk("DMA src begin address 0x%08x\n", desc[0].dsa);
		printk("DMA src current address 0x%08x\n", readl(dmac->iomem + CH_DSA));
	}
}
			 ...
			 unsigned long arg)
{
	...
	case DMA_SLAVE_CONFIG:
		...
		case DMA_SLAVE_BUSWIDTH_1_BYTE:
			dmac->tx_dcm_def = DCM_DP_8;
			dmac->rx_dcm_def = DCM_DP_8;
			break;
		case DMA_SLAVE_BUSWIDTH_2_BYTES:
			dmac->tx_dcm_def = DCM_DP_16;
			dmac->rx_dcm_def = DCM_DP_16;
			break;
		...
		case DMA_SLAVE_BUSWIDTH_1_BYTE:
			dmac->tx_dcm_def |= DCM_SP_8;
			dmac->rx_dcm_def |= DCM_SP_8;
			break;
		case DMA_SLAVE_BUSWIDTH_2_BYTES:
			dmac->rx_dcm_def |= DCM_SP_16;
			dmac->tx_dcm_def |= DCM_SP_16;
			break;
		case DMA_SLAVE_BUSWIDTH_4_BYTES:
