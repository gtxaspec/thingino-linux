			 ...
			 unsigned long arg)
{
	...
	case DMA_SLAVE_CONFIG:
		...
		if (ret) {
			dev_warn(chan2dev(chan), "Bad DMA control argument [%d]!",__LINE__);
			ret = -EINVAL;
			break;
		}
...
static int __init jzdma_probe(struct platform_device *pdev)
{
	...
	dma->clk = clk_get(&pdev->dev, "gate_pdma");
	if (IS_ERR(dma->clk)) {
		goto release_mem;
	}
...
static int jzdma_resume(struct platform_device * pdev)
{
	...
	dma->clk = clk_get(&pdev->dev, "gate_pdma");
	clk_enable(dma->clk);
	ret = request_irq(dma->irq, jzdma_int_handler, IRQF_DISABLED,"pdma", dma);
	if (ret)
        printk("jzdma request_irq is error\n");
	writel(1 | (0x3f << 16), dma->iomem + DMAC);
	writel(pdma_program, dma->iomem + DMACP);
	jzdma_mcu_reset(dma);
	return 0;
