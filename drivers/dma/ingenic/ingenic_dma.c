...
dma_addr_t ingenic_get_dma_current_trans_addr(struct dma_chan *chan,enum dma_transfer_direction direction)
{
    ...
    //printk("dma_chan id:%d ret_val:0x%px \n", dmac->id, ret_val);
    return ret_val;
}
EXPORT_SYMBOL_GPL(ingenic_get_dma_current_trans_addr);
...
#define ADDR_ALIGN_DOWN(addr, size)  ((addr)&(~((size)-1)))
		...
		unsigned int length, struct hdma_desc *desc)
{
	...
	if (length < DTC_TC_MSK) {
		...
		len = ADDR_ALIGN_DOWN(len, sizeof(uint32_t));
...
//	unsigned int tsz, transfer_shift;
	...
	if (WARN_ON(!IS_ALIGNED(length, dmac->transfer_width))) {
		desc->dtc = ADDR_ALIGN_DOWN((length & DTC_TC_MSK), dmac->transfer_width);
	} else {
...
static int ingenic_dma_terminate_all(struct dma_chan *chan)
{
	...
	if(dmac->slave_id == INGENIC_DMA_REQ_AEC || dmac->slave_id == INGENIC_DMA_REQ_I2S0_TX
			|| dmac->slave_id == INGENIC_DMA_REQ_I2S0_RX)
	{
...
static const struct of_device_id ingenic_dma_dt_match[] = {
	...
	{ .compatible = "ingenic,PRJ007-pdma", .data = (void *)HWATTR_INTC_IRQ},
	{ .compatible = "ingenic,PRJ008-pdma", .data = (void *)HWATTR_INTC_IRQ},
	{},
};
