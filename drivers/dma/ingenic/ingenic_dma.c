...
//	unsigned int tsz, transfer_shift;
	...
	if(desc->drt == 0x16 || desc->drt == 0x17 || desc->drt == 0x18 || desc->drt == 0x19)
		rdil = 1;
...
void dump_dma_desc_test(struct dma_chan *chan)
{
	struct ingenic_dma_chan *dmac = to_ingenic_dma_chan(chan);
	struct hdma_desc *desc = dmac->sdesc;
	unsigned int dma_base =  0xb3420000;

	printk("DMA chan id = %d  . base = %x \n", dmac->id, dma_base + (0x20 * dmac->id));
	printk("==>DMA REG = 0x00 . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DSA));
	printk("==>DMA REG = 0x04 . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DTA));
	printk("==>DMA REG = 0x08 . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DTC));
	printk("==>DMA REG = 0x0c . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DRT));
	printk("==>DMA REG = 0x10 . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DCS));
	printk("==>DMA REG = 0x14 . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DCM));
	printk("==>DMA REG = 0x18 . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DDA));
	printk("==>DMA REG = 0x1c . val = %x \n", readl(dma_base + (0x20 * dmac->id) + CH_DSD));

	printk("==>DMA REG = 0x1000 . val = %x \n", readl(dma_base + DMAC));
	printk("==>DMA REG = 0x1004 . val = %x \n", readl(dma_base + DIRQP));
	printk("==>DMA REG = 0x1008 . val = %x \n", readl(dma_base + DDR));
	printk("==>DMA REG = 0x100c . val = %x \n", readl(dma_base + DDRS));
	printk("\n");
}
EXPORT_SYMBOL(dump_dma_desc_test);
...
//		dmac->maxburst = config->src_maxburst;
	...
	case DMA_SLAVE_BUSWIDTH_1_BYTE:
		if(dmac->slave_id == 0x16 || dmac->slave_id == 0x17 || dmac->slave_id == 0x18 || dmac->slave_id == 0x19){
			if(config->direction == DMA_MEM_TO_DEV){
				dmac->dcm = DCM_SRC4_DST1;
				dmac->transfer_width = 4;
			}else if(config->direction == DMA_DEV_TO_MEM){
				dmac->dcm = DCM_SRC1_DST4;
				dmac->transfer_width = 4;
			}
		}else {
				dmac->dcm = DCM_PORT_8;
...
static int ingenic_dma_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	writel(0, ingenic_dma->iomem + DMAC);
...
static int ingenic_dma_resume(struct platform_device * pdev)
{
	...
	writel(0x1, ingenic_dma->iomem + DMAC);
	return 0;
}
