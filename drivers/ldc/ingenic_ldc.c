...
static int set_ldc_init(struct jz_ldc *ldc, struct ldc_param *ldc_init)
{
    ...
    */
	...
	reg_write(ldc, LDC_ADDR_X_CORF, (ldc_default_params[sheet].k2_x << 16 | ldc_default_params[sheet].k1_x));
		printk("LDC_ADDR_X_CORF  = 0x%08x\n",reg_read(ldc, LDC_ADDR_X_CORF));
	...
	printk("LDC_ADDR_BROCF = 0x%08x\n",reg_read(ldc, LDC_ADDR_BROCF));
	reg_write(ldc, LDC_ADDR_R2_REP, ldc_default_params[sheet].r2_rep);
	printk("LDC_ADDR_R2_REP = 0x%08x\n",reg_read(ldc, LDC_ADDR_R2_REP));
	reg_write(ldc, LDC_ADDR_UDIS_R, 0x0);
	printk("LDC_ADDR_UDIS_R  = 0x%08x\n",reg_read(ldc, LDC_ADDR_UDIS_R));

    //set Y
    LDC_DEBUG("-->0x%x,0x%x\n",ldc_init->in_addr,ldc_init->out_addr);
    reg_write(ldc, LDC_ADDR_Y_DMAIN_ADDR, ldc_init->in_addr);
	printk("LDC_ADDR_Y_DMAIN_ADDR = 0x%08x\n",reg_read(ldc, LDC_ADDR_Y_DMAIN_ADDR));
    reg_write(ldc, LDC_ADDR_Y_DMAIN_STRIDE, ldc_default_params[sheet].y_str);
	printk("LDC_ADDR_Y_DMAIN_STRIDE  = 0x%08x\n",reg_read(ldc, LDC_ADDR_Y_DMAIN_STRIDE));
    reg_write(ldc, LDC_ADDR_Y_DMAOT_ADDR, ldc_init->out_addr);
	printk("LDC_ADDR_Y_DMAOT_ADDR  = 0x%08x\n",reg_read(ldc, LDC_ADDR_Y_DMAOT_ADDR));
    reg_write(ldc, LDC_ADDR_Y_DMAOT_STRIDE, ldc_default_params[sheet].y_str);
	printk("LDC_ADDR_Y_DMAOT_STRIDE = 0x%08x\n",reg_read(ldc, LDC_ADDR_Y_DMAOT_STRIDE));

    //set uv
    reg_write(ldc, LDC_ADDR_UV_DMAIN_ADDR, ldc_init->in_addr + offset);
	printk("LDC_ADDR_UV_DMAIN_ADDR  = 0x%08x\n",reg_read(ldc, LDC_ADDR_UV_DMAIN_ADDR));
    reg_write(ldc, LDC_ADDR_UV_DMAIN_STRIDE, ldc_default_params[sheet].uv_str);
	printk("LDC_ADDR_UV_DMAIN_STRIDE  = 0x%08x\n",reg_read(ldc, LDC_ADDR_UV_DMAIN_STRIDE));
    reg_write(ldc, LDC_ADDR_UV_DMAOT_ADDR, ldc_init->out_addr + offset);
	printk("LDC_ADDR_UV_DMAOT_ADDR  = 0x%08x\n",reg_read(ldc, LDC_ADDR_UV_DMAOT_ADDR));
    reg_write(ldc, LDC_ADDR_UV_DMAOT_STRIDE, ldc_default_params[sheet].uv_str);
	printk("LDC_ADDR_UV_DMAOT_STRIDE  = 0x%08x\n",reg_read(ldc, LDC_ADDR_UV_DMAOT_STRIDE));

    reg_write(ldc, LDC_ADDR_LEN_COEF, ldc_default_params[sheet].len_cofe);
	printk("LDC_ADDR_LEN_COEF = 0x%08x\n",reg_read(ldc, LDC_ADDR_LEN_COEF));
    return ret;
}
...
static  int set_ldc_dma_enable(struct jz_ldc *ldc, struct ldc_initarg *ldc_enable)
{
    ...
    if(ldc_enable->on){
       ...
       reg_write(ldc, LDC_ADDR_LDC_WORK_EN, 0x1);
			 printk("LDC_ADDR_LDC_WORK_EN = 0x%08x\n",reg_read(ldc,LDC_ADDR_LDC_WORK_EN));
    }else{
        ldc->streaming = 0;
...
static irqreturn_t ldc_irq_handler(int irq, void *data)
{
    ...
    */
    ...
    status = reg_read(ldc, LDC_ADDR_INT_STATE);
		printk("LDC_ADDR_INT_STATE = 0x%08x\n",status);
    mask = reg_read(ldc, LDC_ADDR_INT_MASK);	
		printk("LDC_ADDR_INT_MASK  = 0x%08x\n",mask);
...
/*    int sclk;
    ...
    */
    ...
    /*
	ldc->clk_gate = clk_get(&pdev->dev, ldc->name);
	if (IS_ERR(ldc->clk_gate)) {
		dev_err(&pdev->dev, "clk_gate get failed\n");
		ret = PTR_ERR(ldc->clk_gate);
		goto err_get_ldc_clk_gate;
	}

    ldc->clk = clk_get(&pdev->dev,"cgu_ldc");
    if (IS_ERR(ldc->clk)) {
        dev_err(&pdev->dev, "clk get failed\n");
        ret = PTR_ERR(ldc->clk);
        goto err_get_ldc_clk_cgu;
    }
    */
...
static const struct of_device_id ingenic_ldc_dt_match[] = {
	{ .compatible = "ingenic,PRJ007-ldc", .data = NULL },
	{ .compatible = "ingenic,PRJ008-ldc", .data = NULL },
	{},
};
