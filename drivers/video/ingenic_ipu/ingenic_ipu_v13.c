...
struct ipu_reg_struct jz_ipu_regs_name[] = {
	...
	{"IPU_2BIT_BIT_1", IPU_2BIT_BIT_1},
	{"IPU_2BIT_BIT_0", IPU_2BIT_BIT_0},
	{"IPU_2BIT_MASK", IPU_2BIT_MASK},
};
...
static unsigned int _hal_to_ipu_outfmt(int hal_fmt)
{
	...
	switch (hal_fmt) {
		case HAL_PIXEL_FORMAT_BGRA_8888:
			ipu_fmt = OUT_FMT_BGRA;
			break;
		case HAL_PIXEL_FORMAT_ARGB_8888:
			ipu_fmt = OUT_FMT_ARGB;
			break;
		case HAL_PIXEL_FORMAT_ARBG_8888:
			ipu_fmt = OUT_FMT_ARBG;
			break;
		case HAL_PIXEL_FORMAT_AGRB_8888:
			ipu_fmt = OUT_FMT_AGRB;
			break;
		case HAL_PIXEL_FORMAT_AGBR_8888:
			ipu_fmt = OUT_FMT_AGBR;
			break;
		case HAL_PIXEL_FORMAT_ABRG_8888:
			ipu_fmt = OUT_FMT_ABRG;
			break;
		case HAL_PIXEL_FORMAT_ABGR_8888:
			ipu_fmt = OUT_FMT_ABGR;
			break;
		case HAL_PIXEL_FORMAT_RGBA_8888:
			ipu_fmt = OUT_FMT_RGBA;
			break;
		case HAL_PIXEL_FORMAT_RBGA_8888:
			ipu_fmt = OUT_FMT_RBGA;
			break;
		case HAL_PIXEL_FORMAT_GRBA_8888:
			ipu_fmt = OUT_FMT_GRBA;
			break;
		case HAL_PIXEL_FORMAT_GBRA_8888:
			ipu_fmt = OUT_FMT_GBRA;
			break;
		case HAL_PIXEL_FORMAT_BRGA_8888:
			ipu_fmt = OUT_FMT_BRGA;
			break;
		case HAL_PIXEL_FORMAT_BGRA_5551:
...
static int _ipu_set_bg_route(struct jz_ipu *ipu, struct ipu_param *ipu_param)
{
   ...
   if (ip->cmd & IPU_CMD_OSD)
	...
	if ((outfmt == OUT_FMT_HSV) || (ip->out_fmt == OUT_FMT_HSV)) {/* out format */
		reg_val = 0x4;
	} else if ((outfmt == OUT_FMT_RGB888) || (ip->out_fmt == OUT_FMT_RGB888) || (outfmt == OUT_FMT_RGBA) || (ip->out_fmt == OUT_FMT_RGBA)) {
		reg_val = 0x40;
	} else if ((outfmt == OUT_FMT_ARGB) || (ip->out_fmt == OUT_FMT_ARGB)) {
		reg_val = 0x0;
	} else if ((outfmt == OUT_FMT_ARBG) || (ip->out_fmt == OUT_FMT_ARBG)) {
		reg_val = 0x8;
	} else if ((outfmt == OUT_FMT_AGRB) || (ip->out_fmt == OUT_FMT_AGRB)) {
		reg_val = 0x10;
	} else if ((outfmt == OUT_FMT_AGBR) || (ip->out_fmt == OUT_FMT_AGBR)) {
		reg_val = 0x18;
	} else if ((outfmt == OUT_FMT_ABRG) || (ip->out_fmt == OUT_FMT_ABRG)) {
		reg_val = 0x20;
	} else if ((outfmt == OUT_FMT_ABGR) || (ip->out_fmt == OUT_FMT_ABGR)) {
		reg_val = 0x28;
	} else if ((outfmt == OUT_FMT_RBGA) || (ip->out_fmt == OUT_FMT_RBGA)) {
		reg_val = 0x48;
	} else if ((outfmt == OUT_FMT_GRBA) || (ip->out_fmt == OUT_FMT_GRBA)) {
		reg_val = 0x50;
	} else if ((outfmt == OUT_FMT_GBRA) || (ip->out_fmt == OUT_FMT_GBRA)) {
		reg_val = 0x58;
	} else if ((outfmt == OUT_FMT_BRGA) || (ip->out_fmt == OUT_FMT_BRGA)) {
		reg_val = 0x60;
	} else if ((outfmt == OUT_FMT_BGRA) || (ip->out_fmt == OUT_FMT_BGRA)) {
		reg_val = 0x68;
	...
	*/
	...
	if ((ip->out_fmt == HAL_PIXEL_FORMAT_NV12) || (ip->out_fmt == HAL_PIXEL_FORMAT_NV21)) {/*Add NV12 or NV21*/
		...
		ipu_reg_write(ipu, IPU_OUT_V_STRIDE, srcw);
			 ...
			 || ip->out_fmt == HAL_PIXEL_FORMAT_ARGB_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_ARBG_8888 \
			 || ip->out_fmt == HAL_PIXEL_FORMAT_AGRB_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_AGBR_8888 \
			 || ip->out_fmt == HAL_PIXEL_FORMAT_ABRG_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_ABGR_8888 \
			 || ip->out_fmt == HAL_PIXEL_FORMAT_RBGA_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_GRBA_8888 \
			 || ip->out_fmt == HAL_PIXEL_FORMAT_GBRA_8888 || ip->out_fmt == HAL_PIXEL_FORMAT_BRGA_8888 ) {/* Add ARGB */
	...
	if (bgrgb) {
		switch(ip->bg_fmt) {
			...
			case HAL_PIXEL_FORMAT_BGRA_8888:
				...
				break;
			case HAL_PIXEL_FORMAT_ARBG_8888:
				reg_val |= CH_BK_ARGB_TYPE_ARBG;
				break;
			case HAL_PIXEL_FORMAT_AGRB_8888:
				reg_val |= CH_BK_ARGB_TYPE_AGRB;
				break;
			case HAL_PIXEL_FORMAT_AGBR_8888:
				reg_val |= CH_BK_ARGB_TYPE_AGBR;
				break;
			case HAL_PIXEL_FORMAT_ABRG_8888:
				reg_val |= CH_BK_ARGB_TYPE_ABRG;
				break;
			case HAL_PIXEL_FORMAT_RBGA_8888:
				reg_val |= CH_BK_ARGB_TYPE_RBGA;
				break;
			case HAL_PIXEL_FORMAT_GRBA_8888:
				reg_val |= CH_BK_ARGB_TYPE_GRBA;
				break;
			case HAL_PIXEL_FORMAT_GBRA_8888:
				reg_val |= CH_BK_ARGB_TYPE_GBRA;
				break;
			case HAL_PIXEL_FORMAT_BRGA_8888:
				reg_val |= CH_BK_ARGB_TYPE_BRGA;
				break;
			default:
				reg_val |= CH_BK_ARGB_TYPE_BGRA;
				break;
...
static int _ipu_set_osd_chx_route(struct jz_ipu *ipu, struct ipu_param *ip, int ch)
{
	...
	unsigned int bit0_fmt = 0;
	unsigned int bit1_fmt = 0;
	...
	switch(ch) {
		case 0:
			...
			bit0_fmt = ip->osd_ch0_2bit_bit0_fmt;
			bit1_fmt = ip->osd_ch0_2bit_bit1_fmt;
			bit_mask = ip->osd_ch0_2bit_mask;
			break;
		case 1:
			fmt = ip->osd_ch1_fmt;
			...
			bit0_fmt = ip->osd_ch1_2bit_bit0_fmt;
			bit1_fmt = ip->osd_ch1_2bit_bit1_fmt;
			bit_mask = ip->osd_ch1_2bit_mask;
			break;
		case 2:
			fmt = ip->osd_ch2_fmt;
			...
			bit0_fmt = ip->osd_ch2_2bit_bit0_fmt;
			bit1_fmt = ip->osd_ch2_2bit_bit1_fmt;
			bit_mask = ip->osd_ch2_2bit_mask;
			break;
		case 3:
			fmt = ip->osd_ch3_fmt;
			...
			bit0_fmt = ip->osd_ch3_2bit_bit0_fmt;
			bit1_fmt = ip->osd_ch3_2bit_bit1_fmt;
			bit_mask = ip->osd_ch3_2bit_mask;
			break;
		default :
			printk("ipu: osd channel num err ch = %d\n", ch);
			return -1;
	...
	} else if (isrgb == 3){		   //The format is 2BIT
		ipu_reg_write(ipu, IPU_OSD_IN_CH0_Y_STRIDE+0x10*ch, src_stride/4);
	} else {
	...
	if (isrgb == 3) {
		ipu_reg_write(ipu, IPU_2BIT_BIT_1,bit1_fmt);
		ipu_reg_write(ipu, IPU_2BIT_BIT_0,bit0_fmt);
		ipu_reg_write(ipu, IPU_2BIT_MASK,bit_mask);
	}
...
long ipu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if(filp != NULL){
		...
		ipu = container_of(dev, struct jz_ipu, misc_dev);
	}else
		ipu = g_jz_ipu;
	...
	switch (cmd) {
		case IOCTL_IPU_START:
			if(filp != NULL){
				...
				memcpy(&iparam, (void *)arg, sizeof(struct ipu_param));
			}
		...
		case IOCTL_IPU_BUF_FLUSH_CACHE:
			{
				...
				dma_cache_sync(NULL, fc.addr, fc.size, DMA_TO_DEVICE);
			}
			break;
		default:
...
static const struct of_device_id ingenic_ipu_dt_match[] = {
	{ .compatible = "ingenic,PRJ007-ipu", .data = NULL },
	{},
};
