...
static int dump_debug(struct jz_ipu *ipu)
{
    int i = 0;
    for(i = 0 ;i < 10;i++){
        printk("----------------dump %d---------------\n",i);
        ipu_reg_write(ipu, IPU_TRIGGER,0);
        printk("write :0x0,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));

        ipu_reg_write(ipu, IPU_TRIGGER,0x40);
        printk("write :0x40,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));

        ipu_reg_write(ipu, IPU_TRIGGER,0x80);
        printk("write :0x80,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));

        ipu_reg_write(ipu, IPU_TRIGGER,0xc0);
        printk("write :0xc0,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));

        ipu_reg_write(ipu, IPU_TRIGGER,0x100);
        printk("write :0x100,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));

        ipu_reg_write(ipu, IPU_TRIGGER,0x140);
        printk("write :0x140,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));

        ipu_reg_write(ipu, IPU_TRIGGER,0x180);
        printk("write :0x180,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));

        ipu_reg_write(ipu, IPU_TRIGGER,0x1c0);
        printk("write :0x1c0,read(1b4) :0x%x\n",ipu_reg_read(ipu,IPU_TEST_1B4));
        printk("--------------------------------------\n");
    }
...
static int ipu_probe(struct platform_device *pdev)
{
	...
	ipu->ingenic_static_minor = 81;

	ipu->misc_dev.minor = ipu->ingenic_static_minor;
...
static const struct of_device_id ingenic_ipu_dt_match[] = {
	...
	{ .compatible = "ingenic,t41-ipu", .data = NULL },
	{},
};
