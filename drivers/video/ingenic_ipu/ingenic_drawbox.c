...
static int dbox_probe(struct platform_device *pdev)
{
	...
	dbox->ingenic_static_minor = 78;

	dbox->misc_dev.minor = dbox->ingenic_static_minor;
	...
	dbox->clk = devm_clk_get(dbox->dev, "drawbox");
	if (IS_ERR(dbox->clk)) {
		dev_err(&pdev->dev, "dbox clk get failed!\n");
		ret = -EINVAL;
		goto err_get_dbox_clk;
	}
	dbox->ahb0_gate = devm_clk_get(dbox->dev, "ahb0");
	if (IS_ERR(dbox->clk)) {
		dev_err(&pdev->dev, "dbox clk get failed!\n");
		ret = -EINVAL;
...
static const struct of_device_id ingenic_dbox_dt_match[] = {
    ...
    { .compatible = "ingenic,t41-drawbox", .data = NULL },
    {},
};
