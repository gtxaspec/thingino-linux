...
static struct proc_ops slcd_devices_fileops = {
    .proc_read		= slcd_proc_read,
    .proc_write		= slcd_proc_write,
};
static int jzfb_probe(struct platform_device *pdev)
{
    ...
    if (request_irq(jzfb->irq, jzfb_irq_handler, IRQF_SHARED/*IRQF_DISABLED*/,
                    jzfb->irq_name, jzfb)) {
