...
#if 1
#include "include/240_320_RGB_565.h" //rgb565 pic
#include "include/rgb.h"
#include "include/rbg.h"
#include "include/gbr.h"
#include "include/grb.h"
#include "include/bgr.h"
#include "include/brg.h"
#include "include/320240.h"
#endif
//#define FPGA_TEST
...
static void jzfb_enable(struct fb_info *info)
{
    ...
    slcd_send_mcu_command_data(jzfb, WRITE_GRAM_CMD, FLAG_CMD);
    wait_busy();
...
static void jzfb_chain_start(struct fb_info *info)
{
    ...
    int count = 10000;
    while((reg_read(jzfb, LCDC_TINY_ST) & WORKING) && count--) {
		udelay(10);
    }
    ...
    if(count > 0) {
		...
		dev_err(jzfb->dev, "SLCDC Dma chain is busy \n");
    }
...
static void jzfb_display_pic(struct fb_info *info)
{
    ...
    bpp = jzfb_get_controller_bpp(jzfb);
#ifdef LCDC_TINY_NV12
    memcpy(p8, __320240_nv12, w*h + w*h/2);
/*	dump_lcdc_registers(jzfb);*/
#else
    memcpy(p8, buf_240_320_RGB565, w*h*2);
   // memcpy(p8, gImage_320_240_rgb, w*h*2);
#endif
}
...
static void jzfb_display_color_bar(struct fb_info *info, unsigned int color_mode)
{
    ...
    p16 = (unsigned short *)jzfb->vidmem[0];
    p32 = (unsigned int *)jzfb->vidmem[0];
    p24 = (unsigned int *)jzfb->vidmem[0];
    w = mode->xres;
    h = mode->yres;
...
void test_slcd_partial_refresh(struct fb_info *info)
{
    ...
    wait_busy();
    slcd_send_mcu_command_data(jzfb, 0+100, FLAG_PAR);
    wait_busy();
    slcd_send_mcu_command_data(jzfb, 0 , FLAG_PAR);
    wait_busy();
    slcd_send_mcu_command_data(jzfb, 99+100 , FLAG_PAR);
    wait_busy();
...
//	jzfb_display_nv12_single_color_red(jzfb->fb);
//	jzfb_enable(jzfb->fb);
//	mdelay(5000);
//	jzfb_display_nv12_single_color_green(jzfb->fb);
//	jzfb_enable(jzfb->fb);
//	mdelay(5000);
//	jzfb_display_nv12_single_color_blue(jzfb->fb);
//	jzfb_enable(jzfb->fb);
//	mdelay(5000);
//#endif
#endif
...
static int jzfb_probe(struct platform_device *pdev)
{
	...
	jzfb->clk_gate = devm_clk_get(jzfb->dev ,"gate_lcd");
	if(IS_ERR(jzfb->clk_gate)){
		dev_err(&pdev->dev,"slcd clk get Failed!\n");
		ret = -EINVAL;
		goto err_get_jzfb_clk;
	}
	jzfb->clk_lcd = devm_clk_get(jzfb->dev, "div_lcd");
	if (IS_ERR(jzfb->clk_lcd)) {
		dev_err(&pdev->dev, "jzfb clk get failed!\n");
		ret = -EINVAL;
        goto err_get_ahb_clk;
	}
	if ((rc = clk_prepare_enable(jzfb->clk_gate)) < 0) {
		dev_err(&pdev->dev, "Enable jzfb gate clk failed\n");
		goto err_prepare_jzfb_clk;
	}
//	if ((rc = clk_prepare_enable(jzfb->clk_lcd)) < 0) {
//		dev_err(&pdev->dev, "Enable jzfb ahb0 clk failed\n");
...
//	}
	...
	rate = 20000000;
    ret = clk_set_rate(jzfb->clk_lcd, rate);
    if(rate != clk_get_rate(jzfb->clk_lcd)) {
        dev_err(&pdev->dev, "the rate to be setted :%ld, the rate getted in actually:%ld \n", rate,ret );
    }
    /* printk("width=%d height=%d pixel=%d\n",fb->var.width,fb->var.height,fb->var.bits_per_pixel);*/

    dev_set_drvdata(&pdev->dev, jzfb);
...
err_get_jzfb_clk:
	devm_clk_put(&pdev->dev, jzfb->clk);
	devm_clk_put(&pdev->dev, jzfb->clk_lcd);
	devm_clk_put(&pdev->dev, jzfb->clk_gate);
err_get_ahb_clk:
	devm_clk_put(&pdev->dev, jzfb->clk_gate);
err_prepare_jzfb_clk:
	clk_disable_unprepare(jzfb->clk_lcd);
err_prepare_ahb_clk:
	clk_disable_unprepare(jzfb->clk_gate);
err_free_file:
