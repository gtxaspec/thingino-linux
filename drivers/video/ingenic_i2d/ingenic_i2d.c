...
static int i2d_reg_set(struct jz_i2d *i2d, struct i2d_param *i2d_param)
{
	...
	h_align = ip->h_align;
	if(16 != h_align && 8 != h_align){
		dev_err(i2d->dev, "i2d: i2d h_align is err,check param, h_align = %d\n", h_align);
		return -1;
	}
	switch(fmt) {
		case PIX_FMT_NV12:
			/*NV12 参数配置放到SDK中处理*/
			//printk("nv12,src(%d,%d),srcstr(%d,%d),dststr(%d,%d),h_align:%d\n",srcw,srch,src_y_strid,src_uv_strid,dst_y_strid,dst_uv_strid,h_align);
		...
		case HAL_PIXEL_FORMAT_RAW8:
			...
			if ((rotate_angle == 0) || (rotate_angle == 180) || (flip_enable == 1) || (mirr_enable == 1)) {
				if(((flip_enable == 1) || (mirr_enable == 1)) && ((rotate_angle == 90) || (rotate_angle == 270))) {
					dst_y_strid = srch;
					dst_uv_strid = srch;
				} else {
					dst_y_strid = srcw;
	...
	if(flag == 0) {
		...
		reg_write(i2d, I2D_TIMEOUT_VALUE, (0x1e0000 << 0)); //timeout value
		flag = 1;
	}
#endif
   ...
   // reg_write(i2d, I2D_IMG_MODE, I2D_DATA_TYPE_NV12);
	...
	if(rotate_enable == 1) {
		...
		switch(flip_mode) {
			case 0:
				/*不旋转90/270则nv12分布和源保持一致*/
				//dst_uv_pbuf = ip->dst_addr_y + dst_y_strid*RoundUp(srch,h_align);
				dst_uv_pbuf = ip->dst_addr_uv;
	...
	src_uv_pbuf =ip->src_addr_y + src_y_strid*RoundUp(srch,h_align);/*目前SDK的内存都是16对齐*/
...
static int ingenic_i2d_suspend(struct device *dev)
{
	struct jz_i2d *i2d = dev_get_drvdata(dev);
	if(i2d->is_enable)
		clk_disable_unprepare(i2d->clk);
	return 0;
}
static int ingenic_i2d_resume(struct device *dev)
{
	struct jz_i2d *i2d = dev_get_drvdata(dev);
	if(i2d->is_enable)
		clk_prepare_enable(i2d->clk);
...
static SIMPLE_DEV_PM_OPS(ingenic_i2d_pm_ops, ingenic_i2d_suspend, ingenic_i2d_resume);
#endif
...
static const struct of_device_id ingenic_i2d_dt_match[] = {
	{ .compatible = "ingenic,PRJ007-i2d", .data = NULL },
	{ .compatible = "ingenic,PRJ008-i2d", .data = NULL },
	{},
};
