...
#ifdef SAFA_RESET_TEST
static int safa_reset_test_flag = 0;
#endif

#ifdef	DEBUG
static int debug_i2d = 1;
static int i2d_timeout_restart = 0;
...
static int i2d_reg_set(struct jz_i2d *i2d, struct i2d_param *i2d_param)
{
    ...
    switch(fmt) {
        case PIX_FMT_NV12:
            src_y_strid  = srcw;
            src_uv_strid = srcw;
            if ((rotate_angle == FLIP_MODE_0_DEGREE) || (rotate_angle == FLIP_MODE_180_DEGREE) || (flip_enable == 1) || (mirr_enable == 1)) {
                dst_y_strid = srcw;
                dst_uv_strid = srcw;
            } else if ((rotate_angle == FLIP_MODE_90_DEGREE) || (rotate_angle == FLIP_MODE_270_DEGREE)) {
                dst_y_strid = srch;
                dst_uv_strid = srch;
            }
        ...
        case HAL_PIXEL_FORMAT_ARGB_8888:
            ...
            break;
        default:
            printk("[I2D]Warning default mode\n");
            src_y_strid  = srcw;
            src_uv_strid = srcw;
            if ((rotate_angle == FLIP_MODE_0_DEGREE) || (rotate_angle == FLIP_MODE_180_DEGREE) || (flip_enable == 1) || (mirr_enable == 1)) {
                dst_y_strid = srcw;
...
#ifdef TIMEOUT_TEST
    if(i2d_timeout_restart == 0){
        reg_write(i2d, I2D_TIMEOUT_MODE, (1 << 0)); //timeout mode 0:not restart  1:auto restart
    ...
    if(rotate_enable == 1) {
        switch(rotate_angle) {
        case FLIP_MODE_0_DEGREE:
            dst_uv_pbuf = ip->dst_addr_y + dst_y_strid*srch;
            value = reg_read(i2d, I2D_IMG_MODE);
            reg_write(i2d, I2D_IMG_MODE, (value | I2D_FLIP_MODE_0));
            break;
        case FLIP_MODE_90_DEGREE:
            dst_uv_pbuf = ip->dst_addr_y + dst_y_strid*srcw;
            value = reg_read(i2d, I2D_IMG_MODE);
            reg_write(i2d, I2D_IMG_MODE, (value | I2D_FLIP_MODE_90));
            break;
        case FLIP_MODE_180_DEGREE:
            dst_uv_pbuf = ip->dst_addr_y + dst_y_strid*srch;
            value = reg_read(i2d, I2D_IMG_MODE);
            reg_write(i2d, I2D_IMG_MODE, (value | I2D_FLIP_MODE_180));
            break;
        case FLIP_MODE_270_DEGREE:
            dst_uv_pbuf = ip->dst_addr_y + dst_y_strid*srcw;
            value = reg_read(i2d, I2D_IMG_MODE);
            reg_write(i2d, I2D_IMG_MODE, (value | I2D_FLIP_MODE_270));
            break;
        default:
            printk("[I2D]Warning default mode %d\n",rotate_angle);
            dst_uv_pbuf = ip->dst_addr_y + dst_y_strid*srch;
            value = reg_read(i2d, I2D_IMG_MODE);
            reg_write(i2d, I2D_IMG_MODE, (value |I2D_FLIP_MODE_0));
            break;
...
#ifdef TIMEOUT_TEST
    udelay(1000000);
    if(i2d_timeout_restart == 1){
        printk("######### timeout restart\n");
        reg_write(i2d, I2D_TIMEOUT_MODE, (0 << 0)); //timeout mode 0:not restart  1:auto restart
        reg_write(i2d, I2D_TIMEOUT_VALUE, (0 << 0)); //timeout value
        i2d_timeout_restart = 2;
        goto restart;
    }
#endif

#ifdef SAFA_RESET_TEST
    if(safa_reset_test_flag == 0){
        printk("#### now safa reset test\n");
	    __reset_safe_i2d();
        safa_reset_test_flag = 1;
    }else if(safa_reset_test_flag == 1){
        safa_reset_test_flag = 0;
        printk("### It has been safa reset and works normally this time\n");
    }
#endif
...
#ifdef TIMEOUT_TEST
    if (status & 0x2){
        if((ttt > 10 ) && (i2d_timeout_restart ==0)){
            i2d_timeout_restart = 1;
            reg_write(i2d, I2D_TIMEOUT_VALUE, 0xfffffff); //timeout value
            I2D_DEBUG("### disable timeout\n");
        }else{
            I2D_DEBUG("### timeout num :%d\n",ttt);
            ttt++;
        }
    }
#endif
...
static int i2d_probe(struct platform_device *pdev)
{
	...
	i2d->ingenic_static_minor = 80;

	i2d->misc_dev.minor = i2d->ingenic_static_minor;
	...
	i2d->clk = devm_clk_get(i2d->dev, "i2d");
	if (IS_ERR(i2d->clk)) {
		dev_err(&pdev->dev, "i2d clk get failed!\n");
		goto err_get_i2d_clk;
	}

	i2d->ahb0_gate = devm_clk_get(i2d->dev, "ahb0");
	if (IS_ERR(i2d->clk)) {
		dev_err(&pdev->dev, "i2d clk get failed!\n");
...
static int __init i2ddev_init(void)
{
	I2D_DEBUG("%s\n", __func__);
	platform_driver_register(&jz_i2d_driver);
	return 0;
}

static void __exit i2ddev_exit(void)
{
	I2D_DEBUG("%s\n", __func__);
	platform_driver_unregister(&jz_i2d_driver);
}

module_init(i2ddev_init);
module_exit(i2ddev_exit);

MODULE_DESCRIPTION("JZ I2D driver");
MODULE_AUTHOR("damon.jszhang <damon.jszhang@ingenic.cn>");
MODULE_LICENSE("GPL");
