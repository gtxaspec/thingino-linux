...
static long vpu_start(struct device *dev, const struct channel_node * const cnode)
{
	...
	vpu_writel(vpu, RADIX_CFGC_BASE + RADIX_REG_CFGC_TIMEOUT_THRD, vpu_clk * 1000000);
...
static irqreturn_t vpu_interrupt(int irq, void *dev)
{
	...
	if(vpu_stat & RADIX_CFGC_INTE_ENDF) {
		...
		vpu_writel(vpu, RADIX_EMC_BASE + 0x60, 2);
...
static int vpu_probe(struct platform_device *pdev)
{
	...
	clk_set_rate(vpu->clk, 600000000);
	clk_set_rate(vpu->ahb1, 600000000);
...
static const struct of_device_id jz_radix_dt_match[] = {
	{ .compatible = "ingenic,PRJ007-radix", .data = NULL },
	{},
};
MODULE_DEVICE_TABLE(of, jz_radix_dt_match);
...
static struct platform_driver jz_vpu_driver = {
	...
	.driver		= {
		.name	= "jz_radix",
		.owner	= THIS_MODULE,
		.of_match_table	= of_match_ptr(jz_radix_dt_match),
	},
};
...
MODULE_DESCRIPTION("PRJ007 VPU radix driver");
