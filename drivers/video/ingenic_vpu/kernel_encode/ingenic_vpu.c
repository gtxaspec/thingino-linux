...
int file_read_thread_fun(void *data)
{
	...
	chnAttr.encPhyaddr = VPU_CHN0_MEMORY_START + stream_out_len;
...
int vpu_work_thread_fun(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		spin_lock(&module->chain_lock);
		if (list_empty(&module->busy_list)) {
			spin_unlock(&module->chain_lock);
			break;
		}
...
static long vpu_stop(void)
{
	...
	int i = 0;
	int retry = 250000; //5s
	struct vpu_inst *vpu_chn = g_module->vpu_chn;
	struct list_head *list = NULL;
	...
	if (!g_module->is_module_start) {
		...
		while (retry --) {
			spin_lock(&g_module->chain_lock);
			i = 0;
			list_for_each(list, &g_module->free_list) {
				i ++;
			}
			if (i == NUM_MAX_CHAIN) {
				spin_unlock(&g_module->chain_lock);
				break;
			} else {
				spin_unlock(&g_module->chain_lock);
				vpu_wrap_usleep(20);
...
int VPU_Kern_Init(void)
{
	...
	if (!g_module) {
		...
		for (i = 0; i < NUM_MAX_CHAIN; i ++) {
			...
			g_module->work_list[i].frame.isp_priv = vpu_wrap_malloc(isp_bufsize);
		...
		g_module->vpu_fops.owner = THIS_MODULE;
		g_module->vpu_fops.unlocked_ioctl = fops_vpu_ioctl;
		g_module->vpu_fops.release = fops_vpu_release;
#ifdef CONFIG_ZERATUL
		g_module->vpu_dev.minor = 57;
#else
		g_module->vpu_dev.minor = MISC_DYNAMIC_MINOR;
#endif
		g_module->vpu_dev.name = DEVICE_NAME;
		g_module->vpu_dev.fops = &g_module->vpu_fops;
		ret = misc_register(&g_module->vpu_dev);
		if (ret < 0) {
...
#endif
err_vpu_work_task:
	misc_deregister(&g_module->vpu_dev);
err_kern_enc_dev:
#ifndef VPU_FILE_INPUT_TEST
	for (i = 0; i < NUM_MAX_CHAIN; i ++) {
		if (g_module->work_list[i].frame.isp_priv) {
			vpu_wrap_free(g_module->work_list[i].frame.isp_priv);
		}
	}
err_isp_priv_alloc:
#endif
...
int VPU_Encoder_GetChannelBufferSize(const VPUEncoderAttr *chnAttr)
{
	...
	if (chnAttr->enType == PT_H265) {
		y_rdsize = C_ALIGN(w_align * (h_align + (2 + 1) * 64), 256);
		c_rdsize = C_ALIGN(w_align * (h_align / 2 + (2 + 1) * 32), 256);
		vpu_size = C_ALIGN(VPU_VDMA_SIZE + 256 + (C_ALIGN(chnAttr->picWidth, 32) / 32) * (C_ALIGN(chnAttr->picHeight, 32) / 32) * (77 * 4) + 20 + 2*1024 + 14*256, 256);
		...
		y_rdsize = C_ALIGN(w_align * (h_align + (2 + 1) * 64), 256);
		c_rdsize = C_ALIGN(w_align * (h_align / 2 + (2 + 1) * 32), 256);
		vpu_size = C_ALIGN(VPU_VDMA_SIZE + 256 + (C_ALIGN(chnAttr->picWidth, 16) / 16) * (C_ALIGN(chnAttr->picHeight, 16) / 16) * 70 + 73 + 2*1024 + 14 * 256, 256);
