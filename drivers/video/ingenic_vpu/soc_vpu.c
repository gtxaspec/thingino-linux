...
#include <linux/math64.h>//hera_bq
#include <asm/div64.h>//hera_bq
...
int vpu_register(struct list_head *vlist)
{
	...
	vpu_node_num++;
	//printk("vpu_node_num: %d", vpu_node_num);
	return 0;
}
...
find_channel_try_point:
	...
	if (ret > 0) {
		...
		if (list_empty(&fclist->fclist_head)) {
			...
			//add byzjzhao//while(try_wait_for_completion(&fclist->cdone));
			ret = -EBUSY;
...
find_vpu_try_point:
	...
	if (ret > 0) {
		...
		if (list_empty(&fvlist->fvlist_head)) {
			...
			//add by zjzhao//while(try_wait_for_completion(&fvlist->vdone));
			ret = -EBUSY;
...
static long soc_channel_flush_cache_all(struct soc_channel *sc, struct channel_node *cnode)
{
	...
	size_t vir_size = cnode->dma_size;

	dma_cache_sync(NULL, vir_addr, vir_size, WBACK);
#endif
...
static long soc_channel_run(struct soc_channel *sc, long usr_arg)
{
	...
	//do_gettimeofday(&end);
	//printk(KERN_DEBUG"id=%d time=%ldus\n", cnode.sensor_id, end.tv_sec * 1000 * 1000 + end.tv_usec - start.tv_sec * 1000 * 1000 - start.tv_usec);
...
static long soc_vpu_set_bsfull_reg(struct soc_channel *sc, struct channel_node *cnode)
{
	long ret = 0;
	...
	if (vpu->dev && vpu->ops && vpu->ops->set_bsfull_reg) {
		ret = vpu->ops->set_bsfull_reg(vpu->dev, cnode);
		if (ret < 0) {
			ret = -EIO;
			dev_err(sc->mdev.this_device, "[fun:%s,line:%d] set bsfull reg failed\n", __func__, __LINE__);
			goto err_set_bsfull_reg;
		}
	} else {
		ret = -ENODEV;
		dev_err(sc->mdev.this_device, "[fun:%s,line:%d] no vpu device failed\n", __func__, __LINE__);
...
static long soc_channel_set_bsfull_reg(struct soc_channel *sc, long usr_arg)
{
	struct channel_node cnode;
	long ret = 0;
	...
	if ((ret = soc_vpu_set_bsfull_reg(sc, &cnode)) < 0) {
		dev_err(sc->mdev.this_device, "[fun:%s,line:%d] set bsfull reg failed\n", __func__, __LINE__);
		goto err_set_bsfull_reg;
	}
...
static long soc_channel_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_CHANNEL_WAIT_BSFULL_COMPLETE:
		...
		break;
	case IOCTL_CHANNEL_SET_BSFULL_REG:
		ret = soc_channel_set_bsfull_reg(sc, arg);
		break;
	default:
		ret = -1;
	}
