...
#define VC8KD_SYNC
#ifdef VC8KD_SYNC
DEFINE_MUTEX(vpu_send_mutex);
DEFINE_MUTEX(vpu_work_mutex);

void vpu_work_lock(void)
{
	mutex_lock(&vpu_work_mutex);
}
EXPORT_SYMBOL(vpu_work_lock);

void vpu_work_unlock(void)
{
	mutex_unlock(&vpu_work_mutex);
}
EXPORT_SYMBOL(vpu_work_unlock);

int vpu_work_is_locked(void)
{
	return mutex_is_locked(&vpu_work_mutex);
}
EXPORT_SYMBOL(vpu_work_is_locked);
#endif
