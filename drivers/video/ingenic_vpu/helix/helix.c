...
static long vpu_reset(struct device *dev)
{
	...
	int timeout = 0x7fff;
	unsigned int srbc = cpm_inl(CPM_SRBC0);

#ifndef CONFIG_FPGA_TEST
	cpm_set_bit(CPM_HELIX_STP, CPM_SRBC0);
	while (!(cpm_inl(CPM_SRBC0) & (1 << CPM_HELIX_ACK)) && --timeout) udelay(20);

	if (timeout == 0) {
		dev_warn(vpu->vpu.dev, "[%d:%d] wait stop ack timeout\n",
			 current->tgid, current->pid);
		cpm_outl(srbc, CPM_SRBC0);
		return -1;
	} else {
		cpm_outl(srbc | (1 << CPM_HELIX_SR), CPM_SRBC0);
		cpm_outl(srbc, CPM_SRBC0);
	}
#else
	cpm_outl(srbc | (1 << CPM_HELIX_SR), CPM_SRBC0);
...
static long vpu_release(struct device *dev)
{
	...
	cpm_clear_bit(CPM_HELIX_SR, CPM_SRBC0);
...
hard_vpu_wait_restart:
	...
	while ((bsreadcnt-- > 0) && (ret > 0) && (vpu->bslen == 0) && (cnode->codecdir == HWJPEGENC)) {
	...
	if ((ret > 0) && (vpu->bslen || cnode->codecdir == HWJPEGDEC)) {
		ret = 0;
...
static long vc8kd_clk_on(struct device *dev)
{
	struct jz_vpu_helix *vpu = dev_get_drvdata(dev);
	clk_prepare_enable(vpu->clk_vc8000d);
	return 0;
}

static long vc8kd_clk_off(struct device *dev)
{
	struct jz_vpu_helix *vpu = dev_get_drvdata(dev);
	clk_disable(vpu->clk_vc8000d);
	return 0;
}
...
static struct vpu_ops vpu_ops = {
	...
	.suspend	= vpu_suspend,
	.resume		= vpu_resume,
	.vc8kd_clk_on = vc8kd_clk_on,
	.vc8kd_clk_off = vc8kd_clk_off,
};
...
#ifndef CONFIG_FPGA_TEST
    vpu->clk_vc8000d = clk_get(&pdev->dev,"gate_vc8000d");
    if (IS_ERR(vpu->clk_vc8000d)) {
        dev_err(&pdev->dev, "clk vc8000d get failed\n");
        ret = PTR_ERR(vpu->clk_vc8000d);
        goto err_get_vpu_clk_gate;
    }
	clk_prepare_enable(vpu->clk_vc8000d);

    vpu->clk = clk_get(&pdev->dev,"gate_jpeg");
    if (IS_ERR(vpu->clk)) {
        dev_err(&pdev->dev, "clk jpeg get failed\n");
        ret = PTR_ERR(vpu->clk);
...
static const struct of_device_id ingenic_vpu_dt_match[] = {
	{ .compatible = "ingenic,jpeg", .data = NULL },
	{},
};
MODULE_DEVICE_TABLE(of, ingenic_vpu_dt_match);
...
static struct platform_driver jz_vpu_driver = {
	...
	.driver		= {
		.name	= "jz-jpeg",
		.of_match_table = of_match_ptr(ingenic_vpu_dt_match),
	},
#ifndef CONFIG_FPGA_TEST
