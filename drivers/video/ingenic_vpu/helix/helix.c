...
static int vpu_probe(struct platform_device *pdev)
{
	...
	vpu->ahb1_gate = clk_get(&pdev->dev, "div_ispa");
	...
	vpu->clk_gate = clk_get(&pdev->dev, "gate_ivdc");
    ...
    vpu->clk = clk_get(&pdev->dev, "gate_rjpeg");
    if (IS_ERR(vpu->clk)) {
        dev_err(&pdev->dev, "clk get failed\n");
        ret = PTR_ERR(vpu->clk);
        goto err_get_vpu_clk_cgu;
    }
    clk_set_rate(vpu->clk,550000000);
...
static const struct of_device_id ingenic_vpu_dt_match[] = {
	{ .compatible = "ingenic,t41-jpeg", .data = NULL },
	{},
};
