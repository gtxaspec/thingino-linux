...
static int vpu_probe(struct platform_device *pdev)
{
	...
	//printk("[probe]pdev->name=%s pdev->id=%d, vpu_id=0x%08x\n", pdev->name, pdev->id, vpu->vpu.vpu_id);
...
*/
	...
	vpu->clk_gate = devm_clk_get(&pdev->dev, "gate_vpu");
    ...
    vpu->clk = devm_clk_get(&pdev->dev,"div_vpu");
    if (IS_ERR(vpu->clk)) {
...
static const struct of_device_id jz_helix_dt_match[] = {
	{ .compatible = "ingenic,t23-vpu", .data = NULL },
	{},
};
