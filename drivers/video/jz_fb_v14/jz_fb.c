...
static int jzfb_update_frm_mode(struct fb_info *info)
{
	...
	lay_cfg[0].g_alpha_val = 0x7F;
	lay_cfg[1].g_alpha_en = 0;
	lay_cfg[1].g_alpha_val = 0xFF;
	
	lay_cfg[0].domain_multi = 1;
	lay_cfg[1].domain_multi = 0;
	for(i = 0; i < MAX_LAYER_NUM; i++) {
...
int fb_test_pattern(struct jzfb *jzfb)
{
	int ret;
...
//		printk("disable before\n");
//		jzfb_disable(info, QCK_STOP);
//		jzfb_disable(info, GEN_STOP);
//		printk("disable before1\n");
//	}
	...
	if(jzfb->pdata->lcd_type == LCD_TYPE_SLCD) {
		...
		//reg_write(jzfb, jzfb->frm_size * (2+next_frm), jzfb->framedesc_phys[next_frm]);
		//reg_write(jzfb, jzfb->frm_size * (2+next_frm), jzfb->layerdesc_phys[next_frm][1]);
...
static int jzfb_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(jzfb->ahb1_gate)) {
		ret = dev_err(&pdev->dev, "jzfb ahb1_gate get failed!\n");
		goto err_put_clk;
	}
#endif
