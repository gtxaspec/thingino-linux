...
#define DISPLAY_PIC
#if 1
#include "include/frame240x320.h"    //nv12 pic
#include "include/240_320_RGB_565.h" //rgb565 pic
#include "include/240_320_1.h"
#include "include/240_320_2.h"
...
static void jzfb_display_pic(struct fb_info *info)
{
    ...
    bpp = jzfb_get_controller_bpp(jzfb);
#ifdef LCDC_TINY_NV12
    memcpy(p8, buf_240_320_NV12, w*h + w*h/2);
#else
	memcpy(p8, __240_320_1_rgb565, w*h*2);
	//memcpy(p8, __240_320_2_rgb565, w*h*2);
#endif
}
...
static int jzfb_probe(struct platform_device *pdev)
{
	...
	jzfb->clk_div = clk_get(&pdev->dev, "div_lcd");
	if (IS_ERR(jzfb->clk_div)) {
		dev_err(&pdev->dev, "clk_gate get failed\n");
		ret = PTR_ERR(jzfb->clk_div);
		goto err_get_jzfb_clk_cgu;
	}

	jzfb->clk = clk_get(&pdev->dev, "gate_lcd");
    if (IS_ERR(jzfb->clk)) {
	...
	rate = PICOS2KHZ(30000) * 100 *5;
	ret = clk_set_rate(jzfb->clk_div, rate);
    if(rate != clk_get_rate(jzfb->clk_div)) {
        dev_err(&pdev->dev, "the rate to be setted :%ld, the rate getted in actually:%ld \n", rate, clk_get_rate(jzfb->clk_div));
    }
    printk("width=%d height=%d pixel=%d\n",pdata->width,pdata->height,pdata->bpp);
	/* Enable lcd/cgu_lpc/ahb0 clk */
   	clk_prepare_enable(jzfb->clk);
  	clk_prepare_enable(jzfb->clk_div);
	//*(volatile unsigned int *)0xb0000064 = 0xa0000048;
...
err_get_ahb0_clk_div:
	iounmap(jzfb->mem);
err_get_jzfb_clk_cgu:
	clk_put(jzfb->clk_div);
err_free_file:
