...
//	int init_level;
	...
	int gpio_sel[INGENIC_PWM_NUM];
	int init_level[INGENIC_PWM_NUM];
	int finish_level[INGENIC_PWM_NUM];
	int irq;
};
		...
		int duty_ns, int period_ns)
{
	...
	/*set init level*/
	if (ingenic_pwm->init_level[channel] == PWM_INIT_HIGH)
		pwm_set_init_level(ingenic_pwm,channel,1);
	else if (ingenic_pwm->init_level[channel] == PWM_INIT_LOW)
		pwm_set_init_level(ingenic_pwm,channel,0);
	else
		pr_err("%s, pwm init level error\n", __func__);

	printk("duty=%d level_init=%d low=%d\n", duty, ingenic_pwm->init_level[channel], period - duty);
	...
	if (ingenic_pwm->finish_level[channel] == PWM_INIT_HIGH)
		pwm_set_finish_level(ingenic_pwm,channel,1);
	else if (ingenic_pwm->finish_level[channel] == PWM_INIT_LOW)
		pwm_set_finish_level(ingenic_pwm,channel,0);
	else
		pr_err("%s, pwm finish level error\n", __func__);
	...
	printk("is enable?:%d\n", pwm_is_enabled(&ingenic_pwm->chip.pwms[channel]));
	printk("is enable?:%d\n", pwm_is_enabled(pwm));
	if(pwm_is_enabled(&ingenic_pwm->chip.pwms[channel])) {
		if(COMMON_MODE == mode){
...
static ssize_t pwm_store_config(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ingenic_pwm->init_level[ingenic_pwm->debug_current_id] = simple_strtoul(str, (char **)&str, 10);
	...
	ingenic_pwm->finish_level[ingenic_pwm->debug_current_id] = simple_strtoul(str, (char **)&str, 10);
...
static ssize_t pwm_store_channel(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	gpio_id = simple_strtoul(str, (char **)&str, 10);
	while (!isxdigit(*str)) {
		str++;
		if(++ret_count >= count)
			goto out;
	}
	gpio_func = simple_strtoul(str, (char **)&str, 10);
	if(gpio_func > 3){
		printk("The gpio function is invalid!\n");
		goto out;
	}
	printk("gpio_id:%d func:%d\n", gpio_id, gpio_func);

	memset(name, 0, sizeof(name));
	sprintf(name, "pwm-%d", pwm_id);
	if(gpio_request(gpio_id, name) < 0){
		printk("Failed to request gpio of %s\n", name);
		goto out;
	}

	printk("gpio:%d\n", gpio_id);
	jzgpio_set_func(gpio_id/32, gpio_func + GPIO_FUNC_0, 1<<(gpio_id%32));
	ingenic_pwm->debug_pwm[pwm_id] = pwm;
	ingenic_pwm->gpio_sel[pwm_id] = gpio_id;
...
static ssize_t pwm_store_free(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(ingenic_pwm->debug_pwm[pwm_id]){
		gpio_free(ingenic_pwm->gpio_sel[pwm_id]);
...
static int ingenic_pwm_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "ingenic-PRJ007 Probe of pwm success!\n");
	return 0;
