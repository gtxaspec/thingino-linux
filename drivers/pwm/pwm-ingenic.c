...
static int ingenic_pwm_probe(struct platform_device *pdev)
{
	...
	ingenic_pwm->clk_div[4] = CLK_1;
	ingenic_pwm->clk_div[5] = CLK_1;
	ingenic_pwm->clk_div[6] = CLK_1;
	ingenic_pwm->clk_div[7] = CLK_1;
...
#else
	ingenic_pwm->clk_pwm = devm_clk_get(&pdev->dev, "cgu_pwm");
	...
	ingenic_pwm->clk_gate = devm_clk_get(&pdev->dev, "pwm");
	...
	if (ingenic_pwm->clk_pwm){
		...
		ret = clk_enable(ingenic_pwm->clk_pwm);
		if (ret){
