...
static int jz_pwm_request(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	config.division = ingenic_pwm->clk_div[pwm->hwpwm];
						 ...
						 int duty_ns, int period_ns)
{
	...
	pr_debug("duty=%d,period=%d\n", duty_ns, period_ns);

	tdfr = div_u64((u64)ingenic_pwm->clk_ch[pwm->hwpwm] * period_ns + 500000000, 1000000000);
	tdhr = div_u64((u64)ingenic_pwm->clk_ch[pwm->hwpwm] * duty_ns + 500000000, 1000000000);
	tdfr < 2 ? tdfr = 1 : --tdfr;
	tdhr ? --tdhr : tdhr;
	pr_debug("tdfr=%u,tdhr=%u\n", tdfr, tdhr);
...
static int jz_pwm_probe(struct platform_device *pdev)
{
	...
	ingenic_pwm->clk_div[0] = TCU_DIV_1;
	ingenic_pwm->clk_div[1] = TCU_DIV_1;
	ingenic_pwm->clk_div[2] = TCU_DIV_1;
	ingenic_pwm->clk_div[3] = TCU_DIV_1;
	...
	dev_info(&pdev->dev, "PWM clk:%u", PWM_CLK_RATE);
	for (n = 0; n < PWM_CHN_NUM; n++)
	{
		ingenic_pwm->clk_ch[n] = PWM_CLK_RATE / (1 << (2*ingenic_pwm->clk_div[n]));
		ingenic_pwm->period_ns_max[n] = 1000000000 / ingenic_pwm->clk_ch[n] * (0xffff+1);
		ingenic_pwm->period_ns_min[n] = 1000000000 * (0+1) / ingenic_pwm->clk_ch[n] + 1;
