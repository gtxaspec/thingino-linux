...
#define AUDIO_DRIVER_VERSION "V20-20200228a"
	...
	if(dp->pipe_state == SND_DSP_STATE_RUN){
		...
		if(file->f_mode & FMODE_WRITE){
			...
			if (dp->dp_dmic) {
				...
				dp->dp_dmic->living_attr.channel = object->channel;
				dp->dp_dmic->living_attr.format = object->format;
				dp->dp_dmic->living_attr.rate = object->rate;
			}
#endif
		}else{
			snd_error_print("Don't support the fmode(%d)!\n",file->f_mode);
