...
#include "../../xb_snd_config.h"

#define time_codec_init
// #define CONFIG_SOC_T31

#if XB_SND_FAST_START
static int codec_fast_init = 0;
static int mic_fast_init = 0;
static int spk_fast_init = 0;
static int mic_fast_inited = 0;
static int spk_fast_inited = 0;
static struct work_struct delayed_work;
#ifndef time_codec_init
static struct work_struct delayed_work_spk;
#else
...
static struct mutex codec_mutex_lock;

static void codec_lock_init(void)
{
	mutex_init(&codec_mutex_lock);
}

static inline void codec_lock(void)
{
	mutex_lock(&codec_mutex_lock);
}

static inline void codec_unlock(void)
{
	mutex_unlock(&codec_mutex_lock);
}

#if 0
static void codec_msleep(int ms)
{
	unsigned long long t1 = sched_clock();

	while(1) {
		unsigned long long t2 = sched_clock() - t1;
		unsigned int t2_ns = do_div(t2, 1000000);
...
#ifdef time_codec_init
static int time_init_codec(int status)
{
	int ret = 0;

	// ddr_printf("Audio init codec\n");
    switch (status)
    {
    case 1:
        ret = codec_reg_set(TS_CODEC_CGR_00, 0, 1, 0);
        break;
    case 2:
        ret = codec_reg_set(TS_CODEC_CGR_00, 0, 1, 0x3);
        break;
    case 3:
        ...
        ret = codec_reg_set(TS_CODEC_CANACR_26, 0, 1, 0x1);
        break;
    case 4:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x1);
        break;
    case 5:
        ret = codec_reg_set(TS_CODEC_CAACR_22, 5, 5, 1);//setup reference voltage
        break;
    case 6:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x1);
        break;
    case 8:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x3);
        break;
    case 10:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x7);
        break;
    case 12:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0xf);
        break;
    case 14:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x1f);
        break;
    case 16:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x3f);
        break;
    case 18:
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x7f);
        break;
    ...
    default:
        ...
        // {
        //     value |= value << 1 | 1;
        //     ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, value);
        // }
        break;
    }
...
static int time_codec_set_buildin_mic(int status)
{
	// ddr_printf("Audio codec_set_buildin_mic\n");
	int ret = 0;
    ...
    case 20:
        ...
        ret = codec_reg_set(TS_CODEC_CANACR_26, 0, 1, 0x2);
        break;
	case 20 + 1:
		/* enable hpf and set mode 0 */
		ret = codec_reg_set(TS_CODEC_CGAINLR_09, 0, 4, 0x8);
		break;
	case 20 + 2:
        /* ADC valid data length */
        ret = codec_reg_set(TS_CODEC_CACR_02, 5, 6, 0x0);
        break;
    case 20 + 3:
        /* Choose ADC I2S interface mode */
        ret = codec_reg_set(TS_CODEC_CACR_02, 3, 4, 0x2);
        break;
    case 20 + 4:
        /* Choose ADC chn */
        ret = codec_reg_set(TS_CODEC_CACR_02, 0, 0, 0x1);
        break;
    case 20 + 5:
        ret = codec_reg_set(TS_CODEC_CACR_02, 7, 7, 0x1);
        break;

    case 20 + 6:
        /* Choose ADC I2S Master Mode */
        ret = codec_reg_set(TS_CODEC_CACR2_03, 0, 7, 0x3e);
        break;  

    case 20 + 7:
	    /* Enable ADC */
        ret = codec_reg_set(TS_CODEC_CMICCR_23, 7, 7, 0x1);
        break;
    case 20 + 8:
        ret = codec_reg_set(TS_CODEC_CAACR_22, 4, 4, 0x1);
        break;
    case 20 + 9:
        ret = codec_reg_set(TS_CODEC_CAACR_22, 3, 3, 0x1);   //micbias enable
        break;
    case 20 + 10:
        ret = codec_reg_set(TS_CODEC_CAACR_22, 0, 2, 0x7);   //micbias
        break;
    case 20 + 11:
        ret = codec_reg_set(TS_CODEC_CMICCR_23, 5, 5, 0x1);  //ref voltage
        break;
    case 20 + 12:
        ret = codec_reg_set(TS_CODEC_CMICGAINR_24, 4, 4, 0x1);
        break;
    case 20 + 13:
        ret = codec_reg_set(TS_CODEC_CMICGAINR_24, 5, 5, 0x1);
        break;
    case 20 + 14:
        ret = codec_reg_set(TS_CODEC_CANACR_26, 6, 6, 0x1);
        break;
    case 20 + 15:
        ret = codec_reg_set(TS_CODEC_CANACR_26, 5, 5, 0x1);
        break;
    case 20 + 16:
        ret = codec_reg_set(TS_CODEC_CANACR_26, 4, 4, 0x1);
        break;
    case 20 + 17:
        ret = codec_reg_set(TS_CODEC_CANACR_26, 7, 7, 0x1);
        break;
    case 20 + 18:
        ret = codec_reg_set(TS_CODEC_CMICCR_23, 6, 6, 0x1);
        break;
    case 20 + 19:
		if(mic_gain)
        	ret = codec_reg_set(TS_CODEC_CMICGAINR_24, 6, 7, mic_gain+1);
        break;
    case 20 + 20:
        ret = codec_reg_set(TS_CODEC_CALCGR_25, 0, 4, 0x10);
        break;
    case 20 + 21:
		if(alc_mode)
			ret = codec_reg_set(TS_CODEC_CMICCR_23, 4, 4, 1);
		break;
    case 20 + 22:
...
static int time_codec_set_speaker(int status)
{
	int ret = 0, val = -1;
    switch (status)
    {
    case 43:
        /* low power consumption */
        ret = codec_reg_set(TS_CODEC_POWER_20, 0, 2, 0x7);
        ret = codec_reg_set(TS_CODEC_CMICCR_23, 0, 3, 0xf);
        /* enable current source */
        ret = codec_reg_set(TS_CODEC_CANACR_26, 3, 3, 0x1);
        break;
    case 43 + 1:
        /* enable reference voltage */
        ret = codec_reg_set(TS_CODEC_CANACR_26, 2, 2, 0x1);
        break;
    case 43 + 2:
        ...
        break;
    case 43 + 3:
        ret = codec_reg_set(TS_CODEC_CHR_28, 5, 5, 0x1);
        break;
    case 43 + 4:
        ret = codec_reg_set(TS_CODEC_CHR_28, 6, 6, 0x1);
        break;
    case 43 + 5:
        ret = codec_reg_set(TS_CODEC_CANACR2_27, 7, 7, 0x1);
        break;
    case 43 + 6:
        ret = codec_reg_set(TS_CODEC_CANACR2_27, 6, 6, 0x1);
        break;
    case 43 + 7:
        ret = codec_reg_set(TS_CODEC_CANACR2_27, 5, 5, 0x1);
        break;
    case 43 + 8:
        ret = codec_reg_set(TS_CODEC_CANACR2_27, 4, 4, 0x1);
        break;
    case 43 + 9:
        ret = codec_reg_set(TS_CODEC_CHR_28, 7, 7, 0x1);
        break;
    case 43 + 10:
        /* set replay volume */
        ret = codec_reg_set(TS_CODEC_CHR_28, 0, 4, 0x18);
        break;
    case 43 + 11:
    	/* low power consumption */
        ret = codec_reg_set(TS_CODEC_CCR_21, 0, 6, 0x1);
...
{
	printk("Audio codec_set_buildin_mic\n");
...
#ifdef time_codec_init
static int codec_start(int status)
{
	int ret = 0;
	mod_timer(timer,jiffies + HZ/100);
    if(time_status <= 19) {
        ret = time_init_codec(time_status);
		if(ret < 0) {
			printk("init codec failed . status:%d\n", time_status);
		}
    } else if(time_status <= (20 + 22)) {
        ret = time_codec_set_buildin_mic(time_status);
		if(ret <0) {
			printk("init codec failed . 1 status:%d\n", time_status);
		}
	} else if(time_status <= (43 + 11)) {
		ret = time_codec_set_speaker(time_status);
		if(ret <0) {
			printk("init codec failed . 2 status:%d\n", time_status);
		}
	} else {
		del_timer(timer);
		printk("codec init complate\n");
	}
    time_status ++;
...
static int codec_fast_start(void)
{
	int ret = 0;

	printk("codec_fast_start init\n");

#ifndef time_codec_init
	ret = init_codec();
	if(ret < 0) {
		printk("codec_fast_start init_codec error\n");
		return -1;
	}

	schedule_work(&delayed_work_spk);

	ret = codec_set_buildin_mic();
	if(ret < 0) {
		printk("codec_fast_start codec_set_buildin_mic error\n");
		return -1;
	}
#else
    ...
    init_timer(timer );
    timer->function = codec_start;
    timer->expires = jiffies + (HZ/100);
    add_timer(timer);

	while(time_status < (43 + 12)) {
		ret ++;
		if(ret > 200) {
			del_timer(timer);
...
#if XB_SND_FAST_START
		if(++spk_fast_init == 1) {
			printk("%s ignore codec_set_speaker\n", __func__);
			ret = 0;
			break;
		}
#endif
...
#if XB_SND_FAST_START
		if(++mic_fast_init == 1) {
			printk("%s ignore codec_set_buildin_mic\n", __func__);
			ret = 0;
			break;
		}
#endif
...
static int codec_codec_ctl(struct codec_info *codec_dev, unsigned int cmd, unsigned long arg)
{
	...
	/*DUMP_IOC_CMD("enter");*/
	{
		...
		case CODEC_INIT:
#if XB_SND_FAST_START
			if(++codec_fast_init == 1) {
				printk("%s ignore init codec\n", __func__);
				ret = 0;
				break;
			}
#endif
...
#ifndef time_codec_init
	INIT_WORK(&delayed_work_spk, codec_set_speaker);
#endif
	schedule_work(&delayed_work);
#else
	thread = kthread_create(codec_fast_start, NULL, "codec_fast_start");
	if (IS_ERR(thread)) {
		return PTR_ERR(thread);
	}
	wake_up_process(thread);
#endif
#endif // XB_SND_FAST_START
