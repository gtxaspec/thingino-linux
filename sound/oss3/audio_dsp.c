...
static enum hrtimer_restart jz_audio_hrtimer_callback(struct hrtimer *hr_timer) {
	...
	long long time_usec_1  = 0;
	long long time_usec_2  = 0;
		...
		if(route && route->state == AUDIO_BUSY_STATE){
			if(route->index != AUDIO_ROUTE_DMIC_ID){
				...
				if(route->index == AUDIO_ROUTE_AMIC_ID){
					...
					do_div(t_nsec , 1000);
					time_usec = t_nsec;
					tv.tv_usec = do_div(t_nsec , 1000000);
					tv.tv_sec = t_nsec;
...
//				printk("jz_audio_hrtimer_callback:id = %d index = %d dma_currentaddr = 0x%08x\n",id, index,dma_currentaddr);
...
static int audio_dsp_probe(struct platform_device *pdev)
{
	...
	dspdev->miscdev.minor = ingenic_static_minor;
