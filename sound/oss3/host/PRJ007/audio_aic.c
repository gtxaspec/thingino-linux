...
{
	printk("PRJ007 AIC registers list:\n");
...
void set_resume_config(void)
{
	struct audio_aic_device *aic = globe_aic;
	struct platform_device  *pdev = &jz_aic_device;
	struct audio_data_type data_type;
#ifdef CONFIG_KERNEL_4_4_94
	aic->ce_rclock = devm_clk_get(&pdev->dev, "ce_i2sr");
	if (IS_ERR(aic->ce_rclock)) {
...
//#endif
	...
	if(record_flag){
		...
		//aic->mic_pipe->dma_config.dst_addr_width = DMA_SLAVE_BUSWIDTH_2_BYTES;
		__i2s_disable_signadj(aic);
		//printk("aic->codec_mic_info->data_type.frame_size = %d\n",aic->codec_mic_info->data_type.frame_size);
		//printk("aic->codec_mic_info->data_type.frame_vsize = %d\n",aic->codec_mic_info->data_type.frame_vsize);
		//printk("aic->codec_mic_info->data_type.sample_channel = %d\n",aic->codec_mic_info->data_type.sample_channel);
		//printk("aic->codec_mic_info->data_type.sample_rate = %d\n",aic->codec_mic_info->data_type.sample_rate);
