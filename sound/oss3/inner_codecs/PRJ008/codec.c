...
static void CODEC_CAGR2_init(struct timer_list *t)
{
	if(time_status <= 7) {
...
static void CODEC_CAADCR1_init(struct timer_list *t)
{
	if(time_status == 0) {
...
static void CODEC_CADACR1_init(struct timer_list *t)
{
	if(time_status == 0) {
...
static void CODEC_CAADCR2_init(struct timer_list *t)
{
	if(time_status == 0) {
...
static int CODEC_CAADCR2_init(void)
{
	if(time_status == 0) {
	    codec_reg_set(CODEC_CAADCR2, 0, 0, 0x1);
		mod_timer(timer,jiffies + HZ/100);
...
#ifdef CONFIG_KERNEL_5_15
	timer_setup(timer,CODEC_CAGR2_init,0);
#else
	init_timer(timer );
...
#ifdef CONFIG_KERNEL_5_15
	timer_setup(timer,CODEC_CAADCR2_init,0);
#else
	init_timer(timer );
	timer->function = CODEC_CAADCR2_init;
#endif
...
#ifdef CONFIG_KERNEL_5_15
	timer_setup(timer,CODEC_CAADCR1_init,0);
#else
	init_timer(timer );
...
#ifdef CONFIG_KERNEL_5_15
	timer_setup(timer,CODEC_CADACR1_init,0);
#else
	init_timer(timer );
