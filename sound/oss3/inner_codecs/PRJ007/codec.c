...
static int mic_gain = 3;
module_param(mic_gain, int, S_IRUGO);
MODULE_PARM_DESC(mic_gain, "ADCL MIC gain: 0:0db 1:6db 2:20db 3:30db");
...
#ifdef codec_fast_init
static int CODEC_CCR_init(void)
{
	if(time_status <= 7) {
        value_ccr |= value_ccr<<1 | 0x01;
		codec_reg_set(CODEC_CCR, 0, 7, value_ccr);
		mod_timer(timer,jiffies + HZ/50);
	}else if(time_status <= 8){
...
static int CODEC_CANACR_init(void)
{
	if(time_status == 0) {
...
static int CODEC_CANACR1_init(void)
{
	if(time_status == 0) {
...
static int codec_enable_record(void)
{
	...
	init_timer(timer );
	timer->function = CODEC_CANACR_init;
    ...
    codec_reg_set(CODEC_CMICGAINR, 6, 7, mic_gain); //mic hw gain
	...
	codec_reg_set(CODEC_CGAINR, 2, 3, 0x3);
...
static int codec_disable_record(void)
{
	...
	codec_reg_set(CODEC_CGAINR, 2, 3, 0x0);
...
static int codec_enable_playback(void)
{
	...
	init_timer(timer );
	timer->function = CODEC_CANACR1_init;
	timer->expires = jiffies + (HZ/100);
	add_timer(timer);
	while(time_status <= 1){
		i++;
